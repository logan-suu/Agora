...
Resolve the requested PR or current branch and inspect the actual diff and code. Read the task's required documents and check G1-G7, R1-R13, the relevant current standing decisions (including D18 for Phase 11–14 work), C4/FE/WO/DEF, interface stability, projection privacy, layering, test realism, documentation sync, and phase exit criteria. Report actionable findings first with severity, file, tight line location, rule, and concrete remediation.
