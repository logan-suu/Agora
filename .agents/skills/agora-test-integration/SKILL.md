...
Read `current_phase` from the `summary` query and query each relevant `phase <id>`. Audit `tests/integration/cross-phase` against implemented phase-to-phase data flows; present a plan and wait for approval before adding missing tests. Run every implemented phase integration suite through the current phase, the cross-phase suite, full unit tests, typecheck, and lint. A newly registered phase may have only planned test paths: explicitly report that coverage as not yet implemented, do not claim it passed, and do not create empty tests. This distinction never permits excluding existing tests or required real-model regressions. Report per-layer and total pass/fail counts. Diagnose failures under R11 and do not skip, mock, or weaken assertions merely to obtain a green result.
