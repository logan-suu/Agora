...
2. Read `docs/task-status.json` and idempotently cascade every eligible `pending` task to `ready`. Before selecting work in Phase N, verify from phase metadata that Phase N-1's `integration_test` task is `done`; stop if this cross-phase gate is not satisfied. Then select the first `ready` task in `current_phase`.
