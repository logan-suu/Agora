...
it('phase10 macOS launcher persists Keychain settings across real server restart and consumes them in production tools', async () => {
  ...
  try {
    ...
    cleanups.unshift(async () => {
      if (free.listening)
        await new Promise<void>((done, reject) =>
          free.close((error) => (error ? reject(error) : done())),
        );
    });
    ...
    cleanups.unshift(() => exec('/usr/bin/security', ['delete-keychain', keychain]));
    ...
    ).rejects.toThrow('Another instance or stale control socket exists:');
    await stop();
    ...
    const masterKey = credentials.key();
    if (!masterKey) throw new Error('Ready credentials must contain a master key');
    ...
    cleanups.unshift(() => composition?.dispose());
    ...
    expect(log.includes(masterKey)).toBe(false);
    ...
    await finishWithCleanup(errors, cleanups);
  }
}, 180000);
