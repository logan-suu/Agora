...
it('phase10 authorized Go connection and automatic compaction diagnostic', async () => {
  if (process.env.AGORA_GO_DIAGNOSTIC !== 'authorized-2026-09-10')
    throw new Error('explicit diagnostic authorization is required');
  const root = resolve('.data/evals', `phase10-go-diagnostic-${randomUUID()}`);
  mkdirSync(root, { recursive: true });
  const budgetPath = process.env.AGORA_GO_DIAGNOSTIC_BUDGET ?? join(root, CONFIG.budgetFile);
      ...
      for (const message of options.messages) {
        for (const block of message.content) {
          if (block.type === 'tool-result' && block.isError) {
            stopped = true;
            throw new Error(
              `diagnostic tool failure before next model request: ${JSON.stringify(block.content).slice(0, 400)}`,
            );
          }
        }
      }
      if (stopped || priorRequests + records.length >= 20 || Date.now() - started > 900_000)
  ...
  const meter = new DiagnosticMeter(ledger, 'go-connection-and-pressure', 'diagnostic');
  function save() {
    writeFileSync(
      ...
      JSON.stringify(
        {
          ...
          stopped,
          failure,
          complete: results.length === 4 && !stopped,
        },
        null,
        2,
      ),
  ...
  console.log(JSON.stringify({ root, diagnosticQuotaLimit: 0.5, maxRequests: 20 }));
  save();
  try {
    for (const stage of ['connection', 'pressure']) {
      for (const model of MODELS) {
        const sandbox = new LocalTempSandbox();
        const taskId = `go-${stage}-${model}`;
