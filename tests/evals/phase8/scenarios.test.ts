...
import { PHASE8_INCREMENT_TASKS } from './tasks';
...
describe('Phase 8 deterministic Eval scenarios', () => {
  for (const task of PHASE8_INCREMENT_TASKS) {
    it(`${task.id} produces a final fail-closed result with hashed evidence`, async () => {
      const root = resolve('.data', `phase8-eval-test-${crypto.randomUUID()}`);
      roots.push(root);
