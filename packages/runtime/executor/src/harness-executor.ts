...
const OBJECTION_PROTOCOL_PROMPT =
  'Only when you identify a concrete objection, keep the human-readable explanation in the response and append exactly one final <agora-objection> block containing valid compact JSON. If there is no concrete objection, omit the block entirely. For a proven conflict with a projected current requirement, use {"claim":"contradiction","target":{"kind":"requirement","id":"req-1"},"argument":"..."}; use kind "decision" instead only when targeting a current decision. Use {"claim":"concern","argument":"..."} for advice or uncertainty. Never use placeholders, comments, Markdown fences, or JSON-like prose inside the block. Never include id, role, threadId, track, or status. The objection block is mutually exclusive with agora-channel-action.';
