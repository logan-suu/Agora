...
function durableFile(path: string, bytes: string | Buffer) {
...
export function inspectLocalReplacementBasis(
  ...
  const value = inspectLocalFileBytes(binding, path, helper);
  const content = value.content.toString('utf8');
  if (!Buffer.from(content).equals(value.content)) throw new Error('unsupported_file_encoding');
  return { ...value, content };
}

export function inspectLocalFileBytes(
  binding: LocalRootBinding,
  path: string,
  helper: string,
  ...
  return { identity: value.identity, metadata: value.metadata, content: bytes };
}
...
export function inspectLocalDirectory(
  ...
  identity: string;
  entries: { name: string; kind: 'file' | 'directory' | 'excluded' | 'unsupported' }[];
} {
  ...
  const path = directory ? `${directory}/__agora_listing__` : '__agora_listing__';
  const parents = targetParents(binding, path);
  const result = spawnSync(
    '/usr/bin/sandbox-exec',
    [
      ...
      helper,
      binding.root,
      binding.chain.map((p) => p.identity).join(','),
      '.agora-operations',
      binding.stagingIdentity,
      path,
      'list',
      parents.map((p) => p.identity).join(','),
    ],
    {
      cwd: '/',
      env: { NODE_ENV: 'production', PATH: '/usr/bin:/bin', HOME: '/', TMPDIR: '/' },
  ...
  if (
    !value ||
    Object.keys(value).sort().join(',') !== 'entries,identity' ||
    value.identity !== (parents.at(-1) ?? binding.chain.at(-1))?.identity ||
    !Array.isArray(value.entries) ||
    value.entries.length > 4096
  )
    throw Error('invalid_native_directory');
  const entries = value.entries.map((entry: { hex: string; kind: string }) => {
    if (
      !entry ||
      Object.keys(entry).sort().join(',') !== 'hex,kind' ||
      typeof entry.hex !== 'string' ||
      entry.hex.length > 510 ||
      !/^(?:[a-f0-9]{2})+$/.test(entry.hex) ||
      !['file', 'directory', 'excluded', 'unsupported'].includes(entry.kind)
    )
      throw Error('invalid_native_directory');
    ...
    if (
      ...
      throw Error('unsupported_directory_name');
    return { name, kind: entry.kind as 'file' | 'directory' | 'excluded' | 'unsupported' };
  });
  if (new Set(entries.map((entry: { name: string }) => entry.name)).size !== entries.length)
    throw Error('invalid_native_directory');
  if (!verifyBinding(binding) || !verifyParents(parents)) throw Error('root_identity_changed');
  return { identity: value.identity, entries };
}
...
async function applyLocalTransaction(
  ...
  if (
    ...
    throw new Error('invalid_file_change');
  const content =
    typeof request.content === 'string' ? request.content : Buffer.from(request.content);
  const binding = structuredClone(request.binding);
  const expected = {
    ...structuredClone(request.expected),
    content:
      typeof request.expected.content === 'string'
        ? request.expected.content
        : Buffer.from(request.expected.content),
  };
