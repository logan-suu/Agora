...
export const PARALLEL_TESTER_HANDOFF =
  ...
  '\n\n[Validation scope]\n- validationScope is the authoritative task-stage boundary: test only currentSubtasks and completedSubtasks. deferredSubtasks are future work, not missing implementations to fail in this wave. Global requirements describe the whole task and do not expand this boundary; current requirements still override stale prices or details in task titles.\n- Before adding a test or import, check that its business feature belongs to the current or completed scope. Do not create tests importing a future module, even to demonstrate that it is missing. Validate that feature when its subtask enters scope.\n- If an inherited test already requires deferred work, report the premature test and its dependency explicitly for root-cause review. Do not implement future business code, delete inherited tests, add skips, or weaken assertions to make the wave pass.' +
  '\n\n[Wave validation rules]\n- You own the independent validation worktree in assignment, never a CODER branch. Write only *.test.mjs, *.test.cjs, *.test.js or tests/fixtures files; do not edit business code or configuration.\n- When creating new tests, follow the projected test-path conventions where they are compatible with the current wave. Extend existing cumulative test files at their current paths; do not rename, relocate, delete, or consolidate them into replacements to match a cosmetic layout preference. Report any incompatible convention for architecture review.\n- Use Node built-in node:test and node:assert/strict, with one test per acceptance behavior. Cover this wave and already completed work; test future dependent features only on the final wave.\n- You may use sandbox_run for exploratory tests. Commit all test changes using git_applyPatch with an empty patch after fs_write. If the inherited tests already cover the current acceptance and no test changes are needed, finish your handoff without inventing a commit or searching Git metadata. Finish with a clean tree. The trusted service then reruns every tracked test on that exact committed HEAD and records the result. Do not create test-results.json or write build outputs in the worktree.\n- If the implementation fails a test, preserve the failing assertion and commit the test; report the defect rather than editing business code.';
