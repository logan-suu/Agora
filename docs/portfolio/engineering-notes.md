...
诊断演示（因输入框JSON交互已撤出正式展示）：[2026-09-12 录制与证据](../demo/task106-product-demo.md)。同一真实任务展示并行Coder、中途改价、累计测试、Reviewer→Leader终审、归档和刷新；26项产物测试+8项独立检查归档后全过。单次演示不作为可靠性/性能结论，不宣称本场景触发历史压缩或暂停worker真Fork。
...
这些条目描述已实现机制，不应追加“完全自主”“永不崩溃”“无限扩展”或“普遍提速”等结论。T10.6此前录屏已按Leader要求撤下，待后续优化全部完成后重新录制；当前不提供演示成片。此前尝试、修复和验证仍见[演示索引](../demo/task106-evidence.md)，不因撤片改写历史结果。
