...
**[2026-09-15 研发测试清理]** 按蓝图§21 TEST-CLEANUP，每轮测试结束后归档结果、清单、hash和必要日志；核对进程/挂载与路径归属后，删除测试专用下载、压缩包、解压构建和安装副本及专用依赖目录。已证实遗留的测试下载也纳入清理。用户明确指定保留的交付文件除外；用户项目、正常项目依赖、全局共享缓存、安装应用和产品状态不在范围。历史保留原产物身份，新增清理回执记录已删除及空间变化，当前报告不再提供失效下载入口。无需为纯清理重跑付费回归，代码/构建输入有变化时仍按原门禁执行。
...
原始输出经筛选/验收并确认无后续用途后清理，不只是移到忽略目录长期囤积。保留原始失败事实；历史迁移区原文/hash不改。其他任务唯一验收依据和未解决问题不并入删除。当前树删除不等于Git历史体积减少。仅文档/证据整理无需虚构TDD或新增付费实验；测试/执行代码变化仍执行适用门禁。
...
Leader已确认四批方向，并于2026-09-13改为现在拆分全部任务：Phase 11–31均已登记，未定细节由对应设计任务先定稿。禁止自行扩展阶段仍适用于未经授权的新路线，不阻止本轮已登记任务。局部待决项写入任务notes/来源规格，注明最晚定稿点与受阻实现；设计任务可先ready，未决不自动让全部阶段blocked。当前状态仍只在task-status.json，开发计划中的决策绑定是规格输入，不维护第二份状态或延期台账。
...
Phase11–31每阶段3–7项，最后一项必须是集成测试与发布验收；integration_test指向该末项，出口显式依赖同阶段其余全部任务，每个下一阶段任务显式依赖前一出口。117项后续任务由原110项加D19迁移7项构成；未开工任务编号对照见开发计划§18.12及§18.14，历史旧ID不可直接作为当前执行指令。设计文档章节号不随任务ID改变。

阶段done须全部任务done且出口真实通过；GitHub发布另核对人工dev→main合并、已验证提交、安装产物、tag与Release。发布准备及实际发布证据归属出口任务历史，不在索引新增release字段或第二份状态文件。发布暂缓不额外阻塞满足出口的后续开发，未发布不能写已发布。详细协议见开发计划§18.11；check验证基础索引，不替代逐阶段出口依赖、范围覆盖或发布证据审查。
