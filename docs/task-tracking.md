...
Leader已确认四批方向，并于2026-09-13改为现在拆分全部任务：Phase 11–30均已登记，未定细节由对应设计任务先定稿。禁止自行扩展阶段仍适用于未经授权的新路线，不阻止本轮已登记任务。局部待决项写入任务notes/来源规格，注明最晚定稿点与受阻实现；设计任务可先ready，未决不自动让全部阶段blocked。当前状态仍只在task-status.json，开发计划中的决策绑定是规格输入，不维护第二份状态或延期台账。
...
Phase11–30每阶段3–7项，最后一项必须是集成测试与发布验收；integration_test指向该末项，出口显式依赖同阶段其余全部任务，每个下一阶段任务显式依赖前一出口。110项后续任务由原94项加16个新出口构成；未开工任务编号对照见开发计划§18.12，历史旧ID不可直接作为当前执行指令。设计文档章节号不随任务ID改变。
