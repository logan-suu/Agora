...
const record = { base, target, startedAt: new Date().toISOString(), sourceHash: hash(join(directory, 'session-lookup-probe.c.txt')), controllerHash: hash(fileURLToPath(import.meta.url)), identity: { uid: identity.uid, dev: identity.dev, ino: identity.ino }, availableBefore: space(), os: run('/usr/bin/sw_vers', ['-buildVersion']) };
const evidence = join(directory, `session-lookup-${suffix}.json`);
try {
  record.build = run('/usr/bin/clang', ['-x', 'c', '-std=c11', '-Wall', '-Wextra', '-Werror', '-Wno-error=deprecated-declarations', '-mmacosx-version-min=15.0', '-framework', 'Security', '-lbsm', join(directory, 'session-lookup-probe.c.txt'), '-o', executable]);
  ...
  record.lookupWhileDescendantAlive = run(executable, ['lookup', String(parent.asid)]);
  ...
  record.lookupAfterDescendantExit = run(executable, ['lookup', String(parent.asid)]);
  record.testedAsid = parent.asid;
  ...
  if (record.testedAsid) {
    record.lookupAfterBootout = run(executable, ['lookup', String(record.testedAsid)]);
    record.delayedLookups = [];
    for (let i = 0; i < 3; i++) {
      await new Promise((resolve) => setTimeout(resolve, 3000));
      record.delayedLookups.push({ at: new Date().toISOString(), result: run(executable, ['lookup', String(record.testedAsid)]) });
    }
  }
  ...
  writeFileSync(evidence, `${JSON.stringify(record, null, 2)}\n`);
  console.log(JSON.stringify({ evidence, lookupWhileDescendantAlive: record.lookupWhileDescendantAlive, lookupAfterDescendantExit: record.lookupAfterDescendantExit, lookupAfterBootout: record.lookupAfterBootout, delayedLookups: record.delayedLookups, error: record.error, build: record.build, cleanup: record.cleanup }));
}
