...
按Leader新决定，不再等待Developer ID/公证权限。仍需两个不同ad-hoc构建之间的钥匙串访问/授权连续性、真实隔离下载安装，以及Apple Silicon macOS15和干净无开发工具环境证据。未收到可用环境前不能补写通过。安装验证使用本机临时复制目录，不代表已验证Gatekeeper隔离下载或普通用户安装路径。阶段出口、固定提交候选及人工PR合并仍按原流程执行。
