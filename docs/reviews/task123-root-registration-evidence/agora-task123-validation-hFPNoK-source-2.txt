...
function source(state: AppState, operation: LocalBindingOperation) {
  ...
  if (
    ...
    localRecordHash(state.messages.find((m) => m.msgId === operation.sourceMessageId)) !==
      localRecordHash(operation.sourceMessage)
  )
    throw new Error('workspace_control_source_invalid');
}
...
export class LocalBindingCoordinator {
  ...
  commitBinding(input: LocalBindingRequest): Promise<LocalBindingOperation> {
    ...
    return this.serial(async () => {
      if (Object.hasOwn(request, 'sourceMessage'))
        assertLocalControlMessage(request.sourceMessage, request);
  ...
  private async finish(operation: LocalBindingOperation): Promise<LocalBindingOperation> {
    ...
    if (!operation.sourceMessage) leader(state, operation.sourceMessageId);
