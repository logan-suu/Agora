...
import { humanGateAlreadyResumed } from '../../../apps/web/src/server/human-gate-replay';
...
const output = resolve('test-outputs/langgraph-spike-round2/results', process.env.LG_RUN_ID ?? '.');
...
it('joins a graph to production local WorkerRuntime and durable Leader resolution', async () => {
  ...
  try {
    ...
    const verifyCompletedWorkspace = async () => {
    ...
    const crash = (boundary: string) => {
      ...
      const current = JSON.parse(readFileSync(join(owner.root, 'tasks/projects', scope.projectId, 'tasks', scope.taskId, 'state.json'), 'utf8'));
      expect(current.workers[0]?.status).toBe('done');
      writeFileSync(join(output, 'fault-window.json'), JSON.stringify({pid:process.pid,signal:'SIGKILL',boundary,workers:current.workers,activeLeases:scheduler.activeCount,compositionDisposed:true,evidence},null,2));
      log('fault-window-kill', { signal: 'SIGKILL', boundary });
      process.kill(process.pid, 'SIGKILL');
      throw Error('fault process unexpectedly survived');
    };
    const originalPutWrites = saver.putWrites.bind(saver);
    saver.putWrites = (...args: [unknown, Array<[string, unknown]>, string]) => {
      if (mode === 'crash-reconcile' && args[1].some(([key,value]) => key === 'phase' && value === 'finished')) crash('before graph pending output putWrites');
      return originalPutWrites(...args);
    };
    const originalPut = saver.put.bind(saver);
    saver.put = (...args: [unknown, {channel_values: {phase?:string}}, unknown, unknown]) => {
      if (mode === 'crash-reconcile' && args[1].channel_values.phase === 'finished') crash('before final graph checkpoint put');
    ...
    const graph = new StateGraph(shape)
      ...
      .addNode('continued', async () => {
        ...
        if (current.workers[0]?.status === 'done') {
          expect(canonicalReceipt).toBeDefined();
          expect(humanGateAlreadyResumed(current, actionId, canonicalReceipt!)).toBe(true);
          await verifyCompletedWorkspace();
          log('reconciled-completed-dispatch');
          return { phase: 'finished' };
        }
        if (current.workers[0]?.status === 'failed') throw Error('NEEDS_ATTENTION_FAILED_WORKER');
    ...
    if (mode === 'initial') {
    ...
    } else if (mode === 'inspect' || mode === 'inspect-crash') {
      ...
      expect(snapshot.next).toEqual(mode === 'inspect-crash' ? ['continued'] : ['gate']);
      expect((await get()).workers[0]?.status).toBe(mode === 'inspect-crash' ? 'done' : 'paused');
      ...
      expect(allEvents().filter(e => e.event.startsWith('dispatch-') || e.event === 'tool-start').length).toBe(before);
      evidence.scope = 'Graph-only no-op after completion';
    } else {
      const beforeLeaderRequest = JSON.stringify(await get());
      ...
      if (mode === 'duplicate' || mode === 'reconcile' || mode === 'crash-reconcile' || mode === 'failed-duplicate') {
        expect(JSON.stringify(await get())).toBe(beforeLeaderRequest);
        expect(canonicalReceipt).toBeUndefined();
        canonicalReceipt = (await get()).messages.find(message => message.msgId === actionId)?.payload.resolution as HumanGateResolutionReceipt;
      }
      expect(canonicalReceipt?.workerResumes).toHaveLength(1);
      evidence.resolution = canonicalReceipt;
      if (mode === 'failed-duplicate') {
        expect((await get()).workers[0]?.status).toBe('failed');
        expect((await graph.getState(cfg)).next).toEqual(['continued']);
        expect(allEvents().filter(e => e.event === 'dispatch-resumed')).toHaveLength(1);
        ...
        const before = allEvents().filter(e => e.event.startsWith('dispatch-') || e.event === 'tool-start').length;
        let invocationConfig = cfg;
        if (mode === 'crash-reconcile') {
          let found = false;
          for await (const snapshot of graph.getStateHistory(cfg)) {
            if (snapshot.next.length === 1 && snapshot.next[0] === 'continued') {
              invocationConfig = { ...cfg, configurable: snapshot.config.configurable };
              found = true;
              log('fixture-selects-prior-pending-checkpoint', { checkpointId: snapshot.config.configurable.checkpoint_id });
              break;
            }
          }
          if (!found) throw Error('missing prior dispatch checkpoint');
        }
        graphCall = graph.invoke(mode === 'reconcile' || mode === 'crash-reconcile' ? null : new Command({ resume: { actionId } }), invocationConfig);
        await graphCall;
        expect((await graph.getState(cfg)).next).toEqual([]);
        if (mode === 'duplicate' || mode === 'reconcile' || mode === 'crash-reconcile' || mode === 'failed-duplicate') expect(allEvents().filter(e => e.event.startsWith('dispatch-') || e.event === 'tool-start').length).toBe(before);
