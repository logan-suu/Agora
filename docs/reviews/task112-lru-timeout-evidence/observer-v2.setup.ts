...
const root = '.data/diagnostics/agora112-lru-timeout/run2';
...
for (const method of ['read', 'write', 'run'] as const) {
  ...
  (LocalTempSandbox.prototype as any)[method] = async function(...args: any[]) {
    ...
    try {
      ...
      emit('sandbox-end', { operation: method, elapsedMs: now() - start, ...(method === 'run' ? { exitCode: result.code } : {}) }); return result;
