...
**[2026-09-02 架构决策更新] D12 Phase 7 角色契约：** `RoleSpec` 是不含启停状态的能力定义，`RosterEntry={spec,status}` 才是项目成员。role id 入库前转大写并校验 `[A-Z][A-Z0-9_-]*`，按规范化值唯一，`leader` 保留；`COORDINATOR` 必须存在且恒 enabled。`addRole` 对同 id、同 spec、enabled 的重放为 no-op；同 id 不同 spec，或同 id 已为 disabled/departing/departed（即使 spec 相同）均冲突，调用方必须改走 enable 或阶段 7.2 离职状态机，禁止借 add 隐式复职。`enableRole` 只接受 disabled，`disableRole` 只接受 enabled，同态重放为 no-op；departing/departed 只能由阶段 7.2 的离职状态机推进，普通 enable/disable 不得越过。任何消费 roster 的 Coordinator、Leader Intent、WorkerRuntime、Channel 生命周期和消息提交入口都必须把持久 roster 作为必需权威源并检查 enabled；缺失依赖时 fail-closed，不得只检查条目存在或静默跳过校验。已 applied 的 Leader 指派若在 Coordinator 消费前变为非 enabled，返回 `required_role_unavailable:<role>` humanGate，由 Leader 选择 enable/reassign/abort，不抛未处理异常。
...
Roster 操作是纯函数产生下一份 `{roster,channels}`：`addRole` 新增 enabled 条目并重算 main，仅现有 enabled 且规范化 spec 完全相同的重放为 no-op；现有 disabled/departing/departed 即使 spec 相同也冲突。`enableRole`/`disableRole` 只做合法状态转换并重算 main，同态操作为 no-op；Coordinator 的非 enabled 转换、同 id 异 spec、普通操作触碰 departing/departed 均明确失败。`removeRole` 不属于该底层管理器，它由 7.2 的离职服务驱动 `enabled|disabled → departing → departed`，在最终转换前必须已有合法 HandoffPacket 与责任转移结果。
