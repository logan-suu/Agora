...
class HarnessExecutor implements Executor {
  ...
  async step(context: StepContext): Promise<StepResult> {
    ...
    return { kind: 'done', output: lastAssistantText(agent),
             reachedSafeBoundary: true,
             mutations: [append('messages', asAgoraMessage(text))] };
  }
  saveSafePoint()   { return String(this.handle?.agent.id); }   // D4 恢复游标
  loadSafePoint(_c) { /* 阶段 0 no-op；8.1 才做 Fork 恢复 */ }
  injectInbox(view) { this.view = view; }              // 下一轮 pre-step 据此重投影
  // setup 回调里挂钩子：
  //   agent/pre-step: () => ({ kind:'enter', messages:[projectionMessage(view)] })  // D1 覆写
  //   agent/request:  (p,next) => ({ ...await next(), provider, model: spec.model })
}
```
