...
> **当前版本 v3.5 / 2026-09-05：** 上游对齐蓝图 v1.4，新增 D16 终审确认协议、同 topic supersedes 约束与 Phase 8 出口/Eval 验收矩阵；下方单行版本串仅保留历史沿革，凡与当前标记规格冲突均以本文后续 `[2026-09-05 架构决策更新]` 为准。
...
*收敛：* test 全过 + REVIEWER 通过只生成 `completionCandidate` → D16 `completion_confirmation` gate → Leader `approve_completion` → Phase 8 顺序退化直接 finalize/done；Phase 9 起再经 integrating 合并 → done。Leader `request_changes <rationale...>` 则按现有保守策略重开待修改 subtasks、路由 CODER，并等待新一轮 review 与新 gate。
