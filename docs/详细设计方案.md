...
>
> **[2026-09-01 架构决策更新] v2.7 Sub 生命周期校准：** participants 改为集合标准化；sub 增 `threadId/topic/createdBy`；定稿可信 actor、稳定 actionId、同 thread 去重、权限关闭、project 串行与 Channel-first/D6-message-second 可重试 saga。6.2 只关闭并公告，6.3 才冒泡摘要，Phase 8 才处理 humanGate/resume。
...
interface ChannelBase {
  ...
  localContext: MessageRef[ ];                 // 任务级消息引用；禁止使用裸 msgId
...
// enabled roster 是角色集合：每项必须为非空 role、全局唯一，且不得等于保留身份 'leader'。
// createMainChannel 与注册表校验使用同一前置校验，禁止工厂先产生无效 Channel 再等持久层拒绝。
...
interface AppState {
  ...
  messages: Message[ ]; roster: RoleSpec[ ];  // Channel 注册表归 Project.channels，任务 State 不复制
...
Channel 注册表边界必须统一校验：`channelId` 项目内唯一；恰有一个 main，其 `closed=false`、不带 `taskId`、participants 为 leader + 当前已启用 roster；sub 的 `taskId/threadId/topic` 为非空字符串、`createdBy` 为 leader 或已启用角色、participants 至少含 leader 与一个已启用角色。participants 是集合，写入前统一标准化为 leader 在前、其余角色按 enabled roster 顺序。读取已关闭 sub 的历史允许，但关闭后不得再提交新消息。阶段 7 修改 roster 时必须同步重算 main participants，该规则不在 6.1 抢跑热插拔实现。
...
type Mutation =
  ...
  | { field: 'workers' | 'subtasks' | 'roster' | 'requirements'; op: 'mergeById'; value: { id: string } }
...
**Channel 持久注册表 + Inbox 读模型（**`comm/channels/`**，阶段 6）：**
...
`ProjectChannelStore.initialize/commit` 必须在写盘前执行上述注册表不变量校验；`expectedRevision` 不匹配必须明确失败，不得 last-write-wins 静默覆盖。返回的 snapshot、Channel、participants 与 localContext 均为防御性拷贝，读模型不得泄漏可原地改写的持久真相引用。

**[2026-09-01 架构决策更新]升级恢复与 no-op 定义。** 若任务 `state.json` 已存在而同项目 `channels.json` 缺失，所有会恢复或访问该任务的组合根/HTTP/SSE 入口必须先用当前已启用 roster 执行 create-if-absent main backfill，再继续读取或提交；并发入口依赖 `initialize` 的项目串行与幂等语义收敛到同一快照。仅 ENOENT 允许补建；JSON 损坏、projectId/revision/Channel 不变量不合法必须 fail-fast，不得用默认 main 覆盖。Channel 语义比较递归忽略普通对象属性顺序；participants 是集合，提交前标准化为 `leader` 在前、其余角色按 enabled roster 顺序，成员相同而输入顺序不同属于 no-op。Channel 注册表数组顺序与 localContext 引用顺序保留；语义相同的 commit 返回 `changed=false` 且 revision 不变。

**6.1→6.2 legacy sub 迁移：** 6.1 已允许内部注册并持久化 sub，但当时 schema 尚无生命周期三字段。缺少全部 `threadId/topic/createdBy` 的旧 sub 在 6.2 首次项目初始化时，于同一项目队列内标准化为 `threadId='legacy-'+channelId`、`topic='Legacy channel '+channelId`、`createdBy='leader'`，并通过正常 expectedRevision commit 一次性写回；三项只缺一部分代表撕裂数据，必须 fail-fast。迁移不重写消息、不生成 lifecycle announce、不改变 closed/localContext；完成后所有新写入都必须满足完整 schema。

> [2026-09-01 实装记录·任务 6.1] `core/domain/channel.ts` 已落 Channel/MessageRef schema 与注册表不变量；`comm/channels` 已落 `ProjectChannelStore` 端口、`JsonProjectChannelStore` 原子快照适配器、`DerivedChannelInbox` 纯读模型和提交前寻址策略。JSON 适配器按项目串行，使用 revision 乐观并发控制与同目录临时文件原子替换，返回防御性拷贝；Web 组合根按启用 roster 幂等初始化 main，并允许 HTTP/SSE 对已注册 sub 动态寻址。`MessageService` 在任务 State commit 前加载项目注册表并校验 Channel 存在、sub task 绑定、sender participant 与 closed；失败路径测试确认既不追加 `AppState.messages`，也不发布 `MessageCommitted`。`project(state,role,roster)` 未改，localContext 投影仍留 6.3。

阶段 5 退化规则：每个 `TaskScope` 只派生一个固定 `channelId='main'`，HTTP/SSE 均显式携带 `projectId/taskId/channelId`；浏览器 POST 不接受可信的 `fromRole`，受信任单用户边界内由服务端标记为 `leader`。阶段 6 在 `.data/projects/{projectId}/channels.json` 落地 `ProjectChannelStore`：初始化必须幂等，提交必须按项目串行、校验 `expectedRevision` 并原子替换快照，进程重启后可恢复。任务 6.1 只落注册表、不变量、持久化与 inbox 读模型；sub 群创建/关闭属 6.2，`localContext` 投影和冒泡属 6.3。完整账号认证不在项目范围；阶段 6 只对服务端固定的 leader 与 worker 角色执行注册表校验，不得因 DEF-015 的 Channel 部分解决而宣称可公开无认证部署。按 D8，阶段 5–9 只承诺单实例自托管后端；Vercel 只承载前端。
...
```text
<agora-channel-action>{"kind":"open_sub_channel","requestedRoles":["TESTER"],"topic":"reproduce cache bug"}</agora-channel-action>
<agora-channel-action>{"kind":"close_sub_channel","channelId":"sub-task-a-action-1"}</agora-channel-action>
```

控制块不得携带 actor 或 actionId；HarnessExecutor 以本 Step 生成的 assistant `Message.msgId` 作为 actionId，把控制块从 `display` 中剥离，并在该 Message 的 payload 留结构化 request 审计，同时返回 `StepResult.output.channelAction`。畸形 JSON、未知字段/kind、多个控制块、控制块后还有非空文本均令 Step 明确失败，既不提交 Message mutation 也不执行 Channel 动作。WorkerRuntime 在 State transition 前调用可选 `handleOutput(state, role, output)`；组合根用 `role` 绑定 actor 并执行 Channel-first saga，之后才提交该 Step 的普通 mutations。无 channelAction 时行为与既有 worker loop 完全一致。该扩展只使用 `StepResult.output` 既有开放结构，不修改 Executor 签名。

**跨存储提交与恢复：** 对合法生命周期请求，先幂等提交 ProjectChannelStore，再以稳定 msgId 经 MessageService/D6 提交原 Leader 消息或 main 群 lifecycle `announce`。消息提交/投递失败不回滚 Channel；同 actionId 重试先读取既有 Channel，再补齐缺失消息，禁止重复开/关群。创建 announce 的 msgId=`channel-open:${actionId}`，关闭 announce 的 msgId=`channel-close:${actionId}`。UI/HTTP 以 Channel 快照为事实，announce 是可恢复审计/展示面。
...
// leader 视图：唯一 main + taskId 匹配的 sub；不返回 localContext、bubbledSummary 原文或消息 payload
```
...
**关闭与冒泡阶段边界：** 6.2 close 只置 `closed=true`、阻止后续 sub 消息并写 main 生命周期公告；6.3 扫描 `closed && bubbledSummary===undefined`，以稳定 msgId 先把结论摘要 + 关键决策及理由写入 main，再提交 `bubbledSummary`。失败重试按 msgId 去重。`threadId` 在 6.2 只保证问答/升级消息的持久链与路由；异议挂起、humanGate 裁决和 resume 仍归 Phase 8。
...
1. 去除前后空白后，只有第一个 token 精确匹配单一 `@ROLE` 才识别为 `assign`；多目标 token 或畸形 mention 返回 `rejected`。显式 deferred 命令只按首 token 识别：`/requirement`、`/decision`、`/priority` → Phase 9，`/approve`、`/reject`、`/resolve-objection` → Phase 8，`/channel` → Phase 6；未知 `/command` 返回 `rejected`，其余文本为 `chat`。Phase 5 不调用 LLM 猜意图，不从任意自然语言关键词臆造 `Requirement`/`Decision` 的必填字段。
...
5. 最终 State、消息与产物引用必须支持页面刷新恢复，但刷新恢复不得依赖活着的 Harness 或 Docker 容器。run 进入 `completed|needs_attention|failed` 后，组合根必须先把待展示产物从沙箱 worktree 复制到 `.data/projects/{projectId}/tasks/{taskId}/artifacts/`，经 reducer/TaskStateStore 把耐久引用写回既有任务字段，再释放 HarnessExecutor、MCP catalog、Git client 与 DockerSandbox；注册表只保留终态摘要，不保留可执行 composition。Phase 5 不承诺活动 run 在后端进程重启后自动续跑；若进程丢失，读模型必须显示中断/非活动状态，不得保留虚假的 running/active 展示。
...
interface ChannelProjection {
  channelId: string;
  kind: ChannelKind;
  localContext: MessageRef[]; // 只含已裁剪的作用域引用，不含原始群聊 log
}
...
function project(
  ...
  channelContext: readonly ChannelProjection[] = [], // 6.3 才由 comm/channels 提供
): ProjectionView {
...
> [2026-09-01 架构决策更新] 阶段边界校准：任务 6.1 只落地持久 Channel 注册表与 inbox 读模型，不修改现有 `project(state, role, roster)` 调用面。任务 6.3 在不改变前三个参数语义的前提下增加可选 `channelContext`；该输入必须由 `comm/channels` 按 participants 与 `{taskId,msgId}` 引用预先裁剪，投影层不直接扫项目全量 Channel 或原始群聊 log。
...
> [2026-09-01 架构决策更新] Phase 6 不为落 Channel 而提前实现上述全量 Project/KB/成本/调度能力。实装以 `ProjectChannelSnapshot { projectId, revision, channels }` 作为 `Project.channels` 的持久切片，真相落于 `.data/projects/{projectId}/channels.json`；未实现字段不用假数据填充。后续全量 Project Store 落地时必须迁移/嵌入该持久切片，不得并存第二份 Channel 注册表。
...
Eval Suite 是研发与作品集证据层，不进入 Agent 的生产决策回路，不修改 `AppState`、`Mutation`、`Executor`、`SandboxManager`、`TaskStateStore`、`ProjectChannelStore` 或 `MessageBus` 签名。确定性 Vitest/阶段集成测试与 G5 仍是合并阻断门禁；模型 Eval 允许非确定性，只输出测量结果和失败分类，单次通过不得替代门禁。
...
interface EvalResult {
  ...
  seed?: number;
  outcome: Record<string, boolean | number | string>;
...
以上是 Eval 层契约，不是领域 State；结果写入独立、gitignored 的评测运行目录，汇总报告只保存统计与非敏感 artifact 引用。公开数据集的原始内容、许可证与大体积镜像不复制进仓库，固定数据集名称、版本、任务 ID 与 adapter 版本即可复现。

#### 11.3 四类 Grader

- **Outcome**：真实测试、补丁应用、需求断言与产物完整性；凡涉及 Agent 代码执行必须复用 DockerSandbox/等价隔离并检查真实退出码、超时和输出。
- **Process**：角色投影不含原始 log、Channel participant/localContext 隔离、路由与 dependsOn、Leader authority、safePoint、handoff、State 写入与消息提交顺序等结构化不变量。
- **Efficiency**：端到端时延、模型/工具调用、token、费用、返工轮次、人工介入和资源峰值；缺失指标显式为 unknown，不推算伪精度。
- **Safety**：宿主执行禁令、路径/符号链接逃逸、网络策略、敏感信息、工具白名单和资源回收。

LLM-as-judge 只能用于主观质量的补充判定，必须固定 rubric、保留 judge 模型版本并人工抽检；可由测试或不变量确定的结果不得改用 LLM 猜测。

#### 11.4 阶段增量

- **Phase 6**：建立 Eval runner、任务/结果 schema、指标采集与 8–12 个 Phase 0–6 基线任务；覆盖编码闭环、消息事实链、main/sub 作用域、localContext 与冒泡。
- **Phase 7**：加入角色增删、强制交接、接手与 Coordinator 不可删除。
- **Phase 8**：加入 blocking/advisory、Leader authority、Terminate & Fork 与 trace 完整性。
