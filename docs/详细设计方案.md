...
阶段 5 退化规则：每个 `TaskScope` 只派生一个固定 `channelId='main'`，HTTP/SSE 均显式携带 `projectId/taskId/channelId`；浏览器 POST 不接受可信的 `fromRole`，受信任单用户边界内由服务端标记为 `leader`。阶段 5 的 JSON State 快照是必须能力，SQLite 仍为可选迁移；动态 `Project.channels`、participants 授权、sub 群与角色 inbox 留阶段 6。完整账号认证不在项目范围，公开暴露前仍须完成 DEF-015。
