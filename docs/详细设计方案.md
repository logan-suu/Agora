...
这些是已接受设计中的新增能力，尚未成为生产导出。`CommandReceipt` 包含 RunResult 原四字段、commandId/inputVersion/policyHash/工具版本、开始结束时间、输出引用与收敛状态；原始命令输出按 D15 不直接进入通用 Trace，秘密过滤不替代访问隔离。
...
| `scripts/build.mjs`、`package.mjs` | 当前只交付arm64原生macOS构建，拒绝跨架构代跑；旧x64入口不构成当前支持承诺；加入Git/pnpm、Node附带npm/npx、两个预编译helper、包内相对启动脚本和来源/许可证。构建时设置helper最低部署目标13.5，产品仍要求macOS15。默认工作树输入逐文件hash仅用于开发验证，固定提交模式才可进入发布候选 |
