...
| `toolchain-installation.ts` | 清单覆盖文件hash、执行位、全部内部symlink目标及必要组件；主进程在启动Node前验证，服务再次验证后才报告工具就绪。失败页指出损坏组件；预览HTTP/SSE只返回就绪/版本，不返回凭据或命令入口 |
...
| `scripts/sign.mjs`、`media.mjs`、`validate-media.mjs` | 按嵌套顺序签名，工具二进制签名后生成清单，再封装bundle；Node/Electron与普通helper区分JIT entitlement。无身份时显式ad-hoc开发验证；有效Developer ID可传入独立介质步骤，生成DMG/ZIP及校验和。脚本不自动上传公证或发布；当前采用ad-hoc DMG发布，不要求Developer ID/公证/staple，隔离下载安装及ad-hoc升级钥匙串连续性仍需真实验证 |
| `src/upgrades.ts`、`upgrade-files.ts`、`storage.ts` | 升级要求持有同一state owner，路径、格式、原/目标hash及固定转换器身份先校验；`preparing`备份准备、`prepared`、逐文件应用/回滚journal、最后格式提交及终态回执。准备未完成时只允许核验原状态后撤回；备份坏、未知转换、源文件异值、失去owner均拒绝。所有备份保留，未闭合阻止服务常规入口，不移动workspace或自动恢复工作 |
