...
**薄执行器（**`runtime/executor/harness-executor.ts`**）——构建骑** `ctx.subagents`**、逐步驱动用** `runStep`**：**
...
- Harness：起单个子 agent（可直接用 `ctx.subagents.start`，`toolFilter`/`persona` 映射 CODER/TESTER 的 RoleSpec），配 `agent/pre-step`(投影) + `agent/request`(固定模型)，先不接 turn-stopping 抢占；`ctx.compaction` 直接挂默认 `BasicCompactionEngine`（零配置即用）。
...
- 编排：entry→coordinator→worker(CODER)→coordinator→worker(TESTER)→coordinator→finalize，无 integrate/humanGate（后补）。
- Worker：单 worker 顺序执行（`runOne` 直接 await），`paused` 恒 false（不抢占）。
- 执行器：只薄 `HarnessExecutor`，工具只接 fs + sandbox.run（阶段 0 由 `LocalTempSandbox` 承担，不依赖 Docker）。
- 投影：CODER 只投 subtask + 失败测试；TESTER 只投 acceptance + 当前分支/patch；无 sub 群/异议。
