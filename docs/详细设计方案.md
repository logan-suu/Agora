...
type Channel =
  ...
  | (ChannelBase & { kind: 'sub'; taskId: string });
...
Channel 注册表边界必须统一校验：`channelId` 项目内唯一；恰有一个 main，其 `closed=false`、不带 `taskId`、participants 为 leader + 当前已启用 roster；sub 的 `taskId` 为非空字符串、participants 至少含 leader 与一个已启用角色。读取已关闭 sub 的历史允许，但关闭后不得再提交新消息。阶段 7 修改 roster 时必须同步重算 main participants，该规则不在 6.1 抢跑热插拔实现。
...
**[2026-09-01 架构决策更新]升级恢复与 no-op 定义。** 若任务 `state.json` 已存在而同项目 `channels.json` 缺失，所有会恢复或访问该任务的组合根/HTTP/SSE 入口必须先用当前已启用 roster 执行 create-if-absent main backfill，再继续读取或提交；并发入口依赖 `initialize` 的项目串行与幂等语义收敛到同一快照。仅 ENOENT 允许补建；JSON 损坏、projectId/revision/Channel 不变量不合法必须 fail-fast，不得用默认 main 覆盖。Channel 语义比较递归忽略普通对象属性顺序，但保留 Channel、participants 与 localContext 等数组顺序；语义相同的 commit 返回 `changed=false` 且 revision 不变。
