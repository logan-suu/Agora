...
> [2026-08-31 架构决策更新] D7 消除 Channel 双重所有权：`Project.channels` 是项目级 Channel 注册表唯一真相源，`AppState.messages` 只保存任务级消息事实，故 `AppState` 与其 Mutation 联合不再声明 `channels`。跨项目/任务投递以 `TaskScope={projectId,taskId}` 信封定位；阶段 5 固定 `main` 群由作用域派生，阶段 6 的 Channel 写入由项目级 Channel 仓储/适配器负责，不借任务 reducer 伪装成任务字段。
...
**Channel/Inbox 读模型（**`comm/channels/`**，阶段 6）：**

```ts
interface ChannelInbox {
  inboxFor(project: Project, task: AppState, role: string): Message[ ];
...
阶段 5 退化规则：每个 `TaskScope` 只派生一个固定 `channelId='main'`，HTTP/SSE 均显式携带 `projectId/taskId/channelId`；浏览器 POST 不接受可信的 `fromRole`，受信任单用户边界内由服务端标记为 `leader`。阶段 5 的 JSON State 快照是必须能力，SQLite 仍为可选迁移；动态 `Project.channels`、participants 授权、sub 群与角色 inbox 留阶段 6。完整账号认证不在项目范围，公开暴露前仍须完成 DEF-015。按 D8，阶段 5–9 的该退化链路只承诺单实例自托管后端；Vercel 只承载前端，不得使用实例本地 `.data` 或进程内订阅充当 Serverless 事实源/实时总线。
...
2. 单实例运行注册表以 `projectId/taskId` 为键，同一任务最多一个活动 run；重复启动返回既有 run/终态或明确冲突。任务 5.5 已按获批计划锁定以下应用接口与 checkpoint 接缝，不修改既有 TaskStateStore/MessageBus/Executor/SandboxManager 签名：
...
POST /api/tasks  // { projectId, taskId, requestId, goal } → 202 started / 200 idempotent terminal-or-active / 409 goal conflict
...
5. 最终 State、消息与产物引用必须支持页面刷新恢复。Phase 5 不承诺活动 run 在后端进程重启后自动续跑；若进程丢失，读模型必须显示中断/非活动状态，不得保留虚假的 running/active 展示。
...
```
function project(state: AppState, projectChannels: readonly Channel[], role: string): ProjectionView {
  ...
  const myChannels = projectChannels.filter(c => c.participants.includes(role));
