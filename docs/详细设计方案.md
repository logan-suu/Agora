...
**[2026-09-13 架构决策更新 · D18 小阶段发布]** 四批主题按Phase11–13、14–20、21–24、25–30分阶段验收。每阶段最后一项集成测试验证该阶段可发布增量，完整契约与安装产物证据见开发计划§18.11。Phase11只开放安装/环境检查；Phase12受控本机执行只在获批验收范围运行，普通项目入口等待Phase13保护、证据、退出恢复与Docker退役完整验收。11.2先定稿安装升级、配置/新桌面状态兼容策略；后续每阶段验证上一已发布版本升级及设置保留。旧Docker任务不兼容不扩大为新桌面版本任意丢弃状态。未交付部分不可用必须明确可见，不伪装通过。
