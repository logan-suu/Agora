...
**[2026-09-14 选型同步]** 蓝图§21的Leader研发测试偏好采用OpenCode Go优先路由：通用G5实测由`tests/helpers/live-model.ts`选择已有Go凭据与`deepseek-v4.1-flash`，通过官方DeepSeekAdapter接入Harness，保留原生会话头、思考与默认容量；无Go配置时可使用既有官方配置，显式`AGORA_TEST_PROVIDER=deepseek-official`覆盖官方专属验证。已选Go失败不降级，不变更生产连接及既有任务绑定。兼容PiAi路径将Go的新V4.1名称纳入既有原生reasoning回放规则；仅补模型识别，用户配置的容量不变。历史评测与费用证据不重写。

**[2026-09-14 Leader测试选择同步]** 当前通用真实测试由`tests/helpers/live-model.ts`固定Go `deepseek-v4-flash`，取代上一段Go优先/V4.1默认/官方后备策略。`AGORA_TEST_PROVIDER`仅允许缺省或`opencode-go`；官方覆盖暂不启用，凭据仍保留。缺Go或调用失败直接失败，三项默认G5不跳过；继续使用原生DeepSeekAdapter、相同断言/思考/容量及期限，真实工具与LocalTempSandbox不变。产品V4.1协议识别及当前Eval的更名修复保留，但不启动V4.1实网调用或正式Benchmark。 测试侧Go adapter复用已验证的`normalizeGoToolStream`，在公开stream/prepareCall两入口保留同一block已知工具身份、先读完流再发布工具块；不改arguments、不猜缺失身份或吞冲突，参见`docs/evals/phase10-go-diagnostic-evidence.md`。
...
**[2026-09-14 研发回归实现同步]** 根目录`vitest.config.ts`采用`fileParallelism: false`，默认`pnpm test`按文件串行调度共享宿主的真实Docker/Harness/Git回归，避免独立测试文件竞争同一宿主资源。场景内部的worker并发、全部测试匹配规则、原断言和期限保持不变；通用真实模型仍按当前Leader选择执行，不以跳过或提供方后备替代。诊断和验证见`docs/task-history/11.4.md`；历史单次超时的精确阻塞位置未复现，不归因为已证实的Git合并逻辑缺陷。
...
**[2026-09-16 研发留证同步]** 按蓝图§21 TEST-CLEANUP，测试原始输出写Git忽略的`test-outputs/`，`docs/reviews/`只保留交付筛选的最小充分证据；每次创建/更新PR清理临时、重复、过时产物并同步活动引用与固定提交追溯。规则见[维护规范](task-tracking.md)。

### 1. 共享状态 State 精确 Schema（TS interface）

**[2026-09-17 D19阶段接口同步]** 现有实现及冻结方法继续有效；Phase14新增受信orchestration聚合、任务共享提交队列、单派工companion及图等待/恢复适配，精确契约见[§13](#langgraph-contract-d19)。只在新路径启用，模型无权写控制收据，图不替代投影/授权/D4/D9/D16/lease。
...
4. **现有公开接口保留，新语义明确走新版本能力。** Executor 和 TaskStateStore 的调用形态可复用；单项目 main、role 身份的 Channel/roster 契约不能假装兼容完整多会话/同岗位多成员，需新增版本化协作端口。SandboxManager 本机语义已在§12.2定稿；11.1本身不批准宿主执行，12.2设计接受也不替代实现验收。
...
本表描述完整目标模型。Phase11–13的新桌面版本仍允许明确标记的task-only执行模型，只保存projectId/taskId/workerId及必要workspace/Harness绑定，不虚构conversationId或memberId；它不宣称已支持产品会话/稳定成员。Phase15启用会话模型后，新工作项必须有唯一会话归属；已有新桌面task-only记录按15.1和11.2升级协议显式处理，未转换记录不能混入会话队列。Phase16之后新成员执行必须引用真实memberId，旧roleSlot历史不事后猜作者。
...
| 会话交流记录（15.1 定稿端口） | 无工作项的咨询/材料讨论及其稳定来源；指向任务规范消息的引用 | 不另存任务审批/需求的权威副本，不将聊天自动变成任务 | 同一用户控制入口；工作变更转任务控制面；展示流按规范来源合成 |
...
| GlobalScheduler / 运行注册表 | 当前进程活 lease capability / composition admission | 持久责任认领不等于持有执行额度；历史 lease 信息不等于恢复令牌 | 全实例 scheduler 和生命周期组合根；后续17.1扩充额度算法 |
...
`roleSlot` 只标识 Phase 11–15 仍按既有岗位执行的能力，不构造伪 memberId 或工作责任。Phase16 实装稳定成员后，新的成员执行必须为 `member`；能力版本不允许的 actor 直接拒绝。

辅助模型调用与WorkerRuntime执行分开识别：当前需求解释有自己的scope/sourceMsgId、官方session及scheduler逻辑key，未登记WorkerState；不能为统一报表伪造一位成员或一条worker生命周期。后续辅助调用记录至少关联projectId、目标taskId（无工作项的咨询由15.1提供conversation来源）、受信purpose、sourceRef及官方session引用；此类记录不属于ExecutionBinding。它们仍取得真实调用额度/预算，不能持成员写权限或编码workspace；把成员实际工作改标为辅助用途属非法身份。新增辅助用途及无task的额度入口由相应设计任务定稿，不在本节启用。

`scope.subtaskIds=[]` 只允许经控制面明确批准的整工作项职责（例如最终审阅），不是任意任务/文件访问权。非空时每一 Subtask 必须属于该 task 并通过派发条件校验。response 责任只供17.1/18.1定稿后启用；它不替换主要责任，也不允许同一成员同时执行两件工作。
...
4. 只有已关联的任务可竞争本会话主工作位置；一次 CAS 选择一个非终态候选。队列次序由15.1定稿；本节不额外批准自动开始独立需求。每次启动重新检查位置和任务事实，不能依据 stale UI。
...
工作项内部测试/审阅不改变 conversationId/taskId。D16 Leader approve 与最终规范产物闭合后，项目控制服务才释放主工作位置；在任务终态提交后崩溃，可以根据同一终态/归档回执幂等释放。gate 等待或工具暂时空闲不等于主工作完成，不允许后排独立需求抢占它。取消流程需先安全点收敛，15.1定稿具体状态及用户操作；引用保留策略由19.1/19.4细化。阶段11不实现这些产品控制操作。

###### 12.1.5.2 成员责任、派发与离职

16.1/16.2 必须在项目协作原子域认领责任，并在同一 CAS 中检验成员 enabled、目标有效、依赖、当前主要责任及未闭合控制操作。已接受的主要责任每成员最多一份；未接受提议/队列候选不算认领。单次派发前，任务控制面登记不可变 ExecutionBinding 和规范 WorkerState；启动再复核 assignment 及其版本，而非缓存 RoleSpec。
...
暂停/等待答复可释放 lease，主要责任保留；成员恢复不能因有责任记录就直接执行。离职按 D12/D13 的收敛和交接原则演进，目标成员变更产生新的 assignment 与 predecessor 引用；原成员/原 worker/原产物的作者归属保留。没有接任者时明确 awaiting_replacement，不抹去责任。具体状态字面量和请求 API 在16.1固定，但上述原子域与历史规则由11.1固定。
...
只有模型 Step/工具自然结束、commit+flush/checkpoint 闭合后才可以释放/切换。成员认领不等于 lease，lease 不等于 composition admission。恢复从安全事实重新排队；保存的 leaseId 只可审计，绝不重新装入 scheduler 当成 capability。身份恢复不改变迭代计数、provider retry 预算或D16终审资格。异常/副作用不明时只开放审查、清理或用户授权的恢复路径；正常退出由13.2定稿，多成员异常恢复由20.2定稿。

###### 12.1.5.4 Channel、消息与权限

新 Channel 主键为不透明 channelId，显式带 conversationId，`kind='main'|'sub'`；每个产品会话恰一个 main。参与者在稳定成员阶段使用 leader/memberId，而不是岗位名称。旧 `threadId` 继续表示问答链/具体关联，不冒充 conversationId。Phase15的角色执行使用明确的 roleSlot 参与者版本，Phase16升级为成员参与者；映射和新桌面状态升级需真实验证，不猜历史发言属于哪一位同岗位成员。
...
工作变更仍统一经过用户消息控制入口，先验证 conversationId/channelId/taskId 归属、目标事实版本和授权，再走 D9 安全点与 commit。普通咨询不自动变成需求。私聊确认的完整多源控制协议由18.1/18.5实现；11.1不启用旁路。MessageBus 仅投递已持久化来源，SSE/DTO不透传 payload；原始会话日志不进入 Harness 历史，投影只取结构化、经引用核验的当前事实。
...
证据引用最少包含 projectId/taskId、生产者 workerId（或受信服务用途）、workspaceId、精确版本描述、控制事实指纹及验证/审阅回执身份。成员/assignment、conversation 从规范绑定解析；保存的历史引用可帮助审计，但不能覆盖其当前所有者。Git 版本是实测 HEAD；普通目录版本按§12.2.4证明实测文件集合/内容和外部修改失效（13.1落地展示与验收证据），不能把时间戳或“测试绿了”当版本。D16完成、累计验证与交付引用同一有效产物；新改动使旧证明失效，回到必要验证。
...
| --- | --- | --- |
| Executor、SafePointExecutor | 四个方法和安全点 companion 保留；受信工厂注入完整执行绑定，opaque checkpoint保持可核验 | 13.2 /16.1/20.2恢复接缝；不得把memberId挤进sessionId参数 |
| TaskScope / TaskStateStore | 保留 projectId/taskId 和方法签名；AppState/Mutation需增加创建/执行绑定字段，属于显式schema演进 | 本机必要绑定12.2/12.3；会话15.1；成员16.1。全部新字段/mutation写权限、版本校验和索引须在实装前定稿；并非“签名没改所以无需审查” |
| ProjectCollaborationStore / ProjectChannelStore | 旧接口保留且只服务原有shape；新增 ProjectControlStoreV2 承载新版协作原子域。旧视图对无法无损表达的新版本明确拒绝，不聚合多个main或合并同岗位成员 | 15.1/15.3会话版本，16.1/16.2成员版本；逐版本adapter验证，旧Docker任务不建设兼容产品入口 |
| Channel / ParticipantId / RosterEntry / RoleSpec | 新版区分conversation/member/roleSlot及模板；这是类型与语义变更，不能声称现有Channel已支持 | 15.1和16.1；角色/成员鉴权、离职与投影同时改，不能只改UI |
...
| GlobalScheduler / SlotLease | workerId继续作执行额度身份；成员责任CAS在lease之前核验；不持久恢复capability | 16.2接成员占用；17.1/17.2两层上限。当前cap=3保留，不能凭本设计改成全局6 |
| MessageBus / MessageCommitted | 当前任务展示事件签名保留；将来新增会话咨询事件应独立带来源类型和scope，不伪造TaskState消息 | 15.1定稿伴随事件/快照尾流协议；18.1跨讨论控制；仍先commit再publish |
...
| 14.1–14.7 | D19持久图编排；task/worker/Harness身份不变，控制收据、共享提交队列、SQLite及显式Continue按§13实现验收 | 不提前启用conversation/member或新配额；14.7前不切换新任务默认引擎 |
| 15.1–15.5 | 实装conversation、WorkItemRegistration、每会话main、主工作队列及咨询来源；项目控制聚合首个所需版本 | 仍按当前岗位执行，不宣称member占用；不得隐式放宽当前全局3额度 |
| 16.1–16.6 | member/assignment/执行绑定、原子认领和离职交接；明确从新桌面roleSlot到member模型的升级 | 不猜旧消息作者；新版本角色槽记录保留其原身份，新增具名执行用新的映射 |
| 17.1–21.6 | 双层lease/短回应、协作请求消费、附件来源、异常恢复、波次/返工；原有工作/成员身份连续 | 不以短回应复制成员，不启用preparation；真实副作用恢复须专门验证 |
| 22–25 | KB/技能/MCP绑定项目、成员和配置版本；工作证据引用稳定，复制成员不复制活责任 | 按D3及对应阶段门禁启用知识写入；跨项目复用必须显式 |
| 26–31 | 工作地图、来源追溯、实验/撤回/增量集成/办公室/插件引用同一套身份 | 展示节点不新增调度实体；实验必须有独立已批准工作范围和资源归属 |

新桌面旧版升级是需要保留的数据契约，与“不兼容旧Docker产品任务”严格分开。11.2定义持久格式/升级失败协议；15.1、16.1提供其版本转换、角色槽保留和活动工作处理。无法无损转换的活动工作必须先按正式退出/终态流程收敛或明确阻止升级，不能丢弃或自动重跑。安装升级策略按§12.3.6，具体schema转换由相关任务确认，11.1固定不得破坏已有新桌面身份/证据的约束。
...
| ID | 场景与预期 | 主要后续任务 |
| --- | --- | --- |
| I01 | 两项目使用相同局部ID，跨项目引用/读写/恢复被拒绝；路径别名不能绕过同根写入保护 | 12.3、12.5、15.2 |
| I02 | 无工作项会话可咨询；连续完成两项工作，conversation保持、task不同、历史不混；不创建无意义编码worker | 15.3、15.4 |
| I03 | 并发争抢同会话主工作仅一个成功；gate等待不释放主工作，终态后补写项目回执不重复推进 | 15.3、15.5 |
| I04 | 同岗位两成员可分别认领；同一成员同时跨会话认领只一份主责任，邀请/复制不能增加其执行实例 | 16.2、16.3 |
| I05 | 在每个项目CAS/TaskState提交间中断并重启，原action只创建一个task/assignment/worker；坏输入和孤儿记录不启动执行 | 15.3、16.2、20.3 |
| I06 | D4暂停时State/flush/receipt任一失败不开放恢复；成功恢复同worker/新官方child/新lease；错成员或跨task引用拒绝 | 13.2、16.2、20.3 |
| I07 | done/failed worker不得重开；合法返工新dispatch保留旧责任/作者，迭代和provider预算不重置 | 13.4、21.4 |
| I08 | 满载时等待成员释放额度，短回应串行切换；保留主责任，回答后核验原工作，不借用跨task lease | 17.2、18.3 |
| I09 | 离职和下一Step同时竞争，只有已准入Step可自然闭合；缺接任者等待，交接后旧成员无法继续普通写入 | 16.5、16.6 |
| I10 | 多会话各一main；同岗位成员鉴权独立；旧视图不能无损表达时明确错误；重复消息/重连不重复控制动作 | 15.3、16.2、18.5 |
| I11 | 普通目录无Git也有真实文件版本证据；外部修改令旧测试失效；Tester独立workspace，最终产物和D16批准精确绑定 | 12.4、12.5、13.1 |
| I12 | schema升级成功保留新桌面配置/身份/历史；失败保留可恢复原状态；旧Docker数据不导入也不自动删除 | 11.2、各阶段出口、13.3、15.5、16.6 |
| I13 | 需求解释等辅助模型调用计入实际额度；禁止辅助用途冒充member写代码/修改责任，也不伪造WorkerState，投影与Trace无原始聊天/工具正文泄漏 | 12.6、17.5、18.4 |
| I14 | 并发提交和恢复后workspace/验证/审阅/归档引用仍指同一累计产物；路径和相似显示名不能选错产物 | 12.4、13.1、21.6 |
...
<a id="local-execution-contract-122"></a>
#### 12.2 本机授权与文件边界（12.2 → 12.3、12.4、12.5）

**[2026-09-15 架构决策更新] 12.2设计已获Leader接受。** 本节为正式契约，审阅时点原稿和源码证据见[12.2审阅记录](reviews/task122-local-execution-contract.md)，接受证据见[任务历史](task-history/12.2.md)。只定稿本机能力边界，不表示已实装、通过G5或开放普通项目入口；Phase12仍为限定验收预览，Phase13出口后才开放完整本机开发。

实际工具调用必须由受信端口校验，不能只靠提示词；L1 零 I/O、L2 只依赖端口不变。并行工作按独立编码工作分配本机 worktree；目录隔离不代表命令、端口、数据库或系统权限隔离。第一批保持task-only、roleSlot和当前全局cap=3，继续遵守D1/D4/D8/D9/D15/D16/D17及KB Write-Block。

##### 12.2.1 已接受的方案与限制

在现有 Harness/MCP 外侧增加受信本机能力层：文件通过带版本的受信写入事务进入实际项目；项目命令通过 macOS Seatbelt 执行；工作区和验证版本有独立类型；所有授权、撤回、接管和交付有持久回执。普通目录无需 Git，独立编码并行才使用真实 linked worktree。

Leader已接受以下取舍及§12.2.4.1的首版本机APFS范围；候选机制的运行时可行性仍须真实验证：

1. **采用系统 `/usr/bin/sandbox-exec` 作为第一版命令强制边界的候选。** 它不增加第三方运行时依赖，适合按每次命令生成目录/网络策略，但本机手册明确标为 DEPRECATED，SDK 的 `sandbox_init` 标为“No longer supported”。这意味着维护风险真实存在。方案要求逐系统构建/工具链实测与启动探测，机制不可用就关闭执行，绝不裸跑或回退 Docker。Apple App Sandbox + 独立 helper 是替代设计方向，需要另做动态目录授权、子进程继承、签名和现有独立服务组合验证；本设计不声称它不可行，也不把它当作已验证替代品。
...
3. **[2026-09-15 架构决策更新] 区分新访问准入与在途能力撤回。** 模型、项目脚本及其后代是不受信方，路径重定向/链接/替换不能取得从未授权的文件或凭据；受信registry/helper/OS策略是强制边界。外部编辑器可非合作修改或移动项目，不能承诺控制任意宿主同UID进程、管理员或让所有已打开文件句柄瞬时失效。根/父链漂移及撤回按§12.2.4.3/§12.2.6关闭准入、收敛在途、保全版本；不能借此放开项目进程越权或把已经发生的副作用假报为拒绝成功。规格复评依据及旧断言处理见[12.3规格复评](reviews/task123-spec-reasonableness.md)。
...
普通授权覆盖该项目内编辑、依赖安装、构建、测试；新增根、扩大网络、删除用户已有文件、破坏性 Git 操作、push/发布另行确认。为自身新增文件撤销，也必须核验当前内容仍属于该操作，不删除后来写入的用户内容。拒绝不能伪装成命令成功。保存授权不是永久活 capability；每次调用核对当前 revision、文件身份、writer epoch 和 task/worker 绑定。路径变化不自动撤销已发出的OS操作或已打开对象权限；它也不授权跟随新路径、接受替换根或通过旧目录fd打开新的未准入对象。活能力只服务原操作和原用途，不得由模型声明或跨操作复用。
...
**[2026-09-16 12.3控制语法细化]** 控制解析保持封闭联合，单条输入最多65536字符；takeover显式路径集合最多256项且不重复，逐项使用§12.2.3.2规范相对路径规则。拒绝重复JSON键（含转义后同名）、多命令尾随和额外字段，不能让JSON默认的末键覆盖改变scope。grant/apply/undo必须携带64位小写十六进制inputHash；解析成功只代表数据有效，Leader transport、当前proposal/selection与registry授权校验仍必需。12.5能力未接入前takeover/return/apply/undo继续由受信控制入口明确拒绝，不因语法已定义而执行。

**[2026-09-16 12.3授权提案持久化细化]** 控制服务在唯一desktop owner的私有`local-workspaces/objects`目录保存按规范JSON SHA-256寻址的不可变提案对象；对象仅为确认/恢复输入，当前授权仍以registry为准。创建排他、不覆盖同名文件；完整写入、fsync及只读权限闭合后才提供引用。读取验证根链、owner、普通单链接文件、大小、权限及内容hash；损坏或未完成对象拒绝，不按同ID重建。提案绑定project/task、selectionRef、expectedRevision、完整根检查及工具链/网络/输出路径配置，policyProposalId与inputHash同源；UI不得从路径字符串直接制造授权。受信选择器与配置提供者生成提案，POST只提交其引用和hash。确认前重检根及当前registry revision，根/配置改变须重新提案；同动作恢复使用首次规范消息和prepared输入，不能用后来配置扩张授权。该私有对象库不供模型任意读写，也不开放文件系统路径参数。

**[2026-09-16 12.3下载代理首版细化]** 下载采用受信Node HTTPS服务，项目子进程保持禁网，不开放CONNECT、Unix socket、Cookie/认证或任意请求头/请求体。首版仅支持明确获批的HTTPS origin、GET和443端口；每跳重定向重新核验同一不可扩张策略、全部DNS结果和当前授权，连接固定到本次已验公网地址并保持原hostname的TLS证书/SNI验证，不使用环境代理或共享连接池。私网、loopback、link-local、元数据、保留/文档/多播、IPv4映射与转换地址拒绝；IPv6仅接受未列入特殊用途的原生全局单播。单次最多3跳、30秒、16MiB，压缩内容不自动解压，完整接收后先核对预期SHA-512完整性再存私有字节对象。取得包名或公网DNS不替代安装授权/锁定输入；当前先实现内部受信原语，未接grant/安装闭环前不注册为模型工具。

**[2026-09-16 12.3下载授权接合]** 授权提案可明确包含install动作及上述封闭brokered-https策略，其完整内容按policyHash存私有对象、networkHash绑定registry。内部安装输入服务只接受当前活worker/claim/lease的install调用，逐边界重验同一grant/根/绑定和完整输入版本；下载之前先持久prepared，完成后保存校验过的私有字节与不可变receipt。GET中断可在相同输入和当前授权复核后重试，不执行包代码或写用户源码；同action异输入冲突，首次完整回执不可替换。重放核对字节hash/完整性/策略/版本，授权或源版本变化拒绝返回。该服务不注册任意模型下载工具，由封闭pnpm-install适配调用；子进程仍禁网，安装和依赖挂接的独立实测见12.3任务历史。

**[2026-09-16 PR #86修复细化]** 内部保留的ASCII名称（`.git`、`.agora-operations`、`.env`及`.env.*`前缀）按ASCII大小写折叠统一排除，大小写敏感卷也采用同一保守规则。TypeScript文件准入、原生helper逐段解析/目录枚举及manifest复核必须一致；被旧实现当作普通输入记录的保留名称不自动迁移或放行。此规则不放宽原有秘密排除范围，首版仍不开放example/template例外；不声称已完成任意Unicode名称等价性的穷举验证。
...
传入 shell 字符串只作为受隔离的项目程序，不以字符串过滤器承诺识别所有危险行为。新生产入口首选 `toolId + argv + cwdRef`，控制面操作使用独立方法；不拼接用户字符串到宿主 shell。包生命周期脚本属于不受信项目代码，与根脚本同边界。候选代理和 supervisor 优先使用受管 Node 内置能力及现有 native helper 工程；任何新增第三方依赖仍须按选型 §12 先讨论，不能因本设计提到代理就自动安装外部服务。

**[2026-09-16 12.3受管工具路径细化]** 受信命令策略除已登记目录祖先外，还允许已批准可执行文件父链的精确目录metadata查询，以满足包管理器对Node路径的stat/定位。此权限不开放父目录枚举、文件内容读取、写入或其他可执行文件；源码/输出/私有域及禁网规则不变。

**[2026-09-16 12.3安装/生成适配细化]** 首个受管Node适配支持`node-generate`：首参数为`@input/<relative script>`，从固定输入建立新执行副本，所有生成只写私有输出；同worker可通过`workspace_read(path, commandReceiptId)`读回成功且已收敛的候选字节，源码应用仍逐文件走原版本/readReceipt和完整读集事务，不自动复制输出覆盖目录。`pnpm-install`的argv为一个封闭JSON数组（最多32包，各含name/version/url/integrity），要求完整覆盖根package.json的精确dependencies/devDependencies，逐包使用已授权HTTPS代理取得固定SHA-512字节。受管pnpm及其清单全量核验，在禁网副本通过offline、copy、hoisted、显式包含devDependencies和私有store/cache运行；生命周期仍是受隔离项目程序。首个适配拒绝已有lock/config、workspace/override/optional/peer解析及源node_modules，不能把无法解析的依赖当已安装，未缓存传递依赖按真实离线失败处理，不放开网络或静默省略。安装生成的lockfile和源码差异只作为候选，不写原目录；实际node_modules经原生安全读取和完整清单核验后冻结为独立依赖快照，链接/特殊对象拒绝，绝不跟随到未授权根。当前安装选择绑定完整源版本、task/root/策略和工具清单；源码改变须重装。后续Node命令将原始固定源码与该固定依赖树物化为新的只读执行输入，可信验证hash包含实际安装/锁和挂接证据，不能记为空依赖。适配范围不宣称任意生态安装可用；普通入口及后续适配仍按阶段门禁。

###### 12.2.2.3 机制选择与失败关闭

| 路线 | 判断 |
| --- | --- |
| Seatbelt / sandbox-exec | 已接受进入 12.3 的候选实装和 G5。OS 自带、可按命令制定策略；弃用风险已获 Leader 接受。必须验证子孙进程继承、IPC、网络与路径别名，不能凭上游使用过就认定 Agora 安全 |
...
**[2026-09-16 12.3启动探测接合]** 本机task factory在模型配置/角色工具准入之前调用受信会话服务ensureReady；每个新服务实例在owner私有目录运行固定Node探针，实测允许输入读/输出写、拒绝输入写/模拟凭据读/loopback连接、白名单环境及真实子进程继承。使用同一held launcher/出生身份journal/bounded收敛，回执绑定OS release/架构、工具清单hash、策略hash、实际输出和journal对象；缺工具、坏hash、任何允许/拒绝断言或收尾失败统一关闭本机执行。探针成功不替代Leader grant、worker lease或任务版本资格，不发放项目能力。失败实例不静默重试或降级，后续重建服务须重新探测。

##### 12.2.3 身份、存储和接口

###### 12.2.3.1 持久记录的唯一所有权

- 现有项目注册继续保存项目名称与根入口；runtime/sandbox 的受信 registry 只保存 root 真实身份、授权、workspace 技术绑定、writer 占用和文件事务回执，不复制 roster/Channel/需求或任务进度。Phase15 ProjectControlStoreV2 接管项目控制元数据时保留这些 capability 引用，禁止双写两份授权。
...
**[2026-09-16 12.3存储底座实装]** 内部LocalRegistryFile在上述路径持久化`{snapshot, sha256}`包络；snapshot遵守local-workspaces-v1外形，完整记录codec为必填受信依赖，外层拒绝额外字段/非法revision，每数组最多4096项、文件与读取最多16MiB。复用桌面owner，文件锁及fsync/rename实现CAS发布，残留锁/pending拒绝，不能自动重置已有缺失/坏记录。文件发布闭合不等于业务operations与TaskState闭合；生产记录codec、授权与转换检查尚未落地，不能据此授予执行。本轮22项真实文件/owner故障测试及完整回归7脚本+199文件/1433测试全绿，0skip，完整结果见[registry文件底座](https://github.com/logan-suu/Agora/blob/05987ccbd20e6a55a553750577a900f44bc353d0/docs/reviews/task123-local-registry-file-unit.md)。
...
**[2026-09-16 12.3类型校验细化]** L1使用上述封闭判别联合，拒绝未知字段、缺字段和不同版本/分支字段混搭，不做字符串裁剪、路径归一化或旧结构迁移。ID沿用内部命令绑定的`[A-Za-z0-9][A-Za-z0-9._:-]{0,127}`；hash为64位小写十六进制，Git对象沿用40/64位小写十六进制及既有受管分支命名规则（分支上限128字符）。文件identity/parentIdentity是有界不透明引用，可承载现有dev:inode表示，实际身份仍由受信文件回执核验。规范POSIX相对路径上限4096 UTF-8字节、每段255字节，拒绝空段、`.`/`..`、控制字符及无效Unicode，不改写合法空格/中文文件名；路径shape通过不代表通过秘密排除、根身份、链接或OS文件系统校验。absent.name必须是目标path末段，remove不得用absent作删除依据；size/revision/epoch须为非负安全整数。workspace引用集合最多4096项、workspaceId全局唯一；CAS允许追加新ID及调整集合顺序，已有ID的全部字段不可替换或删除，关闭/撤权只改技术生命周期，不复用旧ID。文件adapter在读取与发布前强制执行这些workspace规则，完整roots/grants/claims/operations codec仍为必填且尚待接入；纯数据校验不授予执行能力。实现、红绿测试与边界见[工作区身份单元](https://github.com/logan-suu/Agora/blob/05987ccbd20e6a55a553750577a900f44bc353d0/docs/reviews/task123-workspace-identity-unit.md)。

**[2026-09-16 12.3串行状态接合细化]** `localExecution` 的封闭结构为 `{schemaVersion:'local-execution-v1', rootIds, workspaces, bindings, receipts}`：rootIds 为本任务不可变根ID集合；workspaces 为上述不可变引用集合且全部属于本任务；bindings 为 `{workerId, subtaskId?, workspaceId, receiptId}`，同一worker只绑定一次：编码绑定仍必须有真实subtask；未分配subtask的非编码worker可绑定只读验证快照，此时省略subtaskId并与WorkerState完全一致，不创建伪Subtask。存在subtaskId时该任务必须存在且匹配WorkerState。receiptId必须指向本结构的规范回执；receipts 为 `{receiptId, actionId, inputHash, registryRevision}`，ID/action各自唯一，hash为64位小写十六进制。各集合上限4096，空集合合法；首次TaskState绑定不等于可执行授权。根/workspace/绑定/回执只可追加，不移除或重解释已有身份；新worker接手产生新绑定，技术撤权/隔离由registry持久记录决定，旧事实保留。registryRevision记录该动作prepared所在revision，最终committed仍须到registry按actionId/inputHash复核，不能以数字更大推断闭合。

**[2026-09-16 12.3非编码接合复评]** 既有Coordinator会按任务分配PM/Architect/Tester/Reviewer而不必附CODER的subtaskId，先前把local bindings的subtaskId一律必填会强迫伪造责任或重开done编码任务。现明确可选条件如上；已有binding的字段存在性也不可变化，不允许事后给无subtask绑定补身份。PM等无文件工具角色不因此取得workspace；只读工具仍须独立snapshot capability和活lease，不能冒用CODER的write claim或放宽来源文件写权限。这一schema适配不表示非编码执行链已验收。

**[2026-09-16 12.3控制角色生命周期细化]** PM/COORDINATOR通过独立无文件工具companion准入，仍复核规范WorkerState、活lease及当前Leader授权并在安全点持久边界回执；不创建workspace/binding/claim、不投文件快照、不回落旧Docker/LocalTemp执行器。该会话只含关闭/检查点能力，构建Harness时工具集固定为空；完整任务组合仍须接真实Tester验证、审阅和D16。

`applyMutations`只允许串行控制面set完整localExecution，并在整批结束时交叉校验task/project/root/workspace/worker/subtask/receipt；持久加载同验。worker模型在串行或并行模式均无此set权限。已带localExecution的状态不得移除此字段或与旧Subtask.worktree、Integration、parallelExecution及旧Git验证绑定混搭；支持本机linked-worktree的后续适配须按12.4显式接入。`TestResults.workspaceVersion`为可选的上述版本联合；本机结果必须携带属于本任务已登记模式的版本引用，旧路径不得借此伪装本机证据。该字段只是受信验证结果的版本引用，实际输入/receipt与D16复核仍由执行控制面负责，不能由类型校验替代。

**[2026-09-16 12.3授权记录细化]** registry生产codec采用封闭JSON记录，不转换旧数据：root记录含rootId/projectId、selectionRef、规范path、volumeId、dev/inode字符串、由`/`至根的父链`{path,identity}`、staging（初始化前显式null、闭合后path/identity）及inspectionHash；根身份登记后不可替换。grant记录含grantId/projectId/rootId/revision/policyVersion、actions（read/edit/run/install/generate/remove）、toolchainHash/networkHash/policyHash、createdActionId/leaderMessageId，status为active/revoking/revoked；revocationActionId初始null，撤回后必填且不恢复active，扩大权限使用新grantId。claim记录含claimId/projectId/taskId/workspaceId/workerId、writerEpoch、createdActionId、status（active/draining/quarantined/released）及closureReceiptId（初始null，released时必填）；没有TTL或无证据强制释放。

跨存储binding操作记录为`{actionId, inputHash, receiptId, projectId, taskId, preparedRevision, stage, previousLocalHash, nextLocalExecution, sourceMessageId}`，stage只允许prepared→committed，身份与输入不改写；sourceMessageId指向同task的规范Leader主消息，nextLocalExecution中的对应receipt必须精确匹配操作。初始previousLocalHash是不存在localExecution的规范hash，后续是旧localExecution的规范hash；字符串对象键排序后hash，数组顺序有意义。执行服务从registry记录解出候选TaskState，经同一TaskStateStore的commit/applyMutations提交，重读并核验后才committed。prepared期间不执行其他能力操作；不确定中断保留prepared，恢复先校验source/作用域/输入及当前canonical引用，成功提交不重复；若当前canonical引用已是该结果则只补记committed，异输入或无关localExecution变化拒绝。该内部闭合服务接收可信控制面输入，不提供模型/HTTP直接反序列化调用入口；Leader消息存在不单独代表任意scope获批，具体grant动作仍须受控入口按§12.2.2.1验证selection/proposal/hash。

**[2026-09-16 12.3规范消息原子提交细化]** 正式workspace控制入口不得先单独保存applied消息再提交授权引用。跨存储binding的prepared记录可增加不可变`sourceMessage`，保存经受信控制面核验的完整Leader主消息（msgId等于actionId/sourceMessageId，display解析结果与payload.intent完全一致，action严格为applied，scope及expectedRevision匹配）。首次提交通过同一TaskStateStore mutation批次append该消息并set localExecution；MessageService仅在持久提交后投递。恢复先复核prepared消息、当前同ID事实及旧localExecution指纹；同ID异内容、消息单独存在或引用单独存在均拒绝，不能补拼成成功。旧内部生命周期调用可继续引用已存在的规范Leader消息，但不能用该兼容路径替代正式grant/revoke入口。prepared消息是不可变续办输入，不构成第二份消息真相源；committed后逐次与TaskState完整消息交叉核验。语法有效仍不替代selection/proposal/hash及安全点的当前授权检查。

###### 12.2.3.3 新端口与冻结接口

新端口位于既有 `runtime/sandbox`，领域类型放 `core/domain`，编排只依赖 L3，MCP 适配位于既有 `tools/*`；不新增顶层包。

**[2026-09-16 12.3只读快照准入细化]** Architect/Tester/Reviewer的文件能力使用独立`purpose=validation`的固定输入workspace引用，不创建/接管源码写claim；引用baselineManifestId精确指向本次规范根/策略捕获的不可变manifest。无subtask的任务级分配如上保持省略。该非写调用保留WorkspaceCall形状，writerEpoch固定0作为“无写权”标记，逐次仍须原活lease、canonical worker、已闭合binding回执、当前grant/revision与根身份；数字0不授予能力。只允许read；TESTER另在当前grant允许时可run固定基线，其他角色拒绝run，全部拒绝edit/install/generate/remove。read/inspect从固定manifest及私有字节对象返回历史输入，不因用户修改而悄悄换成新文件。命令、审阅/Leader批准及应用的当前版本资格另外复核；读取旧快照不代表它仍是当前用户目录。该细化不复用CODER claim，不承诺已完成各角色产品验收。

**[2026-09-16 12.3活lease准入细化]** GlobalScheduler新增只读`assertActive(lease): void`，必须是当前实例尚未释放的原SlotLease对象且worker分区仍匹配；克隆、旧lease和其他实例对象拒绝。该检查不新建/释放/转移额度。工作区companion保留实际worker取得的lease并在工具准入复核，模型传来的workerId/leaseId字符串不能替代capability；D4重建仍重新acquire。写域claim、grant revision、writer epoch及canonical worker/workspace绑定另行交叉验证，任一条件失效拒绝。
...
| `quiesce(scope, reason): Promise<QuiescenceReceipt>` | 收拢 admission、等待工具/写事务、按§12.2.6完成有界清理及范围核验，显式标注bounded；残留资源隔离，不声称全部后代退出，不取消Harness token流 |
...
这些是已接受设计中的新增能力；12.3已接文件/命令/会话/固定版本端口，后续接管和delivery能力仍按责任任务推进。`CommandReceipt` 包含 RunResult 原四字段、commandId/inputVersion/policyHash/工具版本、开始结束时间、输出引用与收敛状态；原始命令输出按 D15 不直接进入通用 Trace，秘密过滤不替代访问隔离。
...
**[2026-09-16 12.3文件调用回执细化]** read回执保存原始字节对象引用、同次打开的版本/元数据、调用scope/revision/epoch及canonicalSourceRef；对工具返回不透明readReceiptId。action引用按task和动作命名空间寻址、排他写入并校验所指不可变对象，重试不能覆盖首次输入/内容/时间。写入先持久完整prepared及基线引用，再持久native-start标记后调用真实事务；缺终态的旧start只能复核现有native结果，不重新发起未知副作用。多个文件调用在单实例owner下串行提交，未闭合或存在不确定副作用的操作阻止该工作区新准入；无副作用且quiescent的普通版本冲突可重读后以新动作重试。结果保存实际expected/observed/result、基线/候选对象及native回执引用，不把文件操作失败转换为成功，不覆盖已有终态事实。此索引是技术回执，不复制TaskState或另建授权真相源。
...
**[2026-09-16 12.3发现退出竞争细化]** 监督器区分正常出生身份退出与未知发现失败；仅发现为exited并在原deadline内复核同出生身份已退出时记录closedDuringDiscovery，不清除unknown/mismatch/helper错误。held launcher可提交经journal固定两出生身份及唯一bootstrap→target关系验证的discoveryRoot；持续子树枚举从真实项目target开始，已审计只spawn一次且不再fork的受信bootstrap仍保留在身份与停止核验中，避免其回收唯一子进程的窗口被误当项目后代丢失。此例外不适用于项目根/后代，不延长查询或清理期限。

**[2026-09-16 12.3命令端口接合细化]** 内部`LocalWorkspaceCommands`当前仅接Node、禁网和`private-per-operation`输出：工具描述来自受信组合根并对照获批manifest指纹、实际二进制hash及绑定元数据，不接受模型提供可执行绝对路径。`@input/<relative>`与`@output/<relative>`参数分别解析到本动作固定输入和全新输出目录；拒绝越界分段。OS策略只开放固定输入及受控依赖读域，活用户目录仅作绑定身份/版本核验，不向项目命令开放。逐次命令使用独立commandId与不复用输出，native journal持久事实仍保留；上层回执只有输入/授权有效、实际退出/输出和bounded收尾均闭合时才quiescent。stdout/stderr按1MiB上限存字节对象，Trace仍不含原文。needsAttention保存有界reason并阻止该workspace的新文件/命令准入；同动作只交叉验证首次规范输入、native journal和固定输入回执，不再启动。12.3已接专用MCP/WorkerRuntime、本机组合及上述封闭安装/生成适配；普通项目入口保持阶段门禁，不能把当前Node适配扩大为任意生态工具可用。实测状态与完整G1–G7以任务历史为准。

**[2026-09-16 PR #86命令拒绝修复]** 普通Node的`@input/`与`@output/`相对参数在读取/挂接依赖、创建固定输入/输出及写入prepared之前完成纯校验，解析阶段复用同一函数。可预先判定的无效输入直接拒绝，不新增控制引用、不占用动作/资源，也不能阻止同workspace后续合法读写或命令。已进入真实准备/执行阶段的异常仍按原journal与fail-closed规则保全，不把不确定副作用伪装成未执行。

**[2026-09-16 12.3 CODER实链接合]** 专用localCoderRole只把现有CODER的read/write/run授权映射为workspace.read/apply/run，不扩大已缩减的角色白名单、不把其他角色当CODER。既有冻结RoleSpec字段保持，新增本机操作说明，受信factory按session本身的sessionId创建独立catalog和官方HarnessExecutor，Runtime复核该sessionId与canonical分配一致。普通生产task factory仍未绑定；该入口先用于获批临时验收目录。固定LRU G5覆盖真实Go deepseek-v4-flash→Harness→WorkerRuntime活lease→MCP版本读写→固定输入Node测试→实际命令参数/退出/有界收敛→最终版本复核，保留原五项测试和模拟秘密原文不变。单CODER真实闭环不替代Tester/Reviewer/D16/归档、依赖安装/生成、最低OS或全量G4；详细证据在12.3任务历史。

**[2026-09-16 12.3 worker companion接合细化]** 新L3 `WorkspaceWorkerPort.open(admission)`由WorkerRuntime取得实际GlobalScheduler lease后调用，admission包含受信scope/role/subtask/session和只读活lease验证闭包，不序列化capability。local状态必须配置companion、专用executor factory和canonical持久化加载/提交；不进入旧resolveWorktree/buildExecutor。注册闭合后重载canonical绑定并按assignment投影唯一workspace与子任务。每个step/end复核当前授权及工具回执；完成先持久bounded收敛并关闭工具准入，再将worker标done和释放lease，失败不得成为完成。暂停先flush官方安全点再核验工具收敛；reproject保留原会话，D4继续等gate持久化后关闭能力并释放lease。close幂等，排空该workspace串行工具队列，失败保持不可复用能力；本机模型输出不得直接set testResults，可信固定版本测试回执另由控制面提交。内部LocalWorkspaceSessions已接CODER/direct、ARCHITECT/TESTER/REVIEWER独立只读快照及PM/COORDINATOR无工具生命周期；终态worker关闭保留task写claim，任务归档后以同一Leader批准来源及真实会话关闭证据执行可恢复释放；固定输入可信验证、批准前复核、本机组合、D4真Fork和归档均使用同一版本链。跨进程主动恢复仍归13.2，不表示本任务或阶段已完成验收。此接缝不修改冻结Executor/SandboxManager签名或官方Harness loop。

**[2026-09-16 12.3 MCP接合细化]** `runtime/sandbox/workspace-port.ts`保存L3数据契约，内部bound adapter由组合根固定call作用域；MCP位于既有tools/fs，Harness桥接位于tools/bridge，使用既有SDK内存transport和官方ToolRuntime。专用catalog只注册workspace三工具，按受信capabilities进一步缩小，不解析旧工具别名。actionId由规范sessionId+官方exec.callId稳定hash生成，模型不可提交action/worker/epoch/root等字段；MCP内部action参数仅供受信桥接。read无path时CODER捕获当前manifest，validation角色只返回其不可变baseline manifest；两者均返回文件版本清单，有path返回同次读取的版本/回执及UTF-8或显式base64字节。模型工具单次读取/写候选总量限1MiB，底层16MiB原语不变，超限拒绝而非截断。apply携带每个目标的expected/readReceiptId及额外只读依赖，至多64个唯一路径；命令必须显式引用文件inputVersion，默认30秒、禁网、独立输出。工具取消不丢弃在途受控副作用的Promise，服务先闭合实际事实再返回；初始已取消不启动。该接合由WorkerRuntime活lease与角色白名单驱动，受信组合根固定能力；完整验收见任务历史，普通项目执行仍未开放。

**[2026-09-16 12.3组合与暂停接缝修复]** 显式本机任务组合只接受已持久授权绑定的task；Web工厂在创建旧执行资源前按localExecution分流，缺本机工厂则拒绝。受信宿主注入同一owner/registry/工具链，普通目录选择与项目开始UI仍归12.6。PM/Coordinator只获得无工具Harness；其他角色按只读/写入能力分别装配MCP，TESTER闭合前由受信服务运行固定验证，REVIEWER绑定其版本，Leader批准与终态归档重验同源证据。D4恢复先校验canonical paused worker/receipt/安全点的task、role及cwd，再由官方Agent factory创建新context/lineage child；恢复工具端在取得新lease/新会话前不具备调用目标。空活动队列不伪造暂停worker或Fork。

真实组合回归发现：paused之后的close复用了只允许running的工具准入，错误写failed。关闭改用独立只读生命周期核验：仍验证原活lease、canonical binding、当前grant及全部文件/命令journal闭合，不准入新的工具。编排收到外部暂停后返回的canonical humanGate立即停止路由，不继续派发下一角色；Leader解决后的全新composition负责恢复。顺序CODER→TESTER路由在同次串行提交写入规范COORDINATOR/main/announce调度，固定版本回执绑定该消息，不用展示Ledger代替验证调度。旧失败证据保留，最终G5通过情况只查任务历史；上述接合不开放未获授权的普通项目。
...
**[2026-09-16 12.3部分批次重放修复]** MCP适配层准备read-set时，已存在的同动作不可变输入先精确核对完整scope/epoch/revision/条目和原始read回执，仅返回原引用；不把查询旧partial结果当作新文件操作。后续writer仍核对同一批次输入及原生子回执，不再交换文件。新动作、异输入、新的文件读取继续受恢复门禁约束；不开放继续写入或自动回滚。真实MCP复现与验证记录见12.3历史。

**[2026-09-16 12.3同任务顺序写者接续修复]** direct保留task写claim不意味着永久绑定首个worker。下一个同任务CODER注册时，只有旧worker为done/failed、无活会话，且私有边界记录精确绑定其当前session/workspace/epoch/来源回执并证明close+quiescent，才在同一registry/CAS注册事务将旧claim置released并创建更高epoch的新claim；无中间无主窗口，不允许两个active写者。跨task/root接续、仍活动/暂停、隔离claim、缺失/损坏关闭证据均拒绝。旧workspace/binding/claim事实保留，旧capability不复活；这不是12.5的人工接管/交还旁路。

##### 12.2.4 文件版本与受控写入

###### 12.2.4.1 根和文件身份

首次打开固定 volume/device、inode、规范路径及父链身份。不同路径、大小写别名、symlink、嵌套项目、同一 Git common-dir 必须映射到同一冲突域；不能证明独立则拒绝并发写。首版候选只承诺本机 APFS 普通目录；网络盘、同步文件提供器和不支持所需原子操作的卷先明确不可执行，不能退化为普通 path 检查。该范围已随§12.2.1方案获Leader确认。

文件 helper 继续从受信 fd 解析每层。新访问准入及后续副作用步骤发现根/父链身份漂移时拒绝；用户源文件写入拒绝 symlink 路径分量、多硬链接、设备/socket/FIFO、权限异常以及替换根。原已打开普通文件的在途效果按§12.2.4.3收敛，不声称路径移动会瞬时使fd失效；只读访问如需项目内 symlink，必须验证终点仍在准入根。依赖内部的 pnpm 链接单独按 task-owned store 绑定校验，不能为解决依赖链接而放开源码任意链接。

**[2026-09-16 12.3根检查细化]** 受信只读检查器逐层以不跟随链接的目录fd核验父链，要求本机APFS并读取实际卷UUID，不创建技术目录或授予权限；拒绝已知FileProvider/iCloud目录入口及dataless根，其他卷/提供器支持仍须专门实测，不靠APFS标签承诺全部同步场景。卷属性由XNU在mount root核验，因此仅该只读检查器允许`file-read-metadata (mount-relative-literal "/")`；不授予卷根内容读取/写入、不加入项目命令策略。初次叶目录白名单下的EPERM不能误记为APFS不支持，必须保存错误和验证最小例外。当前检查器仍依赖组合根提供已验证受管helper和可信选择引用，不对模型开放绝对路径。

**[2026-09-16 12.3根初始化细化]** 受信选择/只读检查不创建源码目录。确认授权后，控制服务先在受管状态目录持久记录绑定根检查结果、授权动作、helper hash及预定`.agora-operations`名称的prepared，再由独立受限helper从固定父链fd执行排他mkdir(0700)。同名既有项一律冲突，不因权限看似正确而接管。创建前、创建后、返回前重核身份和当前授权；创建后失效保留created事实与目录身份并要求恢复，不删除目录假装拒绝成功。无完整终态回执的重试只报告recoveryRequired，不重新mkdir或认领未知目录；同actionId异输入冲突。成功回执只提供注册所需staging身份，本身不产生worker执行能力。注册与授权闭合仍走registry/TaskState协议；未闭合不开放文件/命令工具。

**[2026-09-16 12.3授权与初始化顺序校准]** 根元信息注册不能以先创建技术目录为前提。`LocalRootRecord.staging`增加显式null，表示已检查的根尚未完成授权后的初始化；此时可持久登记项目授权和根引用，但不得注册workspace/claim或调用文件/命令工具。根ID、选择引用、规范路径/卷/父链及inspectionHash仍不可变；唯一允许的一次补全是staging从null到真实初始化回执证明的身份，之后不得清空、改名或替换。补全必须引用同根同授权的durable prepared/终态初始化记录；通用binding提交不得直接补全已有根或自行虚构已初始化根。控制服务先闭合授权来源，再持久准备初始化、调用受限helper并闭合其回执，最后才允许注册workspace。失败维持未就绪/恢复必需，不回退先裸mkdir再追认授权。此为技术生命周期细化，不改变根身份或扩大Leader授权范围。

根初始化技术操作采用`kind='root-initialization'`，包含actionId/inputHash/receiptId、projectId/taskId/rootId/grantId/grantRevision、sourceMessageId、preparedRevision、stage（prepared/committed）及nativeReceipt（初始null，观测后保存sha256/stage/created/stagingIdentity/quiescent）。引用当前已闭合且允许edit的项目授权；只读选择不创建技术目录。每根只允许一个初始化操作，prepared占用与跨存储binding相同的阻断域；nativeReceipt最多补记一次，不覆盖失败。只有真实applied、created=true、身份已知且quiescent的回执才允许同次CAS将root.staging从null补全并置committed。其他结果保留prepared及实际事实，后续操作恢复必需。数据codec只检查关系/转换，受信协调器必须复核真实回执文件hash与根、当前授权和规范Leader来源；记录本身不能证明OS调用已经发生。
...
**[2026-09-16 12.3原子创建细化]** absent读回执绑定同一次原生检查的既有父目录身份；创建默认普通0644文件，不自动创建未登记父目录。候选在同卷技术目录内写完、校验、flush并关闭可写fd，再用`RENAME_EXCL | RENAME_NOFOLLOW_ANY | RENAME_RESOLVE_BENEATH`安装。目标已存在（含链接）返回conflict且不覆盖；父链漂移/撤权关闭。创建使用独立`local-creation-primitive-v1`技术回执及`created`事实，不声称交换或备份了不存在的旧文件。创建后的漂移保留实际副作用并置recoveryRequired，不回滚用户内容；同action/input只复用完整回执，不重放写入。

**[2026-09-15 架构决策更新] 目录竞争按对象与操作时序验收。** 不将“原已准入普通文件随根移动后仍有在途效果”与“访问从未授权的独立外部对象”混为一类，也不承诺OS根据路径移动瞬时撤销fd。helper/命令必须在实际边界内运行，后置检查不能代替新对象越权防护；纯路径前后检查不能证明TOCTOU已消失。

- **准入与身份**：每次受信调用、下一有副作用步骤及返回成功/发布验证/交付之前重核授权、根/父链及写域。普通文件fd须由受信服务按同一次安全打开记录对象身份、用途和原操作；相同内容hash或inode数字不是可恢复活权限。旧目录fd不能扩大为新父目录、新路径或新加入文件的递归授权。项目源码不得以可写data fd就地改写。
- **在途与失效**：外部移动可发生在最后一次检查之后；已经发给OS的操作、原已准入技术临时文件/专用输出对象上的在途写可能完成，不能保证瞬时撤销。发现漂移后停止新准入及后续副作用步骤，关闭自身fd/映射并收敛工具后代；不得故意用旧句柄继续工作。涉及根变化的操作不能作为当前绑定的有效applied/可信验证/交付；已发生变化按partial/recoveryRequired及原因留证，已有不可变终态回执追加失效事实而非改写。
- **保全与收尾**：保留开始版本、候选、实际旧版、当前可核验事实及journal，不自动回滚覆盖用户当前内容。只有受信写能力关闭、按§12.2.6完成范围核验与有界清理并落盘回执，才能报告该范围的接管/撤回状态；不声称全部后代退出。已发现残留或检查不完整的资源保持needsAttention，不回收、不复用。诊断取得的移动后路径不能作为重新打开/递归清理授权，重新绑定须用户明确选择确认。
- **必须拒绝的越权**：从未授权的独立外部哨兵、凭据与服务仍必须不可读写；路径替换、链接或任务身份伪造不允许引入新对象能力。12.3/12.5须真实验证新调用拒绝、在途闭合、外部哨兵不变及用户版本保全，未满足则关闭相应能力。宿主同UID恶意进程/管理员控制Agora本身不在承诺内，但不豁免不受信项目代码的攻击。

同卷事务暂存区由根登记时明确建立：用户根内保留受保护的 `.agora-operations` 技术目录，已存在同名目录则拒绝占用并要求选择另一空闲保留名；项目代码和模型文件工具均无读写权限。其位置/身份写入 registry，内容不进入构建/交付 manifest。应用状态可以在另一 APFS 卷，原子交换所需临时对象始终在目标同卷；最终证据复制至受保护对象库后核验 hash。不会把普通 `/tmp` 文件与用户外置卷直接 rename。只有无活 fd/引用且回执闭合的事务对象才可清理，已有用户目录不移动或重建。事务对象若随用户根被外部移动，其旧fd是否仍可写不是单独的安全判定；必须执行上面的失效/收敛/保全协议，不把根内暂存区向项目进程开放。候选完成写入、flush及元数据准备后关闭自身可写句柄/映射再交换，不能在候选进入用户源目录后继续通过遗留写fd修改源码。保留应用状态与项目可位于不同APFS卷的设计，不新增“项目必须与应用状态同卷”的限制。
...
**[2026-09-16 12.3有界批次读集细化]** `basisReceiptId`可引用受信服务将已有read回执组装的不可变`readset:<hash>`；读集绑定原scope/revision/epoch，最多64个不重复路径，允许包含不写入的依赖。`applyFiles`最多64项，不接受重复目标，当前put接合不偷偷解释remove。全部候选字节、读回执及当前读集版本通过后才开始逐项事务；每项使用从父动作确定性派生的子动作身份。批次prepared先持久化，子回执逐项持久化，最后复核全部读集（写目标对应实际result版本，未写依赖对应expected版本）。前检冲突无副作用；已应用部分后出现冲突/依赖漂移返回partial并封锁新操作，保留实际成功条目、用户当前内容与全部原生证据。不完整批次重试不能启动未知剩余副作用；完整重试复核子回执及实际native journal，缺失证据关闭。终态落盘后发现授权失效追加不可变失效引用，不改写成功历史。批次只是技术执行回执，不能替代最终固定快照验证或D16。remove及人工恢复仍须各自完成后续单元，不能由类型声明视作已经可执行。

撤销是反向补丁，以原操作 B/A 和当前 U 三方比较；U≠A 时可安全合并才继续，否则冲突。禁止 `reset --hard`、`clean -fd`、自动 stash、覆盖用户 index 或整树恢复作为常规撤销。

**[2026-09-16 12.3字节事务细化]** 普通文件版本与备份按原始字节保存，二进制不经UTF-8往返；文本调用保留明确编码拒绝。受信原语在首次await前复制候选与expected字节，防止控制调用者的后续Buffer变化改变既定输入。内容与元数据仍来自同一次安全打开，原生单文件16MiB上限及所有身份/元数据/竞争检查保持，超限明确拒绝而不截断；二进制和文本都保存原始基线、候选与被交换对象。此为文件原语能力，不替代claim/版本端口准入。
...
**[2026-09-16 12.3输入清单实现细化]** 受信目录枚举使用固定fd链与只读Seatbelt helper，输出目录身份和名称/类型；不跟随链接，.git/.env系列/技术暂存目录按名称显式列为排除项，未知文件类型拒绝而不静默漏项。内容通过同一次安全打开读为字节，存于owner私有只读内容寻址对象；manifest绑定project/task/root、根身份指纹、策略hash、文件版本与内容对象hash、目录集合和排除清单。捕获后再核目录与文件，变化或撤权时不返回有效version。该复核仅检查此次采集稳定性；最终可信验证仍必须从存储的固定字节构建独立输入，不把两次活目录比对当执行快照。当前内部捕获上限4096项/256MiB累计内容/单文件16MiB，超限明确拒绝，不截断或隐式忽略；命令/依赖完整覆盖由后续端口接合验证。

**[2026-09-16 12.3固定输入物化细化]** 受信`LocalFixedInputs`仅从已验证manifest及私有字节对象构建每动作独立输入树，不复制执行时的活用户目录。路径由owner私有根、scope/action的内容指纹派生；排他创建，未闭合旧目录不复用。文件逐个flush并关闭写句柄，保存实际目录/文件身份、精确名称集合、内容hash/字节数/可执行位；只读文件mode用于防误写，真实项目代码的只读强制由命令Seatbelt策略承担。秘密/技术排除项不物化，源码后续变化不改写旧输入；重用必须完整核验对应不可变物化回执与整个私有树，损坏、额外项或身份漂移拒绝。准入和返回前仍复核当前授权，此内部服务本身不授予执行权，也不代替命令实测、当前用户版本资格或D16。
...
**[2026-09-16 12.3文件验证回执契约细化]** direct的受信验证消息使用`kind=workspace_validation/version=1`，精确保存projectId/taskId、Tester dispatchId/workerId、sourceWorkspaceId和不同的validationWorkspaceId、workspaceVersion、controlFingerprint、toolchainHash/policyHash/dependenciesHash、commandReceiptId/commandInputHash、完整testPaths、带相同workspaceVersion的results，以及实际exitCode/timedOut=false/quiescent=true。失败退出可形成失败实测事实，超时/未收敛不能伪造闭合验证。消息由受信验证服务以COORDINATOR/main/announce追加，ID为`workspace-validation:<dispatchId>`；模型不得生成。验证workspace须purpose=validation、独立绑定真实TESTER，保留原coding引用，不取得源目录写claim。首个Node验证器从固定manifest选择全部`.test/.spec.{mjs,cjs,js}`文件，以受信命令运行，不接受模型声称通过。依赖hash必须来自实际私有挂接回执，没有依赖也须显式绑定空依赖事实。

**[2026-09-16 12.3受信验证服务细化]** 新`LocalValidationService`从独立TESTER会话的完整manifest选固定Node命令，运行后通过非MCP的私有证据端口重读prepared/result/原生journal及stdout/stderr内容对象，不接受模型JSON代替执行事实。未声明依赖时逐次核验空依赖事实；已有声明必须取得本版本的实际安装和不可变挂接回执，缺失或损坏时拒绝，不能沿用空证明。控制指纹包含goal、活动需求/当前决策（排除已验证D16裁决）、architecture/conventions和subtask责任/依赖定义，不以iterationCount等进展误使事实失效。服务重验源目录映射、工具链、命令完整测试集、结果和控制指纹，只有通过的当前版本才能形成审阅绑定。调用者须在任务串行控制面提交返回mutations并在审阅/Leader裁决/归档处分别调用复核；WorkerRuntime在模型Step自然闭合后保留同一lease和文件会话，先运行受信completeLocalAssignment，再检查/关闭能力，最后提交done并释放lease，不能先关会话再执行验证。Leader的approve_completion在MessageService串行消息队列内等待异步版本复核，缺本机验证端口或失败时不提交裁决；已应用批准的重放也需复核才触发resume，request_changes仍可用于失效版本返工。上述入口由本机组合统一接合；服务单元测试仍不能替代组合G5。

审阅消息携带`workspaceReviewBinding={kind:workspace_review,version:1,validationReceiptId,sourceWorkspaceId,workspaceVersion,controlFingerprint}`。领域校验交叉核对唯一规范消息、先于实测的TESTER dispatch、任务/工作区/worker绑定、当前完整testResults及最新验证调度；审阅和终审必须引用同一通过版本。领域数据校验不承担I/O证明，受信组合根仍须重验私有命令证据、当前控制指纹和源目录映射，再接入D16/归档。Coordinator审阅路由从当前可信TESTER回执派生上述绑定，本机路径缺REVIEWER时拒绝直接finalize；实际执行前仍由受信组合根核验私有证据和当前映射。此段细化既有固定版本要求；完整Tester/Reviewer/真Fork/归档验收以组合实测为准，单元测试不作产品G5。

**[2026-09-16 12.3直接工作区归档细化]** direct归档必须已获得同一候选的Leader approve_completion、规范D4 resumed事实且phase=done，并完成受信命令/当前映射复核；没有恢复事实不归档。归档从私有manifest字节独立物化，不复制活源码或复用命令可写输出；沿用固定输入物化的身份、完整文件集合与内容验证。任务级首次归档引用排他绑定approvalActionId/validationReceiptId/workspaceVersion和固定物化回执，重试只核验同一引用与原树，不重建或改指新版本；漂移拒绝。产物以文件版本引用和空Git映射返回，不能伪造Worktree。归档核验后，终态收尾逐个核验本任务尚active claim的真实当前session关闭证据及全部worker已done/failed，再以批准消息为source在独立可恢复registry事务释放；归档成功本身不等于claim已释放。半事务、缺失关闭证据或隔离claim保持阻断，不提供force-release。该释放不删源码、工作区引用、历史claim或session；重试复核并复用原释放operation。

##### 12.2.5 Direct 与 Worktree
...
3. 已获宽写权限的活子进程不能靠修改registry缩权。按下述有界清理协议关闭准入并停止已登记/发现成员，相关私有写域不转授新命令；残留资源隔离。长期预览须完成停止检查，或经已验证策略证明对接管源码没有写能力且不持有相关可写fd。工具超时停止进程不是硬杀LLM流。
4. 只有受信源码写事务停止、相关范围能力核验和有界清理检查完成、held回执落盘，才显示该范围可编辑；说明采用有界清理，不宣称所有后代退出。已发现残留持有相关写能力或证据不完整时保持needsAttention。已证明独立的workspace可继续；范围无法确定时扩大阻断，不猜测读写集。
5. 交还时重读实际文件集、生成新版本、使受影响验证和投影失效，重新计算依赖；单任务非阻塞重投影沿用既有语义，若经 humanGate 则按 D4 真 Fork/新 lease 恢复。不能把“交还”当作重新授予已撤销授权。

授权撤回先关闭新操作；在途按上述安全点/工具收敛结束，界面区分撤回请求和已生效，不声称立即中断 token 或瞬时撤销旧文件句柄。根/父链漂移沿用同样的准入关闭与能力收敛，实际副作用和成功资格按§12.2.4.3分别记录。发现漂移后不得主动发起下一写入；检查后已经提交的OS操作可能完成，不靠再次检查伪称其未发生。撤回后未获新授权不取执行 lease。正常退出/重启服务的完整用户协议归 13.2；本契约提供 quiesce/resume 证据，不提前实现全团队恢复。

**[2026-09-15 架构决策更新] Leader已批准原生有界清理契约。** 采用[部署方案](reviews/task123-native-execution-options.md)§4，明确收窄任意后代完整退出承诺；Seatbelt、源码/凭据隔离、文件事务、固定输入验证及D16保持。旧PID/进程组/launchd/审计反例原样保留，不能改写为新契约已通过。

- **身份与写域**：supervisor持久登记commandId、进程出生身份、已知父子关系、授权/策略/根版本、私有输出/home/tmp/cache和预览端口。每命令使用全新身份和私有写域；项目源码/固定输入只读，Git元数据及控制/凭据域拒绝访问。旧输出路径不能改名后重新授予，registry不能撤销已打开fd。准入拒绝范围重叠、链接根、未授权硬链接及额外可写fd，对应保护必须真实验证。
- **有界停止**：默认命令30s；正常主进程结束同样做遗留检查，超时/取消先关闭新操作。对核验出生身份的已登记成员和可发现后代执行TERM（最多1s），仍存在则KILL并复核（最多2s）；发现/复核与输出收尾合计最多5s额外期限，超时即needsAttention。身份不匹配不发信号；不凭重用PID/同名程序/登录UID停止进程，也不把负PID广播当身份安全证明。主进程退出后输出最多再收集1s，单流保留上限1MiB，超限/超期标记truncated，错误单独留证；项目/模型不能延长取消收尾期限。
- **结果分离**：内部版本化记录分开保存mainResult、outputState、cleanupState和resourceState。cleanupState为pending/checked/needsAttention；checked要求全部已登记/发现成员核验停止、发现与复核未报错且输出/回执闭合，仅表示有界检查完成。assurance固定为bounded，不写allDescendantsExited=true，不以EOF、扫描空或job不存在单独签发完成。停止检查和策略证据只由受信控制器生成，不接受模型/项目进程自报。
- **风险与资源**：Leader接受checked后仍可能有漏检后代占CPU/内存、写原授权私有输出的风险。checked的旧私有写域退出活动分配，不作为下一命令写域；只有经过独立受信复制/版本校验的固定输入或候选可供后续验证，不能直接把可变输出当可信产物。残留、身份不明、停止失败、策略/登记漂移均为needsAttention，相关资源quarantined，禁止删除、复用及签发该资源安全接管/可信产物回执。单独取得停用与路径归属证据才允许回收，checked本身不证明全部输出可立即删除。
- **[2026-09-16 正常退出竞争细化]** “发现与复核未报错”不把已登记父进程在存活检查与children快照之间自然退出视作身份错误。原生发现仍如实返回exited及不完整扫描状态；supervisor仅可在原有发现/收尾期限内，按同一出生身份再次核验确已退出，并单独保存closedDuringDiscovery。该事实与扫描前已确认退出同义，不证明其后代全部发现；已登记/发现成员仍逐一停止复核，bounded风险保持。unknown、identityMismatch、helper/解析失败、容量或期限耗尽不适用此分支，继续needsAttention；不得把“后来所有已知成员都退出”作为覆盖这些失败的理由。原始逐身份discoveryFailures也保留，不只存一个布尔值。
- **容量与恢复**：checked且canonical Step/回执闭合后按D17释放执行额度，该额度不计漏检OS进程。needsAttention阻断该workspace新执行并保留composition admission，已静止worker在安全点按原D4/D17释放lease；隔离范围明确时其他workspace仍受原全局限额控制，范围/策略不明时关闭全实例本机新命令准入。重启不清除隔离记录、不凭旧PID杀进程；坏/半记录或缺出生身份保持阻断。13.2完成正常退出/恢复产品协议，12.3先实测内部中断/重放，不提供强制释放旁路。
- **验证与接管**：main exit 0不等于工具成功；还要求授权/根/固定输入有效、必要输出完整、cleanup checked和规范回执闭合。测试仅绑定该固定输入并披露bounded保证。代码生成须经受信事务取候选，不能直接发布共享可写输出。接管源码还需受信写事务及能力关闭证明；已知残留有相关写能力则不显示可编辑，源码只读也不意味着命令已停止。D16及适用测试门禁不变。

**[2026-09-16 12.3异步规范授权接缝]** 持久registry/TaskState授权读取是异步能力。受控launcher在admission/spawn/register/release等待受信`authorizeCurrent`，supervisor在running/completion同路等待；拒绝或读取失败立即关闭准入并执行既有有界收尾，不缓存最初授权替代当前事实。等待完成后仍核验绑定的根、工具和宿主指纹，释放前重读命令journal，实际进程控制辅助程序保持独立身份校验。同步内部诊断回调仍可用，但正式workspace命令必须传入上述异步规范复核。默认30秒执行和5秒收尾上限不变；显式timeoutMs只允许1–30000整数并传递至supervisor，不允许模型扩大上限。

冻结SandboxManager方法不变；明细落在本机内部companion/receipt。不引入coalition、管理员服务、VM或新ES权限，不改最低系统/签名。先验源码/私有写域隔离，再实现停止/资源记录及恢复/容量；保护失败停止依赖实现，批准契约不豁免G5。

##### 12.2.7 可观察失败

**[2026-09-16 12.3启动查询故障修复]** 首次进程加载不能与250ms身份查询预算混为一谈：启动5秒窗口先调用受信helper的`ready`，严格校验`local-control-ready-v1`且无额外字段，使用该窗口的剩余时间完成初始化；失败不创建bootstrap。初始化前后复核工具/绑定/授权，不新增或刷新总窗口。后续查询仍配置250ms超时，并在返回后检查实际截止时间；冷启动就绪不授予任何项目权限，也不保证OS实时调度。受信bootstrap已ready但尚未收到创建项目目标的指令时，首次出生身份捕获是无项目副作用的只读查询。该查询仅对明确`ETIMEDOUT`允许最多3次尝试，每次helper调用仍限250ms，全部共用原启动5秒截止时间，不刷新窗口；不足完整调用预算不启动下一次。每次前后复核绑定、撤权/取消和仍存活的同一ChildProcess；失败后让事件循环处理退出，并在接受token或重试PID前，经原私有控制端点发送`p`并取得新的`ready`响应，不能只凭可能滞后的exit事件判断存活。此时尚无项目子进程或可继承端点；通信同样使用原截止时间，关闭/异常即拒绝。身份格式/查询错误、helper漂移及授权失败均不重试。捕获失败不能进入创建/放行，不延长停止/发现/信号操作期限，也不重试任何写入或信号。内部诊断只记录尝试号、耗时和枚举结果，原始异常不作为对人DTO；该细化不授予产品权限或改变隔离/持久回执要求。原偶发超时的宿主触发因素未留足时序证据，不能写成已证明的OS缺陷；修复须以真实helper延迟故障重现和既有回归验证。实测、反例和最终修复见[helper启动超时修复](reviews/task123-capture-timeout-fix.md)。
...
| `command_timeout` / `process_not_quiescent` | 超时记录真实结果；残留/检查不完整为needsAttention，相关资源隔离，不成功/回收；checked仅表示有界清理完成 |
...
**[2026-09-15 12.3首单元实测与规格复评]** 旧探针5场景4通过/1失败及原始日志保留。复评确认失败断言要求OS在根移动后自动撤销原已打开技术文件fd，超出了本节修订后的准入/在途收敛契约；它没有实现产品失效服务，不能据此断言根内事务区必不可行或产品保护已通过。当前需补真实端口的关闭、失效回执、保全及独立外部哨兵验证，不能只反转断言。证据见[原实测](reviews/task123-local-boundary-feasibility.md)，结论以[规格复评](reviews/task123-spec-reasonableness.md)为准。

##### 12.2.8 后续 G5 矩阵与分工

**[2026-09-15 12.3文件事务首单元]** 新增内部Seatbelt单文件替换原语，包含同句柄版本/扩展属性读取、prepared留证、关闭候选写fd后交换、失效/冲突保全与回执重放检查；17项事务场景及5项原语诊断通过。仅当前macOS26.5上的内部能力验证，未接授权registry、正式workspace端口或桌面打包；最低系统、项目命令/后代收敛的新增能力G5仍未完成。Leader明确授权固定测试外发范围后，原始pnpm test完整通过：7项脚本测试、191个Vitest文件/1362项测试，三个真实模型用例均为Go deepseek-v4-flash。该轮源码G4通过不替代后续变更回归或剩余能力G5。原错误瞬撤fd断言及4通过/1失败证据保留，新断言定位为OS诊断，不能独立算保护通过。实现边界、修复及证据见[文件事务首单元记录](https://github.com/logan-suu/Agora/blob/05987ccbd20e6a55a553750577a900f44bc353d0/docs/reviews/task123-file-transaction-unit.md)。

**[2026-09-15 12.3命令私有写域首单元]** 内部Seatbelt策略及固定C/受管Node子孙进程测试已落地，Phase12本地3文件/24测试通过；源码/固定输入只读、独立新输出、模拟凭据/控制目录/相邻写域拒绝及根准入有实测。尚未接registry/MCP/产品入口，持续身份绑定、继承fd/环境、网络/IPC、有界监督及资源恢复仍待实现，不能作为完整L03/L04/L05/L14/L15通过。本轮完整回归1363通过/1失败，LRU在指定Go模型返回的SSE流缺DONE时按STREAM_CLOSED拒绝；不改代码/配置的LRU单例复验随后通过，保留首轮失败与复验结果；该检查点当时未取得一次完整全绿调用；后续停止单元的全绿结果独立留档，不降低断言。详见[命令隔离首单元](https://github.com/logan-suu/Agora/blob/05987ccbd20e6a55a553750577a900f44bc353d0/docs/reviews/task123-command-isolation-unit.md)。

**[2026-09-15 12.3停止与资源记录单元]** 新增内部audit-token停止原语、有界双流采集及LocalCommandJournal。停止原语仅证明固定登记cohort，assurance=registered-only，不能签发正式cleanup checked；后代发现未接入时即使登记成员停止，资源仍quarantined/discovery_incomplete。日志持久绑定command/workspace、策略/输入hash、根路径+dev/inode及出生身份；同路径/嵌套/改名inode复用、坏记录/遗留锁、过期revision拒绝，不提供释放入口。当前内部容量为256命令/每命令16写根及256身份/日志1MiB，满载拒绝而不删旧记录。10项新增真实过程/文件系统测试加入Phase12，本地5文件34项通过；最终版本原始pnpm test全绿（7脚本测试、194文件/1374测试、0skip），三个真实模型用例均为Go deepseek-v4-flash；生产发现/授权绑定/额度恢复及完整L14仍未验。详见[停止与资源记录](https://github.com/logan-suu/Agora/blob/05987ccbd20e6a55a553750577a900f44bc353d0/docs/reviews/task123-command-stop-unit.md)。

**[2026-09-15 12.3父子关系发现单元]** 内部helper采用sysctl快照筛选候选，当前父子关系查询由子出生身份前后复核包围，整轮由父身份前后复核包围；不同身份/查询竞态/错误不授予成员资格。发现最多256成员、1秒，返回bounded-observation而非cleanup checked；已发现并登记的后代在父进程退出后仍按原身份停止。真实Seatbelt父子孙及独立同级对照通过，输入和已退出父边界已覆盖；累计回归暴露停止阶段尾部查询预算缺陷，先真实延迟复现再修复，原期限不变，修复后本地6文件40项通过，原始完整回归7脚本及195文件/1380测试全绿、0skip；正式连续发现、持久关系/完整回执与生产授权仍待接合，资源继续隔离。来源、兼容性及完整结果见[父子关系发现单元](https://github.com/logan-suu/Agora/blob/05987ccbd20e6a55a553750577a900f44bc353d0/docs/reviews/task123-command-discovery-unit.md)。

**[2026-09-15 12.3持久关系与监督单元]** 内部资源日志v2将首次观察父子关系与出生身份原子登记，拒绝未知父/重复身份/环与过期revision；v1/坏记录保持阻断，不清空重建。内部superviseLocalCommand接受可信launcher的当前活ChildProcess，默认30秒，连续有界发现并持久登记关系；主退出/超时/取消/观察失败进入共用5秒的进程及输出收尾。输出可明确中止观察而不虚构主退出，观察摘要与停止结果落盘后仍quarantined，不签发checked。最终持久化失败durable=false，reserved/隔离不清除；每轮复核控制面revision/隔离。2026-09-16最终原始完整回归7脚本及196文件/1387测试全绿、0skip，新增7项真实监督场景通过；完整生产授权/根绑定、companion/D17及L14未验，见[持久关系与内部监督](https://github.com/logan-suu/Agora/blob/05987ccbd20e6a55a553750577a900f44bc353d0/docs/reviews/task123-command-supervision-unit.md)。

**[2026-09-16 12.3受信暂停启动单元]** 内部bootstrap在Seatbelt内用Darwin START_SUSPENDED创建目标，控制器核验内核父子身份并持久登记后，接好输出监督、复查授权接缝与日志revision，再按audit token发送SIGCONT。控制fd不继承给目标，环境从固定白名单重建；构造函数即时写标记的真实探针验证登记前未运行。目标waitpid退出码/信号与bootstrap退出分开，前置失败保留停止结果；所有资源仍隔离，不把bootstrap exit0当项目成功。Phase12本地8文件55项通过，最终完整回归7脚本及197文件/1395测试全绿、0skip；持续grant/根/工具链绑定、正式持久目标回执和companion/D17仍未接，见[受信暂停启动闸门](https://github.com/logan-suu/Agora/blob/05987ccbd20e6a55a553750577a900f44bc353d0/docs/reviews/task123-command-start-unit.md)。

**[2026-09-16 12.3持续绑定单元]** 内部LocalCommandBinding冻结调用摘要、授权身份/revision/epoch及清单引用、根与父链、工具版本/hash；在启动各检查点、运行循环和收尾复核，失效不可恢复或刷新。停止工具每次调用前独立核验，工具漂移不执行替代文件，返回不确定并隔离。内部日志v3持久保存绑定，观察摘要v2分开记录bindingFailure与实际进程结果；旧格式/重复父链/坏绑定拒绝。冻结后完整回归7脚本及198文件/1407测试全绿、0skip，新增12项真实绑定场景通过；首轮累计66通过/1失败及运行中改源的混合检查点保留。正式Leader registry/TaskState闭合、固定输入清单、工具链/OS探测、companion/D17尚未接合，不能当完整G5，见[持续绑定与失效收尾](https://github.com/logan-suu/Agora/blob/05987ccbd20e6a55a553750577a900f44bc353d0/docs/reviews/task123-command-binding-unit.md)。

**[2026-09-16 12.3目标退出持久化单元]** 内部日志v4新增只追加一次的launchReceipt v1，保存已登记目标出生身份、放行事实、真实exit/signal及受限错误码，独立于bootstrap观察结果；写入/重读校验身份与相容性，终态覆盖、旧revision和旧日志拒绝。返回launchDurable区分最终事实落盘与仅内存可见，未闭合时继续隔离，不重放命令或签发checked/工具成功。24项真实启动/绑定测试、静态/native构建及完整回归7脚本+198文件/1411测试全绿，0skip；完整回归及清理见[目标退出事实持久化](https://github.com/logan-suu/Agora/blob/05987ccbd20e6a55a553750577a900f44bc353d0/docs/reviews/task123-command-result-unit.md)。这只是技术回执，正式registry/TaskState、固定输入、持久输出、companion/D17及恢复仍待接合。

本表是**按责任任务验收的要求清单**，不是通过清单；已执行12.3证据见其任务历史，其他责任任务不得据此视为已通过。现有回归在 Docker 退役前继续执行；13.3 按 12.1 的 V01–V12 替换 Docker 专属覆盖。本机用例必须实际走新生产端口和 OS 边界，不以 mock 或仅端口单元测试替代。

| ID | 真实场景与必要断言 | 责任任务 |
| --- | --- | --- |
| L01 | 非 Git 普通目录直接编辑；安装依赖、构建、测试；编辑器可见同一源文件；无假 branch/commit | 12.3 /12.6 |
| L02 | 文件与项目脚本尝试 `..`、绝对路径、符号/硬链接、根/父目录置换、特殊文件；从未授权的独立外部哨兵不可读写。区分移走的原已准入对象；根漂移后新准入拒绝、不自动跟随，新路径或旧目录fd不扩大能力；策略拒绝可见 | 12.3 /12.5 |
...
| L06 | 文件预期版本冲突、原子创建、替换中崩溃、父目录竞争；原已打开技术文件的在途写与独立外部对象分开留证。发现漂移关闭后续操作/fd，收敛后代、失效成功/交付资格、保全旧/用户版本；半事务不假报安全失败，重复action无重复破坏，不支持卷拒绝 | 12.3 /12.5 |
...
| L14 | shell→孙进程、setsid/double-fork、超时、端口占用、停止失败、PID复用；源码/跨命令写域隔离；残留/检查错误隔离资源、不杀用户进程、不回收；checked不冒充全部退出，实测截断/额度及坏记录恢复 | 12.3 /13.2 |
...
12.3 的首个小单元应先做 launcher/文件事务可行性验证，不接普通产品入口；L02/L03/L05/L06/L14/L15 任一关键保护不能成立则停止依赖实现，回到本契约修订。不得边带着已知保护缺口边批量实验。失败先区分实际越权/保全缺陷与不可实现或矛盾的规格断言，按DOC-CONFLICT先评审并同步修正规格；不能跳过仍有效的保护或仅修改expect宣称通过。测试用目录必须在实际运行前明确批准；本设计接受不授权任意宿主路径执行。
...
本节覆盖：授权/强制边界见§12.2.2；唯一所有权、数据类型和冻结接口接入见§12.2.3；基线/文件事务/精确验证见§12.2.4；模式切换与累计集成见§12.2.5；接管及恢复见§12.2.6；失败与L01–L18验证见§12.2.7–§12.2.8。Apple/Git来源与本机手册核验仅证明机制背景，见[审阅记录](reviews/task122-local-execution-contract.md)，不替代生产路径真实验证。
...
**[2026-09-15 架构决策更新] 11.5当前验收范围调整。** Leader明确“当前无干净 macOS 15设备，先忽略这个验收”。本次11.5及其限定预览候选不以干净macOS15设备验收为阻塞，采用已完成的Apple Silicon/macOS26.5隔离下载安装、真实工具链/生命周期与适用升级证据。DEF-018保留未验证事实和后续补验条件，不能记为macOS15测试通过；最低15.0构建目标与实际验证平台披露不变。此决定不自动豁免后续阶段全部干净环境验证，也不替代新增测试的PR人工合并或具体版本发布授权。 验收例外仅改变当前验证范围，不改变运行时行为。

Electron主进程自动管理受管Node服务，基础Node/Git/pnpm/helper随包提供；用户安装后双击启动，在应用内配置自己的模型连接。服务Node与项目工具链分离。最低产品系统为macOS15.0，仅Apple Silicon（arm64）原生构建/发布及验收；[2026-09-14 架构决策更新] Leader撤销Intel/x64支持承诺，旧两架构设计不再形成验收要求。未完成目标平台实测时，该目标的发布保持未完成，不静默缩减支持承诺。
...
| `~/Library/Application Support/com.agora.desktop/` | 新桌面稳定应用目录，显式设置 Electron userData；子目录区分 `state/`、`profiles/`、`cache/`、`upgrade/`、`upgrade-history/`（已回滚尝试的只读保留归档）。产品版本和渠道不改变 canonical state 路径 |
...
**[2026-09-14 架构决策更新 · 验收暂缓]** 按蓝图§22.3.2的Leader决定，本轮11.4暂不执行macOS15干净环境补验，该环境不再阻塞本轮补验交付；结果仍为未验证，最低15.0构建目标不变。当前证据仅覆盖Apple Silicon/macOS26.5。DEF-018记录补验条件，11.5出口复核；不得把暂缓写成实测通过或扩展为后续全部干净环境门禁豁免。

##### 12.3.5 签名、公证和更新来源

正式发布仅构建arm64产物，应用 bundle 标记产品最低系统。预编译 helper 和所带 Node/Git/native modules 进入受信清单；签名需覆盖 Electron helpers/frameworks、应用自有 helper 与外部工具可执行文件。按嵌套顺序签名后验证整个 bundle，逐类评估最小 Hardened Runtime entitlement，不给普通文件 helper 加 JIT/调试能力。正式构建不以 `codesign --deep --force` 代替明确的嵌套签名计划；本轮该命令只用于 ad-hoc Spike。

**[2026-09-14 架构决策更新] 当前采用GitHub Releases DMG未公证分发。** 使用ad-hoc本地签名，Developer ID、notarytool公证、stapler和离线公证ticket不属于当前验收要求。若以后明确选择正式公证分发，再同步规则并验证。当前必须验证带隔离属性的DMG下载、安装、系统提示及用户通过系统设置手动允许后的首次启动，并记录实际系统版本与操作；不能承诺未实测的免提示安装。上游说明见 [Electron macOS 签名流程](https://www.electronforge.io/guides/code-signing/code-signing-macos) 和 [Apple 公证说明](https://developer.apple.com/documentation/security/notarizing-macos-software-before-distribution)。密钥/certificate/token 只由构建机安全存储或受控 CI 注入，不进入仓库、日志、应用或用户子进程。

当前宿主返回 `0 valid identities found`，本次没有签名账户、公证权限或干净旧系统环境（Intel已移出支持范围）。ad-hoc 完整性检查通过不等于 Gatekeeper 接受或公证通过。Leader已选择§18.11的受限分发路径：发布说明必须如实披露未公证及首次授权步骤，并由人完成适用系统授权，不自动关闭Gatekeeper、删除quarantine或宣称无提示安装达标。Apple签名资格缺失不再阻塞当前任务；最低平台、干净系统及实际安装证据仍由11.4/11.5交付。
...
安装包替换只影响 `.app`，稳定 `state/`、用户项目及工作区不跟随 app 版本改名或迁移。新桌面使用新的凭据 service ID，固定为 `com.agora.desktop.credentials`，当前用户 account 不变；所有后续新桌面版本保持此标识；按实际ad-hoc或以后获批的签名模式验证访问关系，不假设重新签名后系统授权自动连续。既有 `com.agora.local.credentials` 属旧产品配置，不自动导入、覆盖或删除。当前ad-hoc包升级也必须验证同一钥匙串项的访问连续性及授权提示；用两个不同构建的应用替换验证，系统要求授权时明确提示，拒绝授权时可恢复且不生成新主密钥掩盖旧凭据丢失。未来切换Developer ID须另验此关系。
...
5. 校验引用闭合与凭据可用性，展示升级结果；未完成工作仍由用户主动选择恢复，不自动调用模型、重取 lease 或重跑命令。15.1/16.1 负责 task-only 向会话/成员模型的实际转换，不伪造历史作者。
...
| 11.4 | Apple Silicon安装介质与受管工具链、无全局工具环境、项目准备成功/失败、ad-hoc包升级钥匙串连续性、升级事务故障注入和配置保留 |
| 11.5 | 当前系统实际安装、单实例/钥匙串/正常关闭跨包集成、适用累计G1–G7及发布候选证据；干净macOS15本次经Leader明确忽略，DEF-018仍为未验证 |
| 12.2/13.2/13.3 | 本机授权保护、安全退出与主动恢复、Docker移除及本机替代回归 |
| 15.1/16.1及后续出口 | 无损schema转换、身份/证据保留、前版安装升级；不可转换的活动工作先收敛或阻止升级 |

11.2最小Spike的16项检查及失败修复见[验证快照§8](reviews/task112-desktop-packaging.md)。它不替代上述正式验收，不授权自动开工，不代表已通过全量产品G1–G7或发布。

<a id="desktop-implementation-113"></a>
##### 12.3.8 11.3实现与验证入口

**[2026-09-14 实现同步]** 以下是11.3已确认计划的落码位置；当前门禁和交付状态仍以任务索引及[执行证据](reviews/task113-desktop-lifecycle.md)为准。

| 路径 | 实现 |
| --- | --- |
| `apps/desktop/src/main.ts`、`desktop-app.ts` | 正式Electron入口；先单实例锁，再spawn独立Node；窗口关闭隐藏、明确退出等待停服及实际进程退出；故障页及显式重试。固定userData，profiles/cache分别配置，state/upgrade独立 |
| `service-entry.ts`、`service.ts`、`service-lifecycle.ts` | 私有IPC协议、状态机和幂等stop；复用`apps/web/scripts/local-process.mjs`的Keychain与canonical控制路径，直接运行既有Next instrumentation，不改旧CLI接缝或公开端口；退出失败保留所有权 |
| `storage.ts`、`protocol.ts` | 独占owner及规范Unix控制锁、格式1与未闭合升级拒绝；只允许安全环境项、固定新Keychain service、精确Host/Origin/capability和IPC字段验证；不自动清除陈旧锁 |
| `preview-server.ts`、`window-security.ts`、`preload.cts` | 每次启动独立会话；仅允许预览页、静态资源及状态HTTP/SSE；其余产品API在进入Next前拒绝。nonce CSP、精确origin认证注入、外部导航/窗口/权限拒绝、旧session连接撤销及当前主frame校验；preload仅status/restart/quit |
| `apps/web/src/app/desktop/`、`apps/desktop/ui/` | 已有视觉风格下的安装预览及服务故障页面；普通Web运行的桌面页返回notFound，不新增开发入口 |
| `apps/desktop/scripts/build.mjs`、`package.mjs`、`sign.mjs` | 冻结依赖、独立受管构建工具、正式Packager、生产资源清单、fuses及逐层ad-hoc完整性验证；保留输入hash、trace审查结果和资源hash清单 |

打包单独追踪custom server的Next/React/Harness/既有Docker依赖入口；Next动态webpack别名及Babel runtime清单显式纳入。已识别的仓库TypeScript开发源码/测试文件和ssh2 Windows专用资源只作为trace候选记录在`trace-review.json`，不复制到macOS包；未知候选、禁入路径、外部链接及特殊文件报错。pnpm内部链接保持相对且最终包再次审计。原Docker依赖仍用于既有代码兼容，预览无Docker执行入口，实际退役归13.3。

正式候选使用`pnpm build:desktop <verified-download-directory> --revision <git-commit>`，输入提交必须含本任务已审阅代码；默认工作树模式仅供未经提交授权的研发验证，标记`development-working-tree`并记录每文件hash，不能作为固定提交发布候选。本轮构建入口只接受darwin-arm64；完整安装工具链交付由11.4完成；后续Leader已撤销x64交付承诺。当前只读写格式1；有非空upgrade记录、未知格式或未声明转换均拒绝，完整事务由11.4实现验收。

验证入口：`pnpm test:desktop`覆盖真实子进程/HTTP/SSE/控制锁及故障单元；`scripts/validate.cjs`、`validate-host.cjs`由Electron运行，加载正式包内代码和受管Node/真实Next，以专用临时Keychain替换凭据存储目标。注入点只存在于受信主进程/服务组合，不向renderer开放；模型不调用，测试结束删除本次专用Keychain。测试fixture明确区分故障单元与真实G5；后者不依赖源码开发服务器或内存凭据替身。

**[2026-09-14 11.4实际安装补验修复]** 正式ESM入口不得以顶层await等待内部依赖`app.whenReady()`的`runDesktop()`，否则Electron等待模块完成、模块又等待ready而互相阻塞。入口直接启动异步组合并处理失败，`runDesktop()`仍同步执行ready前的路径/单实例设置，保留其内部ready后初始化和退出协议。`apps/desktop/test/entrypoint.test.ts`由真实Electron在ready之前评估实际生产入口，隔离appData并在ready事件后退出，不启动模型或访问用户凭据。保持`GrantFileProtocolExtraPrivileges=false`，每个窗口的隔离session显式注册`file`协议处理器，只对固定包内`status.html/status.js/status.css`的精确URL响应GET并设置对应MIME；其他本地文件和方法拒绝，导航/IPC白名单及CSP不放宽。此前`validate-host.cjs`是在Electron已ready后才加载组合根，不能单独证明正式入口可双击启动；后续必须补验实际`.app`入口及带quarantine安装路径。失败和后续结果见`docs/task-history/11.4.md`。

**[2026-09-14 实现修正 · PR #81]** 已存在state目录也必须属于当前用户、非symlink且无group/other权限；不满足则在创建owner前拒绝，不静默chmod。菜单、IPC与受信测试入口共用幂等restart操作：等待在途启动，停止旧服务并确认实际exit，销毁旧session，再以新capability启动；未退出失败不启动替代服务，quit优先。停服唤醒readiness等待并立即标draining，但已发出的Keychain操作仍须自然收尾；停服后新读写被拒绝，原生操作结束前不释放owner。验证器的有界强制清理仅用于无模型测试进程，统一识别exitCode/signalCode并清理监听器；产品不调用该清理器。fixture分别跟踪服务构建和启动，stop先等构建归属再请求服务停服，最后等待启动闭合，禁止晚到ready。修复及回归见[PR81记录](reviews/task113-pr81-fixes.md)。

<a id="desktop-implementation-114"></a>
##### 12.3.9 11.4工具链、介质与升级基础实现

**[2026-09-14 实现同步]** 下列为11.4功能分支的实现入口；门禁结果与未完成条件见[任务历史](task-history/11.4.md)，不以代码存在或开发包构建代替正式验收。

| 入口 | 行为 |
| --- | --- |
| `apps/desktop/src/toolchains.ts` | 只读检查packageManager、lockfile及Node声明，选择锁定Node24.20.0、npm11.19.0或pnpm9.15.9。显式支持数值完整/部分版本、比较器及`^`/`~`和空白连接的交集；其他语法、管理器、未入清单版本明确拒绝。冲突不改写项目。受管环境从零构造，独立私有HOME与npm用户/全局配置路径；PATH和Git exec/template指向包内工具。该模块不提供项目命令执行入口 |
| `toolchain-installation.ts` | 清单覆盖文件hash、执行位、全部内部symlink目标及必要组件；文件hash经流式SHA-256计算，避免按可执行文件大小分配缓冲；主进程在启动Node前验证，服务再次验证后才报告工具就绪。失败页指出损坏组件；预览HTTP/SSE只返回就绪/版本，不返回凭据或命令入口 |
| `toolchain-cache.ts`、`scripts/download-toolchains.mjs`、`scripts/toolchain-catalog.mjs` | 固定HTTPS来源、版本/架构/hash/体积上限；下载使用应用cache staging和私有回执，flush后原子发布。失败/取消只回收本次未发布staging，已有版本复核hash/回执后复用。清单只含当前锁定基础版本，不启用其他项目版本或后台自动更新 |
| `scripts/build.mjs`、`package.mjs` | 当前只交付arm64原生macOS构建，拒绝跨架构代跑；旧x64入口不构成当前支持承诺；加入Git/pnpm、Node附带npm/npx、既有文件/钥匙串helper、包内相对启动脚本和来源/许可证。12.3新增根检查、根初始化、文件事务、命令bootstrap、进程控制五个预编译helper，纳入签名及最终工具清单；POSIX构建脚本在macOS以15.0为最低目标，钥匙串原目标保留，产品仍要求macOS15。默认工作树输入逐文件hash仅用于开发验证，固定提交模式才可进入发布候选 |
| `scripts/sign.mjs`、`media.mjs`、`validate-media.mjs` | 按嵌套顺序签名，工具二进制签名后生成清单，再封装bundle；Node/Electron与普通helper区分JIT entitlement。无身份时显式ad-hoc开发验证；有效Developer ID可传入独立介质步骤，生成DMG/ZIP及校验和。脚本不自动上传公证或发布；当前采用ad-hoc DMG发布，不要求Developer ID/公证/staple，当前Apple Silicon/macOS26.5隔离下载安装及ad-hoc升级钥匙串授权恢复已补验通过；macOS15干净环境按DEF-018暂缓且未验证，11.5复核 |
| `src/upgrades.ts`、`upgrade-files.ts`、`storage.ts` | 升级要求持有同一state owner，路径、格式、原/目标hash及固定转换器身份先校验；`preparing`备份准备、`prepared`、逐文件应用/回滚journal、最后格式提交及终态回执。准备未完成时只允许核验原状态后撤回；备份坏、未知转换、源文件异值、失去owner均拒绝。所有备份保留，未闭合阻止服务常规入口，不移动workspace或自动恢复工作。已回滚的同ID迁移重试时，在owner复核后将整个旧尝试原子移至私有`upgrade-history/<id>-<uuid>/`并flush两侧目录，原journal/备份字节不变；再建立新的活动尝试。归档后中断可重新准备，未闭合尝试不归档，归档目录不参与活动迁移恢复，也不自动清理 |

生产当前仍只读写桌面格式1，没有已批准的1→2领域迁移，也不对外提供转换器注入或任意迁移控制API。升级内核用明确的专用测试格式及临时密文/身份fixture验证断点恢复；未来领域转换仍由15.1/16.1定稿接入。格式变化未实装不等于可以绕过未知格式拒绝；升级准备目录连首份journal也未完整落盘时保持阻塞，由受信恢复核验处理，不能凭目录年龄自动删除。

验证入口：`pnpm test:desktop`涵盖全部已实现Phase11测试；`tests/integration/phase11/phase11-4.test.ts`验证工具准备/缓存/升级回滚跨模块保留配置；`scripts/validate-tools.mjs <app>`在专用临时项目运行包内Node/npm/pnpm，真实获取固定依赖并执行生命周期/测试脚本、Git linked worktree和文件helper越界拒绝；`validate-host.cjs`验证正式Electron组合根与隔离真实Keychain。通用真实模型回归仍固定Go `deepseek-v4-flash`，正式Benchmark未开启。开发验证不得冒充Apple Silicon无工具干净Mac或macOS15正式安装验收。 11.4原生复验发现正常停服可能打断未完成导航，主进程现以对应lifecycle及当前ready状态判定导航失败，避免旧/已停服务的异步页面拒绝污染当前状态；真实子进程回归覆盖活跃加载失败和停服后迟到失败。

##### 12.3.10 Phase 11 出口验证入口（11.5）

**[2026-09-15 验证范围与清理同步]** 本次11.5的干净macOS15项按蓝图§22.3.2获批忽略，不阻塞当前验收；未验证事实仍由DEF-018保留。按蓝图§21 TEST-CLEANUP，每轮测试后归档结果/清单/hash，再清理已停用的测试下载、解压构建和安装副本。清理不改写既有通过证据；已删候选仅保留验证记录，发布重建后须复核。

**[2026-09-15 验证同步]** `tests/integration/phase11/phase11-exit.test.ts`组合正式DesktopService、工具完整性、状态唯一写者/格式、升级恢复和真实HTTP/SSE：验证损坏组件在状态与Next初始化前拒绝，认证预览保持产品入口关闭，活动SSE随正常停服闭合，重开不接受旧capability；未闭合迁移阻止就绪，回滚后保留配置字节及state根身份。测试为确定性故障/接缝覆盖而控制Next readiness并使用合成工具字节，文件头明确说明；不替代真实安装包、原生Keychain或工具执行G5。

固定提交候选、正式应用入口、隔离下载安装及原生验证结果见[11.5出口报告](reviews/task115-phase11-exit.md)。当前macOS26.5开发机证据与DEF-018的干净macOS15未验证状态分开记录；11.4历史升级证据仅在比较对应helper/服务字节及验证范围后复用，不重写为本轮新执行。此段登记验证入口，不宣称阶段通过或已发布。

<a id="docker-retirement-121"></a>
##### 12.3.11 Docker退役清单、新状态入口与本机回归替代（12.1正式定稿）

**[2026-09-15 架构决策更新] D18 / 12.1正式定稿。** Leader已接受本节退役清单、新状态入口、回归替代与切换门禁，作为13.3实施及13.4验收输入；设计接受不表示当前代码已退役。源码基线、文件清单与审阅证据见[12.1清点报告](reviews/task121-docker-retirement.md)，当前执行状态只查task-status.json。范围是Agora执行后端及依赖它的入口；用户项目的Docker文件、用户安装的Docker与历史研发证据不纳入删除对象。

###### 12.3.11.1 退役范围与工程归属

| 编号 | 现有位置/行为 | 13.3必须收敛到的结果 | 前置输入 |
| --- | --- | --- | --- |
| R01 | `packages/runtime/sandbox/src/docker-sandbox.ts`；`factory.ts`的Docker静态导入、`kind: 'docker'`、Docker类型/客户端导出；`index.ts`再导出 | 删除Docker实现、选项和导出。未知/旧Docker配置明确拒绝，不落入LocalTemp、本机或默认分支。保留Phase0 LocalTemp真实测试能力，但普通产品入口不能通过`kind: 'local'`取得未授权宿主执行 | 12.2本机端口和工厂准入；12.3实现 |
| R02 | `apps/web/src/server/task-composition.ts`默认Docker装配、legacy共享工作区resume、`stableFiles: DockerSandbox`、Git及归档的`withStableFiles` | 产品只装配已授权本机workspace；旧Docker/legacy任务恢复分支退役。改接本机文件稳定性与进程收敛能力，保留失败关闭、不可变归档receipt及重试校验 | 12.2、12.4、12.5、13.1、13.2 |
| R03 | `workspace-adapter.ts`、`packages/tools/git/src/git-service.ts`的执行绑定、停止后回收和task-owned路径假设 | 保留真实linked-worktree身份、确定性worker隔离键、累计base、补偿、冲突abort及验证能力；按12.2/12.4适配用户项目根。不可删掉根身份校验来容纳direct目录；direct使用独立类型/能力，不填假branch或commit | 11.1 §12.1；12.2、12.4 |
| R04 | `packages/runtime/sandbox/package.json`、`apps/web/package.json`、`pnpm-workspace.yaml`、`pnpm-lock.yaml` | 删除dockerode与其类型直接声明；由包管理器按剩余依赖图重建锁文件。docker-modem、ssh2、cpu-features等仅在无其他引用时随闭包移除；simple-git、MCP、Harness、原生文件/钥匙串helper保留，不按名称批删共享依赖 | 13.3源码与Eval调用链完成替代 |
| R05 | `apps/web/next.config.ts`、`apps/desktop/scripts/package.mjs`、`apps/desktop/src/build-resources.ts` | 删除Docker external/tracing/复制白名单入口；重新验证脱离源码的正式服务/应用、包清单、第三方声明及签名hash，不仅检查package.json | 11.3/11.4打包契约；13.3 |
| R06 | `apps/web/scripts/local-diagnostics.mjs`、`local.mjs`及根package脚本；`local-process.mjs`中的共用Keychain/控制组件 | 撤掉旧产品setup/doctor/start/stop/adopt-key命令作为旧状态入口的路由和文档；保留桌面实际复用的受信控制、凭据及开发构建工具，不能整目录删除。需要源码开发入口时必须显式采用新本机准入/状态契约，不保留旧CLI恢复产品任务的旁路 | 本节状态入口；12.6、13.2 |
| R07 | `packages/roles/definitions/src/runtime-contracts.ts`的容器无Git提示；Git及adapter注释、`README.md`、`docs/install-macos.md` | 新角色环境说明按实际已授予MCP工具和受管工具链更新；不因宿主有Git放开shell Git或push。当前安装说明改为本机产品范围；历史阶段记录保留并指向退役证据 | 12.2工具权限；13.3 |
| R08 | `packages/core/__tests__/e2e/phase0-runtime.ts`、`phase1-runtime.ts`及所有Docker测试装配 | 保留LocalTemp低阶段回归，移除Docker工厂选项；产品G5接真实获批本机backend。按下表逐断言分类，不能整阶段删测试或排除匹配 | 12.3–13.3 |
| R09 | `tests/evals/phase6`至`phase10`的Docker scenario/driver/verifier及模型诊断入口 | 保留Outcome/Process/Efficiency/Safety、隔离验收及失败清理逻辑，改接本机受信执行与版本证据。删除镜像/容器专用适配和过时元数据生成，不留下导入失败或静默回退的命令。正式Benchmark仍不自动恢复 | 本节Eval边界；13.1、13.3 |
| R10 | 当前用户界面/HTTP生命周期与预览门禁（desktop preview server、task handlers/runtime、message runtime） | 12.7以前仅授权验收目录可执行；13.4通过前不开放普通项目。拒绝旧状态/旧task/未知backend要在创建Harness/MCP、取lease、执行工具和对外展示旧内容之前完成 | 12.6、13.2–13.4 |

以上路径指当前基线，不是未来任意同名文件的删除授权。13.3须重扫引用及受影响调用图，记录新增/改名项并逐项核对；源码变化不能使清单失效后仍声称覆盖完成。

###### 12.3.11.2 新状态入口与恢复边界

1. **沿用新桌面稳定根**：`~/Library/Application Support/com.agora.desktop/state/`、`desktop-format.json`、唯一写者锁和§12.3.6升级事务继续有效；不另建每次发布的空state根。Phase11桌面配置、模型密文、Keychain service `com.agora.desktop.credentials`以及后续Phase12已验收工作必须保留。所谓“新运行状态”是相对旧Docker产品，不授权再次清空新桌面状态。
2. **唯一生产注入**：主进程选择固定应用根，服务取得owner、验证格式/升级闭合和凭据后，把同一canonical root传给全部State/消息/模型配置/Trace/任务组合。现有`AGORA_DATA_ROOT`仅可作为受信组合根向既有模块传递该根的内部桥接；不得接受renderer、继承环境、旧CLI参数或cwd `.data` fallback来选择产品数据。测试显式注入临时根只限受信测试组合，不开放给用户任务。
3. **首次与升级分开**：空新桌面根按当前格式初始化；已有新桌面格式按声明版本读取，格式变化走§12.3.6的可恢复事务；未知/损坏格式、非空无格式根、owner冲突及未闭合迁移保持入口关闭。不能靠复制旧`.data`再补写`desktop-format.json`把它变成新任务。
4. **新工作创建**：只接用户显式选择/创建的项目和获批workspace。12.2定稿的版本化绑定须证明projectId/taskId/workspaceId、direct/linked-worktree模式、用途及授权根；旧TaskState仅有worktree路径/branch不构成授权。在12.2绑定schema未落地前保持新执行入口关闭；12.1不发明另一份TaskState或控制库。
5. **旧数据无入口**：不扫描、导入、列出、只读展示或恢复旧`.data`任务、`agora-docker-*`目录及`com.agora.local.credentials`。不跟随其中的worktree/session引用、不枚举旧容器、不调用Docker清理。用户可从文件选择器重新授权已有代码项目；这只创建新工作身份，旧gate、session、validation receipt不转换成新工作事实。状态目录不能作为普通代码授权根。
6. **恢复先校验再执行**：13.2只恢复已验证的新本机绑定；核对状态格式、scope/根身份、当前文件/HEAD、checkpoint与官方lineage、模型绑定及规范控制回执。缺绑定、旧Docker描述、跨task路径和未知版本均在副作用前拒绝；没有恢复证明就显示可解释故障，保留文件与配置，不能创建空工作覆盖旧事实或自动重跑。D4保worker身份、真Fork及新lease不变；完整异常整队恢复仍属20.2。
7. **错误分类**：沿用已有`unsafe_state_path`、`state_in_use`、`unsupported_state_version`及升级故障；本机授权/旧任务类型/工作区证据错误由12.2、13.2定义正式类型与用户说明。不同错误须可区分且结果无模型/工具副作用；本节不先造无实现的API错误码。无Docker可用与否不参与新状态判定。

###### 12.3.11.3 冻结契约与安全替代要求

Executor、TaskStateStore、MessageBus及SandboxManager六签名保持§12.1.7原文；删除Docker专属实现/配置不表示允许改通用端口。`SandboxConfig`的Docker分支、`DockerSandboxOptions`和`Dockerode`属于退役实现表面，13.3同步调用者；保留工厂的LocalTemp默认只服务低阶段测试，禁止将其作为新生产本机工厂。direct能力、配置联合与必要schema变化由12.2先定稿，不能在13.3临时扩大公开接口。

现有`WorkspaceExecutionBackend`、Recoverable companion和Git稳定文件hook可作为接缝，不能仅把Docker类替换为LocalTemp而保留原保护声明。12.2/12.4/12.5必须证明授权文件与命令边界、固定根身份、未提交修改/外部编辑保护；13.2证明受管工具/服务的安全停止及失败收敛。无法证明稳定的Git/测试/归档输入就拒绝提交有效receipt；停止失败不得回收仍被使用的工作区。不得承诺阻止任意第三方编辑器，也不得把`withStableFiles`替换成无操作回调。

容器专用机制（镜像Node20、mount namespace、networkMode、memory/cpuShares、daemon/socket、容器内timeout及pause/unpause）随实现退役。其背后的实际承诺分别处理：命令默认30s和真实退出结果保留；新工具/服务停止对象与方式由12.2/13.2确定，不能杀LLM token流；凭据不继承、未授权根不可访问要有受信强制保护与真实G5。容器资源/网络隔离不冒充本机已有能力，也不以其删除为理由放开任意宿主命令。新本机G7依据获批边界；若12.2无法提供所需保护，依赖实现停止而非回退Docker。

###### 12.3.11.4 累计回归映射

每项迁移保存旧测试路径/用例身份、原断言意图、新测试/实现、真实证据及对应提交。下表是必保留语义，当前文件内其他有效断言也须核对；13.3交付逐用例账目。新测试路径由对应实现任务确定，禁止提前建空测试。所有适用项在13.4累计复验。

| 编号 | 现有回归入口 | 处理与本机可观察证据 | 实施归属 |
| --- | --- | --- | --- |
| V01 | sandbox `docker-sandbox.test.ts`、`factory.test.ts` | 镜像版本、container复用、Docker options转发删除；读写、拒绝逃逸、stdout/stderr/非零退出、30s超时、跨task拒绝及幂等清理迁移到真实本机测试；另测旧kind拒绝和生产不能走LocalTemp | 12.2/12.3、13.3 |
| V02 | `docker-isolation.test.ts`；`phase9-filesystem-boundary.test.ts`；`secure-files.test.ts`、`path-guard.test.ts` | mount数量/pause API断言退役；保留别名重定向、根dev/inode漂移、symlink/竞态/越界、Git元数据与后台写入竞争；验证本机保护实际拒绝且未改用户文件。原fs/local覆盖继续执行 | 12.3–12.5、13.3 |
| V03 | `workspace-adapter.test.ts`；`phase9-worktree-integration.test.ts` | 真实Git common-dir与唯一worktree path/branch/HEAD、同岗位worker分区、bind失败补偿、专用integration与conflict abort、停止失败保留和重试；Docker mount断言改为本机授权绑定及根一致性 | 12.4/12.5、13.2/13.3 |
| V04 | `phase1-exit.test.ts`及Phase0/1 runtime；Web `orchestration-flow.test.ts`、`orchestration-flow-docker.test.ts` | 保留全部MCP/State/Harness/角色/投影链；删除Docker endpoint和双后端选择专属项，真实产品G5换本机。LocalTemp低阶段回归保持且不计作本机产品验收 | 12.3、13.3 |
| V05 | `phase5-exit.test.ts`、`phase6-exit.test.ts`及Phase6–8 scenarios间接调用 | 保留真实HTTP/POST、持久消息先于SSE、刷新、角色/Channel、最终产物独立复验；本机临时目录/真实子进程执行，无Docker存在性skip | 12.3/12.6、13.3 |
| V06 | `phase9-parallel-flow.test.ts`、`phase9-exit.test.ts`及Phase9 scenario | 保留全局lease、公平与admission、canonical join、cohort暂停、D4多worker真Fork、DAG累计base、独立TESTER验证、D16终审与不可变归档；资源泄漏断言改为本机进程/工作区/lease证明，不能删掉 | 12.4/12.5、13.1/13.2/13.3 |
| V07 | `phase10-model-settings.test.ts`、`phase10-resilience.test.ts`、`phase10-exit.test.ts` | 保留用户连接冻结、凭据保护、真实模型配置、原生思考/容量/期限、重试预算、需求确认与版本失效、D15白名单；独立产物复验也通过正式本机授权入口 | 12.6、13.1–13.3 |
| V08 | `phase10-benchmark.test.ts`及其`final/single-driver.ts`、`multi-driver.ts`、`verifier.ts`、`holdout-verifier.ts` | 默认测试调用的确定性真实链迁移，保留单/多模式、可信Outcome、D16、模型与验收侧隔离、无伪造通过及清理证据。移除容器实现不豁免这组默认回归 | 13.3 |
| V09 | `final/benchmark-image.test.ts`、`task-container.test.ts`、镜像fixture及专属适配 | 镜像唯一digest、VM挂载路径归一化等仅Docker语义可退役；共有manifest指纹、任务归属与cleanup判定迁移/保留。完整映射留证，不把所有Benchmark文件一起删除 | 13.3 |
| V10 | Web `local-diagnostics.test.ts`、桌面storage/upgrades/security/service及Phase11集成 | Docker CLI/engine安装建议退役；新桌面无Docker照常启动、受管工具完整性、独占状态根、Keychain连续性、未知格式拒绝、未闭合升级与停服失败仍验证。旧CLI专属项随入口删除，共用凭据/控制断言保留 | 12.6、13.2/13.3 |
| V11 | 新状态与发布接缝 | 空新根初始化；新桌面原配置/已验收任务升级保留；旁边旧`.data`/旧凭据字节不变且不列出；复制旧任务/缺本机绑定拒绝；不继承外部AGORA_DATA_ROOT；旧配置/旧resume无副作用；无Docker安装包端到端闭环 | 12.6、13.2/13.3、13.4 |
| V12 | D18新增direct/接管/退出场景 | 非Git目录编辑/安装/构建/测试、未提交修改保留、并发外部编辑与冲突、人工接管交还、关闭窗口后台继续、显式退出到安全点、用户选择恢复及不确定副作用拒绝自动重跑 | 12.3–12.7、13.1/13.2、13.4 |

默认Vitest匹配及任务追踪测试不缩减；移除已删除能力的用例必须在正常PR差异中逐项解释。真实外部模型允许已有规范的外部依赖mock单元测试，但G5不以执行器/文件/子进程test double替代；当前通用真实模型继续Go `deepseek-v4-flash`，失败/缺凭据报错，不换提供方或skip。TEST-CLEANUP在实际测试后保存来源/hash/日志，再清理确认停用的测试专用目录；不删除正常项目依赖、全局缓存、用户项目或产品状态。

###### 12.3.11.5 Eval与历史边界

`tests/evals`不是全部可退役历史：默认`pnpm test`包含其中`*.test.ts`，Phase10 benchmark默认测试还导入Docker driver/verifier；Phase7/8复用Phase6场景。因此13.3必须沿导入关系迁移，不能仅搜测试名或把这些目录从tsconfig/Vitest排除。

工程Eval的通用任务、grader、统计、manifest及报告逻辑保留；使用生产backend的执行/独立验收与清理适配迁移到本机，写新runtime/version元数据。镜像构建fixture和Docker专属image/mount适配只在没有有效调用者且相应共享契约已覆盖后删除，保留原Git提交与证据hash。不能将历史image/model/cost/成绩改写成本机结果，也不能将标准Node案例的本机复验称作未运行的官方Benchmark。

显式Eval命令不得因缺backend自动跑LocalTemp或遗留Docker；13.3在本机已支持的fixture上验证确定性执行/清理。正式模型Benchmark继续保持暂停：旧冻结清单与新runtime不匹配时，在模型请求/依赖下载/用户代码执行前明确拒绝，错误说明需重新批准环境、配置和预算；不在本任务或13.3自动启动正式计费组。既有Phase10冻结/报告入口不应因Docker导入被删除而无法只读处理历史结果。新外部评测环境若需要第三方工具/依赖，另按D11定稿，不为它保留Agora Docker backend。

###### 12.3.11.6 切换顺序、失败与验收

1. 12.1已接受本清单；12.2完成本机授权/文件版本/直接模式/companion及错误设计；12.3–12.6在明确授权的验收目录实施并实测，保留旧路径仍适用的回归。12.7验收限定本机预览，不提供新桌面Docker选项。
2. 13.1补足精确交付与只读展示证据，13.2完成新本机退出恢复；13.3先建立逐用例替代与可跑证据，再在同一受审变更范围移除R01–R10专属路径。移除过程中开发树可暂不构建，但未闭合变更不得宣称通过或交付。
3. 13.3重新搜索Docker imports/动态require、daemon/socket命令、kind/config、Next external、打包清单及锁依赖；对剩余命中逐项说明其为历史文案/数据或用户fixture。生产包不得含可调用Docker后端/客户端。运行全量typecheck、lint、pnpm test、适用真实G5，确认新入口没有旧状态或fallback旁路。
4. 13.4在未安装Docker的受支持Mac运行安装、自配模型、direct/并行Worktree、用户修改保护、真实测试评审、Leader终审、退出及主动恢复，验证上一已发布桌面版本升级。按§18.11记录最终提交/版本/清单/hash、实际机型/系统及G1–G7；11.5的macOS15例外不自动扩展到此阶段。
5. 代码PR仍由人类合并；发布仍须人工dev→main及独立授权。失败时维持未通过/入口关闭并保留状态与证据；不通过运行Docker、旧数据兼容、跳测试或新空状态绕开失败。应用升级回退按§12.3.6执行，不自动回到旧Docker产品。

12.1设计验收：R01–R10覆盖后端/依赖/配置/安装启动恢复/文档/Eval；§12.3.11.2覆盖新状态所有权和旧数据拒绝；V01–V12及§12.3.11.5覆盖保留契约、本机替代与专属测试退役；§12.3.11.6覆盖顺序、失败与证据。12.1接受时12.2强制保护机制尚待定稿；现已按§12.2接受设计，仍须后续实装验证，不构成任意宿主执行授权。本任务没有强制打包/协议Spike；本轮仅文档审阅，未执行任何产品G5。

#### 12.4 后续批次的契约输入与任务绑定

| 批次 | 定稿入口 | 需要验证的关键不变量 |
| --- | --- | --- |
| 第二批 / Phase 15–21 | 15.1、16.1、17.1、18.1、19.1、17.4、20.2、21.1；完整定义入口见§12.6 | 新需求排队与当前需求修改区分；自动调配只消费已授权工作；项目/全局配额；短回应串行切换/释放重取额度；私聊确认与小群共享权威控制入口；附件解析能力与来源；费用估算/在途请求不伪装硬封顶 |
| 第三批 / Phase 22–25 | 22.1、23.3、24.1、25.1；完整定义入口见§12.7 | Librarian 按配额维护；正文/确认版本/索引一致；建议不自动成为规则；事件去重及失败追补；必要记录与非关键维护分级；配置安全点生效、权限撤销、模板升级保留个人覆盖、MCP 生命周期 |
| 第四批 / Phase 26–31 | 26.1、27.1、27.4、28.1、28.3、29.1、29.3、30.1、31.1、31.3；完整定义入口见§12.8 | 工作地图只读真实事实并走规范控制入口；原始模型推理不可展示；需求到代码/测试/交付证据精确关联；语义预警须实证；实验需批准；需求撤回保护共享改动及不可逆副作用 |
...
**[2026-09-13 架构决策更新 · D18 全量任务拆分]** Phase 11–31全部任务已登记。下列是每个设计任务必须产出的规格清单；精确字段、接口签名、算法阈值与实现选型仍在对应任务定稿，本文不凭规划伪造已实现schema。设计应交付：所属真相源及写入所有权、类型/接口和控制入口、权限与幂等规则、生命周期及故障/恢复语义、可观察状态、真实验证案例、受影响旧契约的同步清单。实现依赖该设计完成，阶段出口继续执行，不因预登记而跨阶段开工。

#### 12.6 第二批（Phase 15–21）：会话、人力与协作

##### 12.6.1 项目与工作会话（设计15.1；实现15.2、15.3、15.4）
...
##### 12.6.2 稳定成员与交接（设计16.1；实现16.2、16.3、16.4、16.5）

同成员跨执行保持稳定身份及一件主要工作责任，同岗位可有多位成员。原子登记占用/指派与历史，不复制未完成责任；默认首项工作明确。用户招募与全员建议分别建模，建议可见、Coordinator去重、用户批准；离职先安全点收敛，缺接任者可恢复等待。首版模板与基本职责编辑先落地，通用能力和自然语言岗位由Phase24–25扩展。

##### 12.6.3 两层额度与短回应（设计17.1；实现17.2、17.3、18.3）
...
##### 12.6.4 协作请求、投影与私聊（设计18.1；实现18.2、18.4、18.5、18.6、19.5）
...
##### 12.6.5 附件、视觉与保留（设计19.1；实现19.2、19.3、19.4）

只将图片、文本/代码、PDF列为首版，独立定义上传/提取/提供状态和版本引用；本机代码优先引用。视觉调用只用已配置能力并计费，OCR不冒充图像理解，能力缺失不阻塞文本开发。归档不删除材料，用户删除前显示已有引用影响；23.1再接知识引用。删除不作用于用户原始项目文件。

##### 12.6.6 费用（设计17.4；实现17.5）

逐调用关联成员/协调/视觉等实际用途，未来KB、配置辅助和实验同路计量。明确价格/usage可信来源、未知值、币种/周期、估算及在途费用；达到阈值停新调用，由用户决定继续，不宣称供应商硬封顶。并发额度与费用账本不可互相替代。

##### 12.6.7 多成员接管与整队恢复（设计20.2；实现20.1、20.3）
...
##### 12.6.8 编排优化与效果（设计21.1；实现21.2、21.3、21.4；验证21.5、21.6）

模型建议只生成可校验计划，代码约束权限/依赖/资源与调整范围；波次改进不遗失batch或跳过精确累计验证。失败归属有证据才缩小修改范围，未知时保守扩大，测试自身错误按根因处理。动态增量集成及Tester preparation分别由29.1/29.3设计，不能在21.3提前启用。固定条件重复评估等待、返工、时间、费用和质量，收益未测不得宣称。

#### 12.7 第三批（Phase 22–25）：项目知识与成员能力

##### 12.7.1 知识权威、维护与有效性（设计22.1；实现22.2、22.3、22.4、22.5、23.1、23.2；门禁22.6）

正文在项目可读文件，索引/来源/确认版本与正文一致；不替代任务、代码或测试状态。客观事实与已确认记录可自动维护，新需求/架构/规范只能提案，经Leader确认才生效。Librarian拥有稳定成员身份，事件队列去重/重试/受配额及预算约束。必要决策、验证与交接齐全后开发可完成，非关键整理独立显示；过期、直接编辑及删除需传播到消费者并重新核验。D3写入须测试与显式/approve-kb双门禁，登记22.6不代表已经批准写入。

##### 12.7.2 显式知识复用（设计23.3；实现23.4）

先决定复用内容与复制/引用语义，再提供源/目标/权限/冲突预览和用户确认。跨项目不自动共享聊天或未确认知识，不把源规则自动提升为目标项目权威；来源变化、撤销访问及目标个人调整必须可解释。

##### 12.7.3 技能、MCP和配置版本（设计24.1；实现24.2、24.3、24.4、24.5）

项目安装/接入、成员分配、工具端可执行授权分离；Skill文本不授予权限。定义配置来源/版本、凭据存储、连接进程生命周期、工具能力与失败。普通变更在安全点生效，撤权阻止新的非法调用并规定在途收尾。模板升级展示差异且保留个人覆盖；恢复必须核对依赖和授权版本。通用插件包/市场后置到31.1、31.2、31.3、31.4，但现已具备任务编号。

##### 12.7.4 自定义岗位与综合验收（设计25.1；实现25.2、25.3、25.4；验证25.5、25.6）
...
#### 12.8 第四批（Phase 26–31）：展示与工程深化

##### 12.8.1 地图、上下文与证据（设计26.1；实现26.2、26.3、26.4）
...
##### 12.8.7 虚拟空间（设计30.1；实现30.2）

空间只呈现真实项目/成员/小群/待决事实，与地图共用控制入口。视觉/引擎/化身由设计任务决定，键盘、减少动态效果、性能预算和等价紧凑视图必须验收；动画不调度或额外调用模型。

##### 12.8.8 插件包与市场（设计31.1、31.3；实现31.2、31.4）

先包格式、权限/依赖/兼容与生命周期，再来源及分发契约。安装/更新/撤回进入24.1的授权与成员配置流程，不因安装自动向全员开放；不预选外部兼容标准、支付、账号SaaS或外部编码Agent。目录发现不等同于安全保证，来源不可用、离线、已安装版本和授权变化均应可解释。

##### 12.8.9 全路线出口（验证31.5、31.6）
...
**[2026-09-13 架构决策更新 · D18 小阶段发布]** 四批主题原按Phase11–13、14–20、21–24、25–30分阶段验收。**[2026-09-17 D19同步]** 当前为Phase11–13、15–21、22–25、26–31，另在第一、二批之间增加技术迁移Phase14；后续任务以开发计划§18.14的新编号为准。每阶段最后一项集成测试验证该阶段可发布增量，完整契约与安装产物证据见开发计划§18.11。Phase11只开放安装/环境检查；Phase12受控本机执行只在获批验收范围运行，普通项目入口等待Phase13保护、证据、退出恢复与Docker退役完整验收。11.2已在§12.3定稿安装升级、配置/新桌面状态兼容策略；后续每阶段验证上一已发布版本升级及设置保留。旧Docker任务不兼容不扩大为新桌面版本任意丢弃状态。未交付部分不可用必须明确可见，不伪装通过。


<a id="langgraph-contract-d19"></a>

### 13. LangGraph持久编排正式契约（D19 / Phase14）

**[2026-09-17 架构决策更新]** Leader授权定稿与登记并纳入PR #87。本节固定此前独立方案§28的契约，作为新路径唯一落码来源；设计已形成可审阅定稿，能力仍待14.1–14.7按阶段验收，不由文档存在开启模型/工作区执行。

#### 13.1 责任边界与阶段

13.4本机正式闭环与Docker退役出口完成后，Phase14接管工作项持久编排；14.7出口才切新任务默认引擎。LangGraph保存图任务与pending writes，TaskState保存业务及控制收据，Harness保存官方会话；三个存储无跨库原子提交承诺。图节点每次execute对应一次初始或恢复派工，包含完整Harness多Step；不拆模型/工具内循环。WorkerRuntime保留注册、任务写分区、安全点及lease；图控制层决定派工和批次汇合，二者共用当前task唯一提交队列。

coordinator/agent/integrate/finalize等节点可重入，因此持久业务先核验回执；graph interrupt必须在规范gate落盘、worker静止及资源收尾后才宣称等待。内部supervisor独立于super-step边界收集安全点，兄弟分支失败时自然all-settled，不把框架abort直接传给Harness token流。新路径退出旧while-loop/pump，旧路径在迁移前保持适用回归。

#### 13.2 身份与端口数据

以下为新companion数据契约；旧公开Executor/SandboxManager/TaskStateStore/MessageBus签名不变。实际值须经§13.4 exact-key schema验证，不是只靠TypeScript约束。
...
threadKey固定为SHA-256(agora-canonical-json-v1(["agora-langgraph-v1",projectId,taskId]))，scope明文映射随binding核验；不把群聊threadId或Harness sessionId当图身份。invocationKey由同scope、规范dispatch、worker和initial/resume动作派生。execute必须经当前owner绑定的runner能力调用，runAuthorizationId只作规范引用；工作区/模型/角色从当前受信绑定读取，不接受图自报配置。图只保存收据引用和必要指纹，不复制AppState、raw log、prompt、reasoning、工具正文或活能力。

#### 13.3 写入、存储、切换与兼容

新State新增受信orchestration={schemaVersion:1,binding,receipts}，仅当前获授权控制服务可提交；使用既有applyMutations通道下经验证的串行set/注册生命周期接缝，不让模型或并行普通输出直接提交。共享队列内基于最新State生成整个受信聚合变化；不可合并缓存AppState快照，不修改TaskStateStore签名。旧字段不强制回填，版本解析先区分原生legacy与新binding，损坏拒绝。

官方SQLite saver适配器位于runtime/state；L2仅调用GraphCheckpointPort，L1无I/O/框架依赖。SQLite只在受保护应用状态目录内，由合法owner管理；锁/损坏/busy/磁盘不足须有界失败并保全，不换MemorySaver、不重建空库。WAL与备份使用经过验证的官方/数据库机制；参数、耐久性与恢复以固定版本实测为准，不承诺断电零丢失。跨JSON/SQLite成功窗口通过业务回执对账，工具journal、Git HEAD和artifact映射各由原权威端口验证。

首次user_start可授权新任务执行；重启只读，原生legacy任务只读且不跨引擎续跑，旧Docker任务不重新提供兼容。未知graphDefinitionVersion不执行。切换前显式处置全部活动任务并核对规范终态/保全；升级回退核对整套状态与外部工作区版本，不能只回滚图库或覆盖用户代码。通用恢复仍不自动删除.desktop-owner，13.2与20.2/20.3分别验收宿主恢复和多会话整队恢复；本次只定义Phase14任务图的有限恢复能力。

#### 13.4 控制收据与Continue

##### 13.4.1 聚合、编码与身份

新引擎TaskState增加一个受信`orchestration`聚合：`{schemaVersion:1,binding,receipts}`。`binding`首次创建后不可变，`receipts`是按规范提交顺序排列的不可变收据数组。阶段、最近run epoch、当前激活与剩余容量由经验证的收据推导，不额外保存第二份可写worker状态表。旧任务不补该字段，新字段损坏不降级为legacy。

每份收据共同字段如下；各kind只允许自身声明字段，未知字段、未知kind、重复集合项及非法引用均拒绝。

| 字段 | v1约束 |
| --- | --- |
| `schemaVersion` | 固定1，不猜测旧/新格式 |
| `receiptId` | `ogr1:`加SHA-256；由scope、binding hash、operationKey、kind及kind规定的子键派生 |
| `scope` / `bindingHash` | 完整projectId/taskId，及已固定GraphBinding的规范hash；必须与所属State相符 |
| `operationKey` | 由规范控制来源产生的稳定键；禁止图checkpoint内部task id、模型自报ID或本次时钟替代 |
| `kind` | §13.4.2的封闭集合 |
| `inputFingerprint` | 该操作首次准入时控制输入的规范hash；字段范围见§13.4.3 |
| `sourceReceiptIds` | 同任务的前置收据有序集合，按ID排序，最多128项；不得自引、缺失、成环或引用提交顺序之后的收据 |
| `recordedAt` | 首次规范commit的非负安全整数毫秒；重放沿用首次值，不用时间戳证明权限或先后完成 |
| `producer` | 受信服务的封闭标签，供检查与审计；标签本身不授予写权限 |

hash输入使用本地纯函数`agora-canonical-json-v1`：只接受显式schema白名单的JSON基本值，数字限安全整数；对象键按ASCII字典序（字段名仅允许schema声明的ASCII键），UTF-8编码，不改写字符串，不接受undefined/NaN/Infinity或隐式toJSON；可选字段缺省即省略。集合先按契约排序，顺序有语义的向量保持原序。hash覆盖域分隔字符串、版本及完整规范对象；不截短hash。此函数须有固定字节向量和顺序/Unicode反例测试，不依赖对象插入顺序。

`receiptId`定位不可变记录；另算整份规范收据的`receiptHash`用于外部引用。它们用于一致性核验，不是签名或持有者凭证。同ID完整内容一致才幂等，任何异内容拒绝；调用方重试不得重造recordedAt或加入本次临时字段。保存前必须核对前置规范事实，不以hash自洽代替事实真实性。

##### 13.4.2 收据种类与原子提交

表内字段是共同字段之外的必填内容，`?`表示明确可选。引用均带规范身份及必要hash，不复制源正文。所有集合界限遵守§13.4.9。

| kind / producer | 额外字段 | 必须同commit的事实与约束 |
| --- | --- | --- |
| `dispatch.created` / `dispatch_service` | `workerId,assignmentBinding,roleBindingHash,modelBindingHash,workspaceBindingRef,controlInputRef,batchId?,waveRef?` | WorkerRuntime受信注册工厂生成的pending WorkerState与当前assignment；已有同worker异派工拒绝，不直接让图注册worker |
| `activation.prepared` / `invocation_service` | `invocationKey,dispatchReceiptId,runAuthorizationId,mode,sourceResumeRef?,reservedSlots,reservedBytes` | 在初始化可能产生session/工作区等持久效果的资源前，固定激活身份、来源及收尾容量；mode为initial或resume，resume必须绑定规范D4/后续已批准恢复计划 |
| `activation.started` / `worker_runtime` | `invocationKey,preparedReceiptId,sessionRef` | WorkerRuntime把自身置running、确定真实session身份的同次commit；活lease只在内存核验，不序列化为可复用能力 |
| `activation.finished` / `worker_runtime` | `invocationKey,startedReceiptId,outcome,evidenceRefs` | outcome为completed/quiesced/failed；与本次最后合法Step事实、对应worker生命周期变化一次提交。completed不表示任务完成；quiesced须引用已flush的safePoint；failed保留实际已提交效果与明确错误码 |
| `activation.not_started` / `invocation_service` | `invocationKey,dispatchReceiptId,pauseActionId,reason` | 只证明该暂停动作阻止排队中的激活；不能有prepared/started，亦未初始化该激活资源。不得伪造Fork或把worker置done |
| `activation.settled` / `invocation_service` | `invocationKey,finishedReceiptId,resourceSettlementRefs` | 核验原lease已释放及对应资源按既有协议收尾后记录；只释放该激活预留，不改变其业务结果。D18有界清理证明不扩张为所有后代已退出的承诺 |
| `run.authorized` / `lifecycle_service` | `actionId,runEpoch,ownerIncarnationId,cause,recoveryViewHash,cursor,plan,planHash,previousRunReceiptId?` | 首次启动沿用显式任务启动动作，cause=user_start；Continue为user_continue，当前新裁决的显式继续为gate_resolution。三者均绑定当前owner、稳定actionId及服务器规范计划；恢复视图在首次启动时由受信空图/新任务前沿构造。§13.4.5的单次用户启动/继续意图；单队列内确认没有另一个活动run，固定可恢复计划；epoch由规范历史递增，不接受客户端指定 |
| `run.entered` / `lifecycle_service` | `authorizationReceiptId,runEpoch,ownerIncarnationId` | 在调用graph之前持久登记本run已进入；同一合法内存runner负责调用，重复请求复用该runner，不再次invoke |
| `run.closed` / `lifecycle_service` | `authorizationReceiptId,reason,settlementRefs` | reason为completed/waiting_gate/paused/needs_attention；先收敛全部已激活分支并核验所需资源状态。资源未收敛时可记录needs_attention及保全引用，但不表示owner/资源可释放 |
| `batch.joined` / `join_service` | `batchId,expectedInvocationKeys,outcomeReceiptIds,controlFingerprint` | 固定集合全部有合法结果，canonical queue排空并重读后才提交；同批一调用恰一个匹配结果，不拿其他批历史结果补数 |
| `node.committed` / `control_service` | `node,canonicalEffectRef` | node限route/integrate/prepare_gate/finalize；若源事实在TaskState内，与源mutation同commit；外部受信回执先核验再引用，不冒充跨存储原子事务 |

`mode=initial`的invocationKey由dispatch派生；`mode=resume`另加入规范恢复动作ID，但workerId不变。prepared/started/finished各自一份，重放不产生第二份；`not_started`另以pauseActionId作子键，可在后续新run重新准入同一个尚未started的invocation，但其历史事实不能当completed。已finished的invocation永不再启动；quiesced之后只允许经规范恢复计划创建新invocation。done/failed worker保持既有不重开规则。

prepared中的runAuthorizationId记录首次准备来源，不是后续进程的执行能力。新run若要接管该尚未started的invocation，必须在其固定plan中明确列出并绑定准备资源核验结果，复用同一prepared及确定性资源身份；默认不能把prepared降为not_started并重新初始化。首版没有正式接管证明的准备中断保持needs_attention，实验中“派工后未准备”的可重试结论不覆盖这个窗口。

没有完成收据不自动等于未完成业务。prepared可能已创建child/session；started可能已发模型或工具；failed worker也可能已有文件效果。无法验证闭合边界时，inspect返回needs_attention，保存规范错误/保全事实，不捏造finished来让图过关。内存supervisor先让其他Harness自然收敛；不得把不确定错误直接交给图通用重试或取消兄弟token流。

业务完成与lease释放分开：finished提交后lease释放失败仍需处置资源；不得再次执行已完成Agent，也不得仅凭finished认为整个batch/run已经可关闭。D4完整gate提交、cohort释放、composition suspend、Leader resolution及resumed marker继续使用既有事实，本节只引用它们。

##### 13.4.3 输入指纹与证据引用

`assignmentBinding`引用并核验现有roleSlot或已启用的member/assignment身份；不得为未落地产品字段伪造memberId。模型绑定保存连接ID和配置版本/hash，不含凭据。workspace引用当前受信绑定ID、版本及授权代数，不接受图传入绝对路径。

操作首次输入至少包括scope、bindingHash、操作kind、worker/assignment、dispatch/activation来源、模型配置版本、要求该操作消费的需求/决策版本、workspace授权/输入版本，以及存在时的plan/wave/attempt/base和D4恢复引用。集合排序规则属于schema；不hash原始群聊、展示文本、时间、trace或整个AppState。实际session/safePoint在其生成后进入对应阶段证据，不倒填首次指纹。

运行期间D9非阻塞更新仍在规范安全点改变投影，不改写旧派工输入hash、不结束当前activation、不重新取lease。每次后续Step引用当前结构化指令/版本事实；结果证据绑定实际最终验证版本。恢复或下游消费结果时分别核验历史输入真实性和当前适用性，不能用“当前State hash变了”否定合法历史，也不能沿用旧授权执行新副作用。

`evidenceRefs`用封闭标签区分`task_message`、`harness_safe_point`、`workspace_operation`、`validation`、`artifact`。每种只接受其正式端口返回的opaque引用与固定hash；不接受通用URL/文件路径、任意对象或原始日志正文。D4 resolution引用规范Leader消息ID并复核其完整payload/Decision/workerResumes；不能只验证消息ID存在。opaque token交回对应提供方校验，控制服务不得自造Harness cursor、补半段JSONL或修改官方seed。
...
v1沿用append/mergeById/set三种mutation，不新增绕过`applyMutations()`的写入。新增受信`set('orchestration', nextAggregate)`只允许任务串行控制面使用：reducer校验binding不变、旧receipts是新数组的完整逐字规范前缀、只追加合法新记录、引用/状态转换/限额成立。同ID异内容、删除/重排/修改历史一律拒绝；相同聚合为no-op。

权限不靠JSON内producer或authority字段。图和worker回调只能向受信服务提交绑定调用身份的窄请求；服务验证本进程不可序列化的能力、当前task/run/worker分区，再在共享队列里重读最新State并生成mutation。模型输出解析器、并行输出校验器及控制服务边界都拒绝模型提交orchestration字段，无论角色是COORDINATOR还是其他角色。L1只做纯结构/转换校验，不导入I/O、LangGraph或进程能力；新聚合不进入角色投影/通用SSE/Trace。

现有TaskStateStore的commit串行化只覆盖给定mutation的apply/write，不能让外部`load → await → commit`自动成为CAS。新`TaskControlQueue`必须覆盖同task的**重读、条件校验、规划、commit**完整区间，并由组合根唯一注入Coordinator、WorkerRuntime、D9和生命周期服务；不能另建一个只保护图的队列。各worker只提交自己的Step/收据追加意图，当前聚合从队列最新State构造，不携带缓存AppState或完整替代快照。

队列内禁止等待模型、工具执行、用户裁决、lease acquire或资源drain。可先读取受信资源证据，再在提交前校验其身份/版本及仍持有的能力；对跨存储变化采用既有失效关闭与后置核验，不宣称这一队列锁住整个文件系统或collaboration CAS。操作需要当前worker停止时，队列内只登记关闭准入/固定cohort，队列外等安全点，再入队提交闭合事实。

##### 13.4.5 Continue 请求与响应

新增生命周期接口固定为`GET /api/tasks/:taskId/recovery?projectId=…`和`POST /api/tasks/:taskId/continue`，使用已有本机身份、origin/capability与scope验证。它们不能解析自然语言、裁决gate或改需求；那些操作继续只走`POST /api/messages`。端点最终注册属于14.6，当前产品尚无此能力。

只读GET返回有界DTO：`scope,disposition,reasonCode,recoveryViewHash,graphVersion,pendingSummary`。disposition限waiting_gate/continuable/needs_attention/running/completed/legacy_readonly；summary只有必要计数、角色和规范结果引用，无prompt/工具参数/内部收据正文。GET不得调用invoke、创建Context、取得worker lease、补写业务事实或回收owner锁。

POST body exact-key为`{projectId,actionId,recoveryViewHash}`，taskId仅来自路由；三者缺失、额外字段或非法scope先拒绝。actionId沿用D9安全token规则并限128 UTF-8字节；哈希为固定64位小写hex。客户端不能提供graph config、cursor、runEpoch、worker列表、模型/工作区参数或resume值。

`recoveryViewHash`绑定服务器读取的规范cursor（thread、namespace、checkpoint ID及框架pending task/write的有界摘要）、bindingHash、控制前沿、待处理gate/resolution、可恢复激活和资源/授权版本。它不是签名、执行能力或对整个AppState的hash。服务重新读取生成，过期返回409与需刷新提示；读取期间状态不稳定则有界重试读取后返回冲突，不把不一致快照签发成可继续视图。

| HTTP / code | 持久效果与含义 |
| --- | --- |
| 202 / `accepted` | 已持久run.authorized；可能仍在准备，不能展示为模型已运行 |
| 200 / `already_applied` | 同action参数及原事实核验通过；返回原authorization ID与当前disposition，不重开runner |
| 400 / `invalid_request` | 无业务写入 |
| 409 / `stale_view`、`action_conflict`、`run_active`、`gate_unresolved` | 不发新执行授权；新的用户点击必须基于刷新视图，不能后台换actionId循环重试 |
| 409 / `needs_attention` | 有未证实副作用、证据损坏或不可恢复版本；保全，返回白名单原因码 |
| 429 / `admission_capacity` | 在昂贵资源初始化和新授权commit前拒绝；重试原请求时仍须复核视图 |
| 503 / `owner_unavailable`、`storage_unavailable` | 关闭新准入；不能因写入响应未知就认定未接受，恢复读取后用原actionId对账 |

重放先按actionId查既有授权，再判断当前视图是否过期。已有动作复核原body、规范计划与后置事实；正常checkpoint进展不能让同请求误报stale_view。但若旧动作对应的事实损坏仍拒绝，不仅凭ID返回成功。

##### 13.4.6 Continue 提交与竞态

1. 验证有效desktop owner与当前内存incarnation；这是现有受信owner的附加审计标识，不替代其能力、不根据磁盘字符串恢复能力。
2. 在同task生命周期互斥区内检查action幂等、活动runner和有界composition admission。先保留该动作准入，再读取图/业务视图；等待worker lease不在这里发生。
3. 进入共享TaskControlQueue后二次核验相关业务/授权前沿。cursor必须来自该互斥区保护的稳定图读侧；后台存储写未闭合时不授权。原子追加`run.authorized`，cause固定为user_continue，plan记录待续办控制阶段及invocation引用，不允许任意新派工列表。
4. 在同一合法进程登记唯一runner，持久`run.entered`，再由新端口执行图。图恢复值仅由规范计划构造。每次正式执行仍在WorkerRuntime复核能力并取新lease；没有lease不能运行模型。
5. 任一步失败返回真实准备/待处理状态并补偿本次可证明归属的资源；存储写入结果未知时先对账，不释放互斥后启动第二个同义run。新请求不能覆盖旧授权或回滚worker来凑可运行状态。

两个窗口用不同actionId同时Continue：只允许第一个满足视图及准入的动作接受；第二个返回run_active或stale_view，不生成第二份授权。相同actionId并发：复用同一个在途结果；不得各自调用graph。相同actionId异body始终action_conflict。

`runEpoch`只增加，不因图重试增加。Continue授权当前任务从该恢复前沿继续正常领域流程，可经过后续Coordinator派工；不等于每个节点都再次要求用户点击。遇到gate、显式暂停、不可恢复错误、正常退出或任务完成时关闭run；下次进入执行须新的有效动作。新的派工仍按当前需求/权限和预算核验，run授权不提供额外费用或工作区权限。

##### 13.4.7 重启、D4 与 D9 的衔接

| 情形 | v1处理 |
| --- | --- |
| 授权已写，entered未写，同一进程响应丢失 | 原在途生命周期操作幂等完成登记；同action请求只复用该操作，不创建第二个runner |
| 授权或entered已写，服务进程退出 | 启动只读。旧action重放确认既有事实，不授权新进程执行；用户从当前视图提交新action，获得新epoch与owner incarnation |
| started未闭合 | 新Continue不能越过needs_attention；先按受信证据证明已完成/安全暂停/确未启动。未知效果不能用新action洗成新工作 |
| completed结果已证实但图落后 | 有效Continue下只补图结果与控制收尾，不再运行该activation |
| waiting_gate尚未裁决 | Continue返回gate_unresolved；D9仍是唯一裁决入口 |
| 当前界面新D9裁决且显式继续该任务 | 本次受信动作可构造cause=`gate_resolution`的run授权，只引用规范resolution；已满足准入可同commit，否则先提交裁决，在旧run收尾后由本次在途动作幂等追加授权。不能因容量/清理未完成阻止合法裁决持久化，也不得第二次解析或伪造裁决 |
| gate在退出前已裁决或旧D9请求重放 | 复核既有receipt/marker；新进程不由旧HTTP重放启动执行。需新的Continue；不能重复清gate、创建child或改Decision |
| child已经创建、prepared后退出 | 先经官方loader核验header、完整seed、scope/cwd及规范恢复计划，再接管同child；无法证明时needs_attention，不创建第二个child |

D4 resume对同worker产生新activation；同进程非阻塞reproject保留原activation/context/lease。`run.authorized`不替代D4 safePoint、resumed marker或D16批准。`run.closed:completed`也不自行设置`isRequestSatisfied`；须已有同一验证版本的Leader完成批准和归档事实。

D9的裁决提交与后续执行不伪装成必须同时完成的跨资源事务。延后追加run授权仅属于仍有效的同进程首次在途裁决动作；它中断或进程退出后，不靠重投旧D9请求重新取得执行意图。用户改用新Continue，仍引用已经规范提交的resolution。授权之前始终等待旧run收敛并重读事实，不能因gate已清除就同时启动两个composition。

跨进程恢复将旧run按证据关闭并建立新epoch，但不能凭“旧PID没了”证明其后代/工具资源已停用。宿主owner协议、原生资源收敛与任务业务恢复分别验收。本节不实现自动清理`.desktop-owner`；13.2及20.2/20.3的阶段边界仍按来源执行。

##### 13.4.8 新 companion 端口与实现顺序

下列companion端口职责定稿，不修改冻结Executor/SandboxManager/TaskStateStore/MessageBus方法。端口由组合根注入，L2不导入SQLite或文件I/O。

| companion / 操作 | 输入与返回 | 所有者 |
| --- | --- | --- |
| `TaskControlQueue.withCurrent` | scope、受信短规划函数 → 规范commit结果 | 组合根每task唯一队列；不接受renderer/模型函数 |
| `OrchestrationReceiptService.inspectInvocation` | scope、invocationKey → not_started/running/completed/quiesced/failed/needs_attention及已验证引用 | L2纯规则+L3证据读取；running还须当前合法内存handle；not_started须无prepared/started且正式资源证明未启动，缺证据是needs_attention |
| `OrchestrationReceiptService.prepareInvocation` | 已授权run、规范dispatch、initial/resume来源 → 固定invocation引用 | L2受信调用，内部同队列预留容量并提交 |
| `HarnessWorkerInvocationPort.execute` | §13.2的AgentInvocation → 有规范收据的AgentNodeOutcome | WorkerRuntime companion；完整Harness保持原实现 |
| `GraphCheckpointPort.inspect` | scope、固定binding → 有界只读cursor/待处理摘要；cursor包含threadKey、checkpointNamespace、checkpointId及pending摘要 | L4官方saver适配；未经对账的结果不直接交UI |
| `GraphExecutionPort.continue` | scope、规范授权引用及当前内存runner能力 → 在途run handle | L2图执行与L4端口组合；拒绝客户端RunnableConfig |
| `TaskRecoveryService.inspect/continue` | scope；或§13.4.5请求 → 安全DTO | Web生命周期组合根；错误DTO不透传底层异常 |

14.2先实现纯schema/转换、可信写入和唯一队列；14.3接WorkerRuntime单派工与顺序图，14.4接并行，14.5闭合D4/D9/D16，14.6公开只读inspect/Continue及发布打包。各阶段使用受信测试组合验证已实现接缝，不提前开放产品入口；最终14.7统一故障和等价验收。不能先从实验payload直接上线，再补权限；不能先把while-loop删掉再寻找等价生命周期。

##### 13.4.9 限额、资源保全与回收

v1首版限额：每任务最多4096份收据；每份规范编码最多16 KiB，整个orchestration聚合最多8 MiB；每批最多128个invocation（实际准入还须满足剩余收据/字节预留，128不是必须同时可执行的承诺），每份引用集合最多128项。服务一次DTO最多100项摘要及64 KiB，不公开完整聚合；分页游标绑定scope/binding/视图，过期刷新。opaque引用仍受所属端口更严格的原有上限，超限不得截断后继续。

每次准入activation前预留32份收据及512 KiB收尾额度；每个活动run另预留同样额度用于gate、关闭和终态控制。剩余通用容量不足则在准备session/工作区前拒绝新准入。预留由prepared/授权与settled/closed事实推导，同一余额不能被两个并行准入重复使用；finished不单独释放激活预留，failed但资源未收敛仍保留所需额度。预留消费只允许该操作的封闭收尾种类，不能用来派新工作；每份实际收据同时占总量一次，消费预留相应扣减，避免双重计数。错误重试不逐次追加无界诊断收据，只复核固定操作的事实；详细失败留既有受保护诊断通道。

这组数值是有界首版工程参数，须在14.2容量/退出验收实测，不是性能测量结论。若无法证明最坏合法收尾能落在预留内，必须调整schema限额/拆分计划后再开放执行；禁止靠收尾写失败后忽略回执来通过验收。数据库/磁盘真实故障仍可能导致无法持久收尾，此时保全并needs_attention，不声称预留能防止磁盘满。

活动任务不删除任何仍被恢复引用的控制收据。终态清理由既有任务归档/保留策略统一负责，保留业务、官方会话、图和artifact之间的引用闭包；不在节点重放时自动压缩、覆盖或删除历史。达到上限不自动迁移数据库、不切MemorySaver、不删前缀。

##### 13.4.10 契约验收与本轮完成范围

| 编号 | 必须证明的行为 | 证据要求 |
| --- | --- | --- |
| C01 | 规范编码/身份稳定，同ID异内容和未知schema拒绝 | 固定字节向量、属性顺序/集合/边界反例；纯测试 |
| C02 | 模型/renderer伪造producer、收据、配置和mutation均无法授权 | 实际解析/输出校验/受信服务边界测试，不只测类型 |
| C03 | dispatch+worker、started+running、finished+结果同commit | 正式TaskState路径前后故障注入；无业务/收据半边状态 |
| C04 | D9、worker Step、Continue共享队列，过期规划不覆盖新事实 | 并发交错，真正读后等待与二次核验；不能只测顺序调用 |
| C05 | prepared/started不自动视为未执行，未知效果保持待处理 | 正式Harness/session/工具journal故障窗口；真实调用次数 |
| C06 | pending writes/终态跳过节点时仍核验业务 | 外层入口+节点内部双重检查，坏业务证据拒绝 |
| C07 | 同action重放、异action竞争和写入结果未知不生第二个run | HTTP/生命周期/进程重启联合验证，检查epoch/runner/调用次数 |
| C08 | 旧Continue/D9重放不授权新进程，新的显式继续可推进合法状态 | 原owner退出、新owner合法取得、真实会话恢复，无自动调用 |
| C09 | 新D9裁决/Continue不替代D4、D16或扩大授权 | 真实gate提前/重复/暂停清理、同child接管、Leader终审闭环 |
| C10 | quiesced/failed/not_started与finished、资源释放语义不混淆 | 多worker混合结果、释放失败、排队取消、done/failed不重开 |
| C11 | 边界容量下仍能完成gate/暂停/关闭，不无限追加诊断 | 满水位和最坏收尾实测；磁盘失败保持保全/待处理 |
| C12 | 只读inspect无执行、无owner时关闭入口、版本/作用域不匹配拒绝 | 正式应用和受管Node上的重启/多窗口/版本场景 |

C01–C12是必须满足的验收要求，不是本次设计定稿已通过的测试。正式任务与S/V矩阵映射见开发计划§18.13；准备中断与真实工具无收据窗口不得用计数fixture替代。14.1复核前置机制，14.2–14.6逐项实现，14.7累计验收。

##### 13.4.11 有界字段与一致性补充

新actionId/operationKey及binding各版本标签限128 UTF-8字节，既有scope/worker/opaque引用沿用所属端口更严格界限；hash固定64位小写hex，receiptId固定ogr1:加64位hex。所有exact-key对象拒绝未知字段，数组拒绝重复与越界，不按JSON序列化偶然行为推断schema。GraphBinding的checkpointNamespace首版固定空字符串，禁止调用方任意传入子图namespace；checkpointId仅来自官方saver读取。父/子session验证仍经官方loader。

外层inspect必须验证完整receipt DAG和业务引用；仅图里存在结果、无pending任务或直接END都不能跳过此校验。图binding缺失只在明确legacy格式路径处理，受信新格式缺字段或损坏不得静默退成旧任务。运行时capability通过已绑定端口实例及调用参数核验，不能以JSON里的producer/runAuthorizationId代替能力。

每次activation执行必需当前runAuthorizationId与当前runner能力；跨进程接管复用既有prepared时，以新run.plan显式授权该invocation并核对资源，不改写prepared中的首次授权。finished缺失而实际效果未知时不生成新的dispatch来绕过。单个receipt来源最多128项，batch.joined只汇合最多128个激活；run.closed以有界batch/settlement回执引用汇总，不一次引用全任务4096项。14.2须给出每种合法收尾的最坏收据/字节上界，32份/512KiB不足时先同步本节及来源，不能开放执行后再依靠丢日志或截断收尾。
