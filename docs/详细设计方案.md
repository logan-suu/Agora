...
正式候选使用`pnpm build:desktop <verified-download-directory> --revision <git-commit>`，输入提交必须含本任务已审阅代码；默认工作树模式仅供未经提交授权的研发验证，标记`development-working-tree`并记录每文件hash，不能作为固定提交发布候选。本轮构建入口只接受darwin-arm64；x64与完整安装工具链交付由11.4完成，平台承诺不变。当前只读写格式1；有非空upgrade记录、未知格式或未声明转换均拒绝，完整事务由11.4实现验收。
