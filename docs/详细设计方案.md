...
// sub participants/createdBy 可引用任意已知 roster 身份以保留历史；新建 sub、agent 发言和 worker 执行只允许 enabled。
...
**[2026-09-02 架构决策更新] D12 Phase 7 角色契约：** `RoleSpec` 是不含启停状态的能力定义，`RosterEntry={spec,status}` 才是项目成员。role id 入库前转大写并校验 `[A-Z][A-Z0-9_-]*`，按规范化值唯一，`leader` 保留；`COORDINATOR` 必须存在且恒 enabled。`addRole` 对同 id、同 spec、enabled 的重放为 no-op，对同 id 不同 spec 冲突；`enableRole` 只接受 disabled，`disableRole` 只接受 enabled，同态重放为 no-op；departing/departed 只能由阶段 7.2 的离职状态机推进，普通 enable/disable 不得越过。任何消费 roster 的 Coordinator、Leader Intent、WorkerRuntime、Channel 生命周期和消息提交入口都必须检查 enabled，不得只检查条目存在。
...
Roster 操作是纯函数产生下一份 `{roster,channels}`：`addRole` 新增 enabled 条目并重算 main；`enableRole`/`disableRole` 只做合法状态转换并重算 main；同态操作为 no-op；Coordinator 的非 enabled 转换、同 id 异 spec、普通操作触碰 departing/departed 均明确失败。`removeRole` 不属于该底层管理器，它由 7.2 的离职服务驱动 `enabled|disabled → departing → departed`，在最终转换前必须已有合法 HandoffPacket 与责任转移结果。
...
**[2026-09-02 架构决策更新] D12 Phase 6→7 迁移：** `collaboration.json` 缺失而 `channels.json` 合法时，以六角色 `DEFAULT_ROSTER` 生成 enabled `RosterEntry[]`，连同原 channels 写入临时文件、原子 rename 并复核，再移除 legacy 文件。崩溃导致两文件并存时，仅当 legacy channels 与 collaboration channels 语义相同才允许完成清理；内容不一致必须 fail-fast。`channels.json` 损坏、协作快照损坏或任一不变量失败都不得回退默认值。迁移完成后 `channels.json` 不再是事实源，也不得继续双写。
