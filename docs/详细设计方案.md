...
> **当前版本 v3.8 / 2026-09-07：** 上游对齐蓝图 v1.7，补齐 Integration 前缀状态机、单步丢失回执证明、canonical Git common-dir、worktree 创建/回收补偿、当前波次投影与并发终态归档规则。下方单行版本串仅保留历史沿革，凡与当前标记规格冲突均以本文后续最新的 `[日期 架构决策更新]` 为准。
