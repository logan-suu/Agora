...
> **路径映射（旧 → 新，下文正文旧路径均按此理解）**：`core/state.ts`→`core/domain/state.ts`；`core/orchestrator.ts`、`core/coordinator.ts`、`core/worker-runtime.ts`→`core/orchestration/`；`core/preemption.ts`→`core/preemption/preemptor.ts`；`core/projection.ts`、`core/ledger.ts`、`core/roster.ts`→`core/domain/`；`executors/`→`runtime/executor/`；`sandbox/`→`runtime/sandbox/`；`comm/bus.ts`→`comm/bus/`；`comm/intent.ts`→`apps/web/lib/intent.ts`（Phase 5 前端实现）；`TaskStateStore`→`runtime/state/`（D6，Phase 5）。
