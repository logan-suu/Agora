...
> **当前版本 v3.7 / 2026-09-07：** 上游对齐蓝图 v1.6，在既有 D17 真并行契约上定稿 worktree 唯一所有权、结构化 `WorktreeRef`、可恢复 `Integration`、专用集成 worktree、冲突 Leader gate，并删除无消费者的 `pendingPatch` / `branchOrPatch` 占位。下方单行版本串仅保留历史沿革，凡与当前标记规格冲突均以本文后续最新的 `[日期 架构决策更新]` 为准。
...
**TESTER（薄或厚）**——按 acceptance 写并跑测试，回填 `testResults`；失败推回对应 Coder。工具：fs.read/write、sandbox.run、git。投影：requirements.acceptance、当前 `branchOrIntegration` 引用、接口约定。Prompt：「你是测试者，以验收标准为客观判据编写并运行测试，产出结构化结果，不修业务代码。」
...
*收敛：* Phase 0–8 顺序退化为 test 全过 + REVIEWER 通过只生成 `completionCandidate` → D16 `completion_confirmation` gate → Leader `approve_completion` → finalize/done。Phase 9 的拓扑波次必须先完成 `并行 CODER → integrating（专用 integration worktree）→ 单一集成态 TESTER`，全部波次测试通过后才进入 REVIEWER 与同一 D16 Leader 终审；不得在 Leader 批准完成后才首次合并未经集成测试的分支。9.3 落 workspace/integrate 服务与显式节点，9.4 再将 Coordinator 的拓扑波次自动路由接入。Leader `request_changes <rationale...>` 则按现有保守策略重开待修改 subtasks、路由 CODER，并等待新一轮集成、测试、review 与新 gate。
...
> **[2026-09-07 架构决策更新] `branchOrIntegration` 切片：** 9.3 删除旧 `pendingPatch` 与 `branchOrPatch` 占位，TESTER/REVIEWER 改读同名 `branchOrIntegration`，返回上方精确 `BranchOrIntegrationProjection` shape。`worktrees` 只含当前任务、当前可审阅波次中已与 WorkerState/Subtask 交叉验证的条目，按 `{subtaskId,workerId}` 稳定排序；`integration` 只含当前 canonical Integration 的结构化引用与进度。所有层级都作防御性拷贝，不携带代码、patch 或 diff 全文；调用方凭 branch/head/resultCommit 用 git 工具按需取 diff。字符串 legacy worktree 未完成真实 Git 迁移时 fail-closed，不得投影猜测值。
