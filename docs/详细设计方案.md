...
生产当前仍只读写桌面格式1，没有已批准的1→2领域迁移，也不对外提供转换器注入或任意迁移控制API。升级内核用明确的专用测试格式及临时密文/身份fixture验证断点恢复；未来领域转换仍由14.1/15.1定稿接入。格式变化未实装不等于可以绕过未知格式拒绝；升级准备目录连首份journal也未完整落盘时保持阻塞，由受信恢复核验处理，不能凭目录年龄自动删除。
...
6. **恢复先校验再执行**：13.2只恢复已验证的新本机绑定；核对状态格式、scope/根身份、当前文件/HEAD、checkpoint与官方lineage、模型绑定及规范控制回执。缺绑定、旧Docker描述、跨task路径和未知版本均在副作用前拒绝；没有恢复证明就显示可解释故障，保留文件与配置，不能创建空工作覆盖旧事实或自动重跑。D4保worker身份、真Fork及新lease不变；完整异常整队恢复仍属19.2。
