...
> **当前版本 v3.6 / 2026-09-05：** 上游对齐蓝图 v1.5，新增 D17 真并行调度、稳定 worker 身份、任务级 canonical commit join 与 GlobalScheduler lease 契约；顶层 worker 使用现有 `HarnessExecutor` + 独立 `Context` / `ctx.agents.create()` Agent / JSONL session，one-shot `ctx.subagents.start()` 不承担顶层长跑 worker 生命周期。下方单行版本串仅保留历史沿革，凡与当前标记规格冲突均以本文后续 `[2026-09-05 架构决策更新]` 为准。
...
> **[2026-09-05 架构决策更新] D17 并行写入边界：** Phase 9 启用 `workers` 的 `mergeById` 写入；每个 assignment 必须在启动前以稳定 `workerId` 追加/更新自己的 `WorkerState`，同一逻辑 dispatch 重放不得生成第二个 worker。并行 worker 仅可：① append 由该 step 产生、带稳定身份且通过既有生产者校验的事实；② merge 自己 `workerId` 的 WorkerState；③ merge 自己 `subtaskId` 的非控制面进展字段。不得并行写 `requirements`、他人 worker/subtask，也不得产生任何 `set` mutation；`phase/testResults/nextRole/iterationCount/humanGate/integration/architecture/conventions/complexity` 继续由 Coordinator 或专用服务在任务串行控制面提交。`mergeById` 只有在不同身份分区、或同一 worker 内保持程序序时才具交换语义；旧“所有 set 也可交换”表述无效。并行 mutation 权限在 TaskStateStore commit 前校验，违规不落盘。
...
>
> Tier 2 真实流水线按**拓扑波次**推进：Coordinator 从 `todo` 中选择 `dependsOn ⊆ doneIds` 的全部就绪 subtask，以稳定 id 排序后生成 CODER batch；可选 TESTER preparation assignment 只能产出 append-only 测试计划/测试文件分支，不得写全局 `testResults`。同波次 worker 全部 settled 后进入 integrate，9.3 按稳定顺序合并 workerId worktree；合并成功后只运行一个集成态 TESTER 写权威 `testResults`。测试通过才由 Coordinator 将该波次 subtasks 标 `done` 并激活下一波；失败或冲突按现有返工/humanGate 路径处理。由此保留单一全局 `testResults`，不引入多个 Tester 对同一 set 字段竞争。
...
class WorkerRuntime {
  ...
  private async pump(queue: Assignment[], join: CanonicalTaskJoin): Promise<void> {
    while (queue.length > 0 && !join.hasFailure) {
      ...
      const lease = await this.scheduler.acquire(join.projectId, assign.workerId);
      try {
        if (join.hasFailure) {
        ...
        await this.scheduler.release(lease);  // 完成/失败均幂等释放
      }
    }
  }
  ...
  private async loop(join: CanonicalTaskJoin, h: WorkerHandle): Promise<void> {
    while (!h.done) {
      if (this.paused) { await h.saveSafePoint(); return; }
...
**worktree 分配 / 依赖序 / 集成：** 9.3 以 `createWorktree(taskId, workerId)` 语义调用冻结的双字符串端口，给每个稳定 worker 身份独立分支；worker 文件操作限其内，同 role 不复用目录。Coordinator 按 `subtask.dependsOn` 形成就绪拓扑波次；`integrate` 节点按拓扑序、再按 workerId 稳定排序合并 `integration.pendingBranches`，冲突默认升级 leader。
...
POST /api/tasks  // { projectId, taskId, requestId, goal } → 202 started / 200 idempotent terminal-or-active / 409 same-scope goal conflict（D17 起无跨 scope 容量冲突）
...
interface SandboxManager {
  createWorktree(taskId: string, role: string): Promise<Worktree>;   // 参数名为历史兼容；Phase 9 传稳定 workerId，禁止同 role 碰撞
...
class GlobalScheduler {             // 所有项目之上唯一的跨项目调度者
  ...
  release(lease: SlotLease): Promise<void>; // 幂等；完成/失败/suspend 都必须释放
  account(projectId: string, usage): void; // 按项目成本出账
}
```

> **[2026-09-05 架构决策更新] D17 GlobalScheduler lease 与公平性：** `grantSlot(): boolean` 无法表达等待、公平、取消和幂等释放，Phase 9 改为异步 lease 端口。`SlotLease={leaseId,projectId,workerId,grantedTs}`，同一 worker 的未释放 acquire 幂等返回同一 lease；释放时完整校验归属，重复释放 no-op，错配 lease fail-closed。调度队列按 project 轮转：有空闲槽时先让每个有等待工作的项目各取得一个，再把剩余槽位继续轮转分配；当等待项目数大于 cap 时后来的项目排队，因此旧“每个活跃项目同时保底 1”只表示防饿死目标，不是不可实现的容量承诺。`WorkerRuntime.maxParallel` 只限制单 task 同时发起数且不得超过 scheduler cap；全局在跑 worker 数只由有效 lease 计数。
>
> 任务 9.1 落 scheduler 核心、稳定 lease、正常/失败释放和 WorkerRuntime 队列；任务 9.2 再把 D4/全局抢占接入：只有 worker 到 step/end、checkpoint flush 且 composition suspend 收尾后才释放其 lease，resume 重新排队取得新 lease，旧 lease 不跨 Fork 复用。进程重启后的普通活动 run 仍按既有边界视为 interrupted；自动恢复留 10.2，不得从残留内存 lease 推断仍在运行。
